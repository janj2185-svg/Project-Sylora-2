...
- editable profile and UK/EN/PL/DE/RU interface-locale preference
...
- responsive Living Horizon web shell with separate desktop/tablet/mobile composition, route-specific ambient motion and reduced-motion support
- visible creator connection surfaces for TikTok owner relay, YouTube, OBS, TikFinity and RTMP(S), with truthful readiness labels rather than decorative connected states
...
AI uses a real OpenAI provider when `OPENAI_API_KEY` is configured and fails explicitly when it is not. Payments, object storage/CDN, production SFU/browser-scale media, per-platform transcoding and OAuth/chat connectors, creator payouts, signed App Store/Google Play distribution and large-scale infrastructure are not implemented in this slice. The Android/iOS development-build client is runnable, but store signing and release still require the owner's Apple/Google accounts. The optional RTMP(S) distribution control plane is code-ready but still needs public DNS/TLS, platform stream keys and an operator-run end-to-end test. The current SYLORA viewer transport remains deliberately P2P; external STUN/TURN can be configured, and the current media/HLS pipeline is local-server based.
