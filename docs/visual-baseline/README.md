...
- launch headless with no browser channel or custom executable; hard-pin `PLAYWRIGHT_LEGACY_SCREENSHOT=1` in the isolated visual child process so Playwright omits `CDPScreenshotNewSurface`, add exactly one bare `--run-all-compositor-stages-before-draw` switch so Chromium serializes compositor pipeline stages before completing a frame, then use browser-level CDP to prove that exact runtime switch plus the pinned `chromium-headless-shell` distribution, revision and executable, require `--disable-field-trial-config`, and reject any runtime `--enable-features` entry that enables `CDPScreenshotNewSurface`; this switch is a deliberately isolated scheduling control, not a claim that every possible paint path universally converges;
- record the exact rendered head commit, verified browser fingerprint including `screenshotBackend = legacy-force-redraw` and `compositorScheduling = all-stages-before-draw`, Playwright/Chromium versions, runner OS/image, font family, DPR and input mode without persisting the absolute executable path; both browser controls are capture-only and change no production UI, motion or composition;
...
- record the fail-closed compositor proof, scheduling mode and screenshot backend in raw capture schema 8 and propagate that required provenance into promoted manifest schema 4; then re-read every PNG from disk, verify its recorded SHA-256 and dimensions, and prove that all 44 persisted digests plus the full browser fingerprint match between independent capture and repeat passes.

A raw PNG is written only after its runtime profile, diagnostics and in-memory screenshot have passed. Immediately afterward, the runner validates the persisted bytes and atomically/exclusively creates one canonical record sidecar under `tmp/playwright-visual-results-<mode>/capture-ledger/`. After every ledger write and again during worker teardown, it rebuilds `metadata.json` from the cumulative ledger, revalidates each referenced PNG plus commit/run/browser/runner consensus, rejects PNGs without records or records without PNGs, sorts the records canonically, and atomically replaces the report. The ledger therefore preserves successful records when Playwright restarts its pinned single worker after a failed viewport instead of silently reporting only the final worker's memory.

Failed runs that reach Playwright teardown retain the complete successfully validated subset, mark `metadata.json` as `INCOMPLETE_VISUAL_CAPTURE`, and omit `capture-metadata.json`. A process interruption between the PNG and ledger write is detected as an orphan PNG; an interruption before any later aggregation may leave `metadata.json` absent. Every such state is non-promotable. Ledger sidecars stay outside the raw candidate, are diagnostics-only, and are uploaded only with the failed-run Playwright diagnostics. On an A/B mismatch, those diagnostics also retain the already-captured raw A and B frames, their digests and viewport context, plus the later catch screenshot and capability-only JSON; no retry frame or credential-bearing Playwright trace is produced.

A complete GitHub-run raw capture directory contains exactly the 44 PNGs plus two non-promotable sidecars; a local complete run intentionally omits terminal-run provenance, and the diagnostics ledger is deliberately not part of either directory:
