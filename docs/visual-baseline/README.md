...
- after exact style restoration, discard one fixed full-page paint-fence frame, prove canonical geometry, visibility and source state, then take final full-page frames A and B across only a fixed compositor-frame boundary with no intervening DOM/layout probe, retry or cherry-picking; require A and B to be byte-identical, re-check canonical state after B, require locked canonical contrast and require every visible canonical crop to differ from its stable hidden crop, then persist only B;
