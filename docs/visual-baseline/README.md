...
- after exact style restoration, prove scroll origin plus canonical geometry, visibility and source state before the fixed discarded full-page paint fence; because those reads can flush layout and rebuild composited backdrop layers, no DOM/layout/scroll probe may run between that warmup and final frames A and B; take A and B across only fixed compositor-frame boundaries with no retry or cherry-picking, require them to be byte-identical, re-check canonical state after B, require locked canonical contrast and require every visible canonical crop to differ from its stable hidden crop, then persist only B;
