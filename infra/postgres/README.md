...
`schema.sql` is migration `001_initial_schema`; subsequent immutable migrations live under `migrations/` (currently through `014_session_status_invalidation`). Apply all pending migrations with `npm run db:migrate`. The runner holds a PostgreSQL advisory lock across the ordered run and records every migration plus its SHA-256 checksum in `_sylora_migrations`, so concurrent deploys serialize, re-running is safe, and editing an already-applied migration fails loudly instead of silently rewriting production history.
