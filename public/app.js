...
const REFERENCE_ROUTE_LABELS={feed:'Добрий вечір',live:'SYLORA LIVE',clips:'Clips',videos:'Відео',studio:'Creator Studio',ai:'Sylora AI',explore:'Відкриття',communities:'Спільноти',learning:'Навчання',business:'Бізнес',messages:'Inbox',profile:'Профіль',gifts:'Гаманець',more:'Налаштування'};
function syncReferenceShell(view){
  ...
  const routeTitle=document.querySelector('#routeTitle');if(routeTitle)routeTitle.textContent=REFERENCE_ROUTE_LABELS[view]||'SYLORA';
...
async function renderFeed(){
  ...
  const rawName=String(state.me?.displayName||state.me?.username||'Іване').split(/\s+/)[0];
  ...
  app.innerHTML=`<div class="screen-grid home-screen">
    <section class="content-stack">
      <div class="hero-copy">
        ...
        <h1>${esc(hello)}, ${esc(greetingName)}<span>✦</span></h1>
      ...
      <div class="quick-worlds">
        ...
        <button class="world-button" style="--world-accent:#16899b" type="button" data-horizon-view="studio"><span>${referenceIcon('studio')}</span><b>Studio</b><small>5 підключень</small>${referenceIcon('arrow')}</button>
        ...
        <button class="world-button" style="--world-accent:#ad7a28" type="button" data-horizon-view="business"><span>${referenceIcon('business')}</span><b>Бізнес</b><small>+18%</small>${referenceIcon('arrow')}</button>
      </div>
      ...
      <div class="home-feature-grid">
        <button type="button" class="glass-card featured-live reference-live-button" data-horizon-view="live"${featured?.id?` data-live-id="${esc(featured.id)}"`:''}>
          <img src="/assets/reference/phoenix-master.png" alt="Кінематографічна сцена Phoenix Rebirth"><div class="media-shade"></div>
          ...
          <div class="live-copy"><small>SYLORA ORIGINAL</small><h3>${esc(featured?.title||'Phoenix Rebirth')}</h3><p>Живий світ реагує на аудиторію, подарунки й голос ведучого.</p><div><span class="avatar avatar--host">ES</span><b>${esc(featured?.host?.displayName||'Eva Shine')}</b><span>• AI-directed show</span></div></div>
        </button>
        <div class="moment-list">
          <article class="glass-card moment-card" data-eco-nav="clips"><span class="moment-icon violet">${referenceIcon('clip')}</span><div><small>НОВИЙ CLIP</small><b>Момент із вчорашнього LIVE</b><span>24K переглядів</span></div>${referenceIcon('play')}</article>
          <article class="glass-card moment-card" data-eco-nav="learning"><span class="moment-icon mint">${referenceIcon('learning')}</span><div><small>ПРОДОВЖИТИ</small><b>${esc(continueItem?.label||'AI для креаторів · урок 4')}</b><span>18 хвилин</span></div>${referenceIcon('arrow')}</article>
          <article class="glass-card moment-card" data-eco-nav="business"><span class="moment-icon gold">${referenceIcon('business')}</span><div><small>БІЗНЕС</small><b>Дохід виріс на 18%</b><span>за останні 7 днів</span></div>${sparkline([26,31,39,45,43,58,70])}</article>
        </div>
      </div>
    ...
    <aside class="context-stack">
      <article class="glass-card pulse-card"><div class="card-top"><span>${referenceIcon('activity')} LIVE PULSE</span><span class="status-pill status-pill--success">online</span></div><div class="pulse-number"><b>${totalViewers.toLocaleString()}</b><small>людей зараз у ваших просторах</small></div>${sparkline([26,38,32,56,47,65,61,78,72,91])}<div class="split-stats"><span><b>+18%</b><small>залучення</small></span><span><b>04:32</b><small>середній час</small></span></div></article>
      <article class="glass-card"><div class="section-title"><div><h2>Важливе</h2></div></div><div class="focus-list">
        ...
        <button type="button" data-horizon-view="gifts"><span class="focus-icon gold">${referenceIcon('wallet')}</span><span><b>+840 LUMEN</b><small>дохід за сьогодні</small></span>${referenceIcon('arrow')}</button>
      </div></article>
      <article class="glass-card event-card"><span class="event-date"><b>29</b><small>СЕР</small></span><div><small>НАСТУПНА ПОДІЯ</small><b>Creator Future Meetup</b><span>Завтра · 19:00</span></div><button type="button">${referenceIcon('calendar')}</button></article>
    </aside>
  </div>`;
...
async function renderWalletReference(){
  ...
  app.innerHTML=`<div class="wallet-layout">
    ...
    <aside class="context-stack">
      <section class="glass-card gift-preview"><span class="status-pill status-pill--gold">LIVE GIFTS</span><img src="/assets/reference/phoenix-master.png" alt="Живий подарунок Phoenix" loading="lazy"><h3>Phoenix Rebirth</h3><p>Світло, звук і рух запускаються у LIVE через власний runtime SYLORA.</p><button type="button" data-wallet-live>Відкрити LIVE ${referenceIcon('arrow')}</button></section>
...
async function renderExploreReference(){
  ...
  <section id="discoveryDefault" class="discovery-grid">
    <article class="glass-card discovery-hero"><img src="/assets/reference/phoenix-master.png" alt="Phoenix Rebirth у світі SYLORA"><span class="live-stage-shade"></span><div><span class="status-pill status-pill--gold">LIVING HORIZON</span><h2>Світ, який відповідає.</h2><p>Відкривайте нові голоси й ідеї без копіювання чужої стрічки — у власній мові SYLORA.</p></div></article>
    ${[
...
async function renderLive(){
  ...
  const lifecycle=[
    ['lifecycle-mint','↻','Reconnect','автоматично',true],['lifecycle-cyan','⌁','Мережа','adaptive',true],['lifecycle-violet','▣','Камера','за запитом',false],
    ['lifecycle-blue','◉','Мікрофон','за запитом',false],['lifecycle-sylora','✦','Sylora','готова',true],['lifecycle-gold','♧','Гості','co-host',false],
    ['lifecycle-rose','♡','Подарунки','realtime',true],['lifecycle-teal','CC','Субтитри','AI',true],['lifecycle-steel','⛨','Модерація','активна',true]
  ];
  ...
  <div class="live-layout">
    <section class="live-lifecycle"><div class="lifecycle-heading"><div><small>FULL LIVE LIFECYCLE</small><b>Важливий стан завжди видно — від підключення до завершення</b></div><span class="status-pill status-pill--success">${rooms.length} LIVE · ${viewers.toLocaleString()} viewers</span></div><div class="lifecycle-row">${lifecycle.map(([tone,icon,label,status,active])=>`<button type="button" class="lifecycle-control ${tone} ${active?'active':''}"><span>${icon}</span><span><b>${label}</b><small>${status}</small></span><i></i></button>`).join('')}</div></section>
    <section class="live-stage-card"><img src="/assets/reference/phoenix-master.png" alt="SYLORA LIVE stage"><div class="live-stage-shade"></div><div class="live-stage-top"><div><span class="status-pill status-pill--live">${referenceIcon('live')} LIVE</span><span class="status-pill">${Number(stageRoom?.viewerCount||0).toLocaleString()} дивляться</span></div><button type="button" data-live-tab="studio" aria-label="Відкрити Studio">${referenceIcon('studio')}</button></div><div class="live-stage-copy"><small>SYLORA ORIGINAL · LIVING HORIZON</small><h1>${esc(stageRoom?.title||'Phoenix Rebirth')}</h1><p>${stageRoom?'Живий простір реагує на аудиторію, чат і голос ведучого.':'Створи перший LIVE — сцена, чат і Sylora вже підготовлені.'}</p><div class="host-line"><span class="avatar">${esc((stageRoom?.host?.username||state.me?.username||'S').slice(0,1).toUpperCase())}</span><span><b>${esc(stageRoom?.host?.username||state.me?.displayName||'SYLORA Creator')}</b><small>${stageRoom?'ведучий LIVE':'твоя майбутня сцена'}</small></span></div></div><div class="stage-controls">${stageRoom?`<button class="watch-live" data-id="${stageRoom.id}">${referenceIcon('play')} Дивитися</button><button class="open-live" data-id="${stageRoom.id}">${referenceIcon('mail')} Чат</button>${state.me?`<button class="ask-live" data-id="${stageRoom.id}">✦ Sylora</button>`:''}`:`<button data-live-empty-action="create">＋ Створити LIVE</button><button data-live-tab="studio">${referenceIcon('studio')} Studio</button>`}</div></section>
    <aside class="live-chat-panel"><div class="panel-tabs"><button class="active" type="button">Живий чат</button><button type="button">Гості</button><button type="button">Модерація</button></div><div class="chat-stream"><div class="chat-event">${referenceIcon('activity')}<span><b>Чат готовий до підключення</b><small>${stageRoom?'Відкрий активний LIVE, щоб бачити повідомлення в реальному часі.':'Повідомлення з’являться після запуску ефіру.'}</small></span></div><div class="sylora-chat"><span class="ai-mark">✦</span><p><b>Sylora</b><small>Слухаю лише за запитом ведучого. Можу озвучувати дозволені події природним голосом.</small></p><div class="platform-pills"><i>TikTok</i><i>YouTube</i><i>OBS</i><i>TikFinity</i></div></div>${stageRoom?`<button class="depth-button open-live live-chat-open" data-id="${stageRoom.id}">Відкрити реальний чат →</button>`:''}</div><div id="tiktokPilotMount"></div></aside>
  ...
  if(state.me)tiktokPilotCleanup=mountTikTokOwnerPilot({root:document.querySelector('#tiktokPilotMount'),api,toast,humanError});
...
function distributionDestinationMarkup(item,selected){return`<div class="distribution-destination" data-distribution-row="${item.id}"><label><input type="checkbox" data-distribution-destination value="${item.id}" ${item.enabled&&selected.has(item.id)?'checked':''} ${item.enabled?'':'disabled'}><span><b>${esc(item.label)}</b><small>${esc(item.providerLabel)} · ${esc(item.ingestHost)} · key ${esc(item.keyFingerprint)}</small></span></label><button type="button" class="ghost" data-distribution-toggle="${item.id}" data-enabled="${item.enabled?'1':'0'}">${item.enabled?'Призупинити':'Увімкнути'}</button><button type="button" class="ghost danger" data-distribution-delete="${item.id}" aria-label="Видалити ${esc(item.label)}">×</button></div>`}
...
async function createStudioDistributionDestination(event){event.preventDefault();const form=event.currentTarget,button=form.querySelector('button[type="submit"]'),input=Object.fromEntries(new FormData(form));button.disabled=true;button.textContent='Шифрування…';try{await api('/api/studio/distribution/destinations',{method:'POST',body:JSON.stringify(input)});form.reset();toast('Напрямок додано · stream key зашифровано');await reloadStudioDistributionPanel()}catch(error){toast(distributionErrorText(error));button.disabled=false;button.textContent='Зашифрувати й додати'}}
...
async function deleteStudioDistributionDestination(id){if(!confirm('Видалити цей напрямок? Stream key буде стерто без можливості відновлення.'))return;try{await api(`/api/studio/distribution/destinations/${id}`,{method:'DELETE'});await reloadStudioDistributionPanel();toast('Напрямок видалено')}catch(error){toast(distributionErrorText(error))}}
