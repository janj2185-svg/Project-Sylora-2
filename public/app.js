...
async function preflightStudioDistribution(){const liveId=document.querySelector('#studioLiveRoom')?.value,status=document.querySelector('#distributionStatus');if(!liveId)return toast('Спочатку обери або створи LIVE');status.textContent='Перевіряю router, шифрування та напрямки…';try{const out=await api(`/api/live/${liveId}/distribution/preflight`,{method:'POST',body:JSON.stringify({destinationIds:distributionSelectedIds()})});status.textContent=out.ready?`Готово · ${out.destinations.length} напрямків · media router відповідає.`:distributionErrorText({data:{details:{reasons:out.reasons}}})}catch(error){status.textContent=distributionErrorText(error)}}
function renderStudioDistributionStatus(out){const status=document.querySelector('#distributionStatus'),start=document.querySelector('#distributionStart'),stop=document.querySelector('#distributionStop'),session=out?.session;if(!status||!start||!stop)return;const active=session&&['preparing','waiting_for_source','live','degraded'].includes(session.status);start.disabled=!!active;stop.disabled=!active;if(!session){status.textContent='Multistream ще не підготовлено для цього LIVE.';stopStudioDistributionPolling();return}if(session.status==='live'){const forwarding=session.destinations.filter(x=>x.status==='forwarding').length;status.textContent=`● MULTISTREAM LIVE · ${forwarding}/${session.destinations.length} напрямків · in ${formatDistributionBytes(session.inboundBytes)} · out ${formatDistributionBytes(session.outboundBytes)}`}else if(session.status==='degraded'){const failed=session.destinations.filter(x=>x.status==='error'||x.hasError).map(x=>x.label);status.textContent=`⚠ MULTISTREAM DEGRADED · помилка: ${failed.join(', ')||'зовнішній напрямок'}`}else if(session.status==='waiting_for_source')status.textContent=`Очікую OBS source · ${session.destinations.length} напрямків налаштовано · key ${session.ingestKeyFingerprint}`;else if(session.status==='failed')status.textContent='⚠ Multistream зупинено: media router втратив session path. Запусти нову сесію з новим ingest key.';else if(session.status==='stopped')status.textContent=`Multistream зупинено · ${new Date(session.stoppedAt).toLocaleString()}`;else status.textContent=`Multistream ${session.status} · router ${session.routerReachable===false?'offline':'online'}`;if(active&&!studioDistributionPoll)studioDistributionPoll=setInterval(()=>refreshStudioDistributionStatus().catch(()=>{}),5000);if(!active)stopStudioDistributionPolling()}
...
async function startStudioDistribution(){const liveId=document.querySelector('#studioLiveRoom')?.value,status=document.querySelector('#distributionStatus'),button=document.querySelector('#distributionStart');if(!liveId)return toast('Спочатку обери або створи LIVE');const destinationIds=distributionSelectedIds();button.disabled=true;status.textContent='Створюю захищений ingest і налаштовую fan-out…';try{const out=await api(`/api/live/${liveId}/distribution/start`,{method:'POST',body:JSON.stringify({destinationIds,record:document.querySelector('#distributionRecord')?.checked!==false})}),ingest=document.querySelector('#distributionIngest');document.querySelector('#distributionIngestServer').value=out.ingest.serverUrl;document.querySelector('#distributionIngestKey').value=out.ingest.streamKey;ingest.hidden=false;renderStudioDistributionStatus({session:out.session});toast('Multistream підготовлено · встав server і key в OBS')}catch(error){button.disabled=false;status.textContent=distributionErrorText(error)}}
async function stopStudioDistribution(){const liveId=document.querySelector('#studioLiveRoom')?.value;if(!liveId)return;if(!confirm('Зупинити розподіл потоку на всіх зовнішніх платформах?'))return;const button=document.querySelector('#distributionStop');button.disabled=true;try{const out=await api(`/api/live/${liveId}/distribution/stop`,{method:'POST',body:'{}'});forgetStudioDistributionKey();renderStudioDistributionStatus(out);toast('Multistream зупинено на всіх напрямках')}catch(error){button.disabled=false;toast(distributionErrorText(error))}}
async function copyStudioDistributionValue(selector,message){const input=document.querySelector(selector);if(!input?.value)return;try{await navigator.clipboard.writeText(input.value);toast(message)}catch{input.select();toast('Значення виділено — скопіюй вручну')}}
...
function mountStudioIntelligence(ownRooms){
  ...
  try{
    const intel=document.createElement('section');intel.className='card studio-intelligence-card';intel.innerHTML=`<span class="eyebrow">CREATOR INTELLIGENCE</span><h3>LIVE insights · clips · captions</h3><div class="studio-intelligence-actions"><select id="creatorLiveSelect"><option value="" data-sylora-copy="chooseLive">Оберіть LIVE</option>${ownRooms.map(r=>`<option value="${r.id}">${esc(r.title)}</option>`).join('')}</select><button id="creatorInsightsBtn" class="primary" type="button" data-sylora-copy="analyzeLive">Аналізувати LIVE</button><button id="creatorPackBtn" class="ghost" type="button" data-sylora-copy="contentPack">Пакет контенту</button></div><pre id="creatorIntelOut" hidden style="white-space:pre-wrap;font-size:12px"></pre>`;
