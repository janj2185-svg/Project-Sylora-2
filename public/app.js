...
function account(){
  ...
  el.innerHTML=`<select id="localeSwitch" class="ghost" aria-label="${esc(t('language'))}">${localeOptions}</select>${state.me?`<button class="header-icon" data-account-view="gifts" title="${esc(t('gifts'))}">♢</button><button class="header-icon" data-account-view="messages" title="${esc(t('inbox'))}">◌</button><button class="header-avatar" data-account-view="profile" title="${esc(t('profile'))}">${esc((state.me.displayName||state.me.username||'S').slice(0,1).toUpperCase())}</button><button class="header-exit" id="logout" title="${esc(t('signout'))}">↪</button>`:`<button class="primary" id="signin">${esc(t('signin'))}</button>`}`;
...
async function renderFeed(){
  ...
  const hour=new Date().getHours(),hello=t(hour<12?'goodMorning':hour<18?'goodAfternoon':'goodEvening'),homeTitle=state.me?`${hello}, ${state.me.displayName}!`:`SYLORA · ${t('home')}`;
  ...
  const strip=(title,cards,more)=>cards.length?`<section class="eco-strip eco-${more}"><div class="eco-strip-head"><b>${esc(title)}</b><button type="button" data-eco-nav="${more}" aria-label="${esc(t('showAll'))}">→</button></div><div class="eco-carousel">${cards.join('')}</div></section>`:'';
  const continueHtml=hub?.continue?.length?`<section class="eco-strip home-continue"><div class="eco-strip-head"><b>${esc(t('continue'))}</b></div><div class="eco-carousel">${hub.continue.map(item=>`<button type="button" class="eco-card" data-eco-nav="${item.view}" data-live-id="${item.id||''}"><b>${esc(item.kind)}</b><small>${esc(item.label||'')}</small></button>`).join('')}</div></section>`:'';
  const inboxHtml=hub?`<section class="eco-strip eco-messages"><div class="eco-strip-head"><b>${esc(t('inbox'))}</b><button type="button" data-eco-nav="messages">→</button></div><div class="eco-carousel"><button type="button" class="eco-card" data-eco-nav="messages"><b>${hub.inboxPreview.unreadNotifications} ${esc(t('notifications'))}</b><small>${(hub.inboxPreview.conversations||[]).length} ${esc(t('conversations'))}</small></button>${(hub.inboxPreview.conversations||[]).slice(0,3).map(c=>`<button type="button" class="eco-card" data-eco-nav="messages"><b>${esc(t('message'))}</b><small>${esc((c.preview||'').slice(0,60))}</small></button>`).join('')}</div></section>`:'';
...
async function renderLive(){
  ...
  <details class="live-capability-details"><summary>${esc(t('liveCapabilities'))}</summary><p>${(ent.battleModes||[]).slice(0,6).map(esc).join(' · ')} · ${(ent.roomKinds||[]).slice(0,5).map(esc).join(', ')}</p></details>
  <div class="live-tabs">
...
async function renderAI(speakText=''){
  ...
  const transparency=document.querySelector('#app>.ai-transparency'),conversation=document.querySelector('#aiMessages'),compose=document.querySelector('#aiForm'),memory=document.querySelector('#app>.ai-memory');
  if(transparency&&conversation&&compose&&memory){
    const workspace=document.createElement('div'),dialogue=document.createElement('div'),context=document.createElement('aside');
    workspace.className='ai-workspace-grid';dialogue.className='ai-dialogue-column';context.className='ai-context-column';
    transparency.before(workspace);dialogue.append(conversation,compose);context.append(transparency,memory);workspace.append(dialogue,context);
  }
  document.querySelector('.sylora-ai-hero')?.classList.add('ai-presence-container');
...
api('/api/ai/intelligence').then(intel=>{const sel=document.querySelector('#aiProactive');if(!sel)return;sel.value=intel.proactive||intel.agent?.proactiveLevel||'IMPORTANT_ONLY';sel.onchange=async()=>{await api('/api/ai/proactive',{method:'PATCH',body:JSON.stringify({level:sel.value})});toast('OK')}}).catch(()=>{});
  const form=document.querySelector('#aiForm');document.querySelector('#aiRealtime')?.addEventListener('click',startSyloraRealtime);document.querySelector('#aiMute')?.addEventListener('click',toggleSyloraRealtimeMute);document.querySelector('#aiEndCall')?.addEventListener('click',stopSyloraRealtime);document.querySelector('#aiMic')?.addEventListener('click',startSyloraListening);document.querySelector('#aiVoiceToggle')?.addEventListener('click',()=>{syloraVoiceEnabled=!syloraVoiceEnabled;localStorage.setItem('sylora_voice',syloraVoiceEnabled?'1':'0');if(!syloraVoiceEnabled)stopSyloraVoice();renderAI()});if(syloraRealtimePeer){clearInterval(syloraCallTimer);syloraCallTimer=setInterval(realtimeClock,1000);realtimeClock()}
