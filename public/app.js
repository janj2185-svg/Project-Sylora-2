...
async function renderBusinessReference(){
  ...
  document.querySelector('#bizInvoice')?.addEventListener('click',async()=>{await api('/api/business/invoices',{method:'POST',body:JSON.stringify({items:[{description:'Service',quantity:1,unitNetPrice:100,taxRate:23}],seller:{name:'Me'},buyer:{name:'Client'}})});toast('Draft invoice створено');await renderBusinessReference()});
  document.querySelector('#bizCrm')?.addEventListener('click',async()=>{const name=prompt('Client name');if(!name)return;await api('/api/business/crm',{method:'POST',body:JSON.stringify({type:'client',name})});toast('CRM запис створено')});
  document.querySelector('#bizQuote')?.addEventListener('click',async()=>{await api('/api/business/quotes',{method:'POST',body:JSON.stringify({items:[{description:'Estimate',quantity:1,unitNetPrice:50,taxRate:0}]})});toast('Quote draft створено')});
  document.querySelector('#bizTime')?.addEventListener('click',async()=>{await api('/api/business/time',{method:'POST',body:JSON.stringify({action:'start',note:'Work'})});toast('Робочий таймер запущено')});
  document.querySelector('#bizFinanceAsk')?.addEventListener('click',async()=>{if(!state.me)return renderAuth();const out=await api('/api/business/finance/ask',{method:'POST',body:JSON.stringify({query:'неоплачені фактури'})});toast(`Неоплачених: ${out.answer?.unpaidCount??0} · підтвердження обов’язкове`)});
  ...
  document.querySelector('#countryForm')?.addEventListener('submit',async event=>{event.preventDefault();await api('/api/business/country',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(event.currentTarget)))});toast('Country profile збережено');await renderBusinessReference()});
  document.querySelector('#orgForm')?.addEventListener('submit',async event=>{event.preventDefault();const name=new FormData(event.currentTarget).get('name');await api('/api/orgs',{method:'POST',body:JSON.stringify({name})});toast('Організацію створено');await renderBusinessReference()});
  ...
  document.querySelectorAll('.add-org-team').forEach(button=>button.onclick=async()=>{const name=prompt(t('teams'));if(!name)return;await api(`/api/orgs/${button.dataset.id}/teams`,{method:'POST',body:JSON.stringify({name})});toast('Команду додано');showWorkspace(button.dataset.id)});
  document.querySelectorAll('.add-org-doc').forEach(button=>button.onclick=async()=>{const title=prompt(t('documents'));if(!title)return;await api(`/api/orgs/${button.dataset.id}/documents`,{method:'POST',body:JSON.stringify({title})});toast('Документ додано');showWorkspace(button.dataset.id)});
  document.querySelectorAll('.add-org-task').forEach(button=>button.onclick=async()=>{const title=prompt(t('tasks'));if(!title)return;await api(`/api/orgs/${button.dataset.id}/tasks`,{method:'POST',body:JSON.stringify({title})});toast('Задачу додано');showWorkspace(button.dataset.id)});
