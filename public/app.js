import { t, setLocale, getLocale, humanError } from './i18n.js?v=20260830-studio2';
...
const state={token:localStorage.getItem('sylora_token')||'',me:null,wallet:null,view:'feed',intent:null,inboxTab:'messages',liveTab:'discover',incomingCall:null};const app=document.querySelector('#app');let giftEngine={play:async()=>{}};const recentRealtimeIds=new Map();let userEventsAbort=null,liveEventSource=null,liveViewerSource=null,liveViewerPublicSource=null,liveViewerPeer=null,liveViewerEpoch=0,liveViewerPendingIce=[],liveViewerId=null,liveViewerLiveId=null,liveViewerHostPeerId=null,liveViewerAnnounceTimer=null,liveRtcConfigCache=null,liveRtcConfigCachedAt=0,studioRenderEpoch=0,studioSourceStream=null,studioSourceKind=null,studioSourceRequestPending=false,studioSourceRequestId=0,studioScenes=[],studioLoadedSceneId='',studioRecorder=null,studioRecordingStream=null,studioRecordingFinalizing=false,studioRaf=0,studioLastBlob=null,studioLiveSource=null,studioBroadcastStream=null,studioBroadcastEpoch=0,studioBroadcastPending=false,studioBroadcastStopping=false,studioBroadcastHostAcquired=false,studioBroadcastConnectTask=Promise.resolve(),studioBroadcastCleanup=Promise.resolve(),studioBroadcastLiveId=null,studioHostPeerId=null,studioOverlayImage=null,syloraRecognition=null,syloraVoiceEnabled=localStorage.getItem('sylora_voice')==='1',syloraRealtimePeer=null,syloraRealtimeStream=null,syloraRealtimeChannel=null,syloraRealtimeAudio=null,syloraAudioContext=null,syloraAudioRaf=0,syloraCallTimer=null,syloraCallStartedAt=0,conferenceSessionCleanup=null,activeCallCleanup=null,tiktokPilotCleanup=null;const studioPeers=new Map(),studioPendingSourceStreams=new Set();
let studioObsClient=null,studioCompanionClient=null,studioObsCredentials=null,studioObsReconnectTimer=null,studioObsReconnectAttempt=0,studioAudioContext=null,studioAudioGain=null,studioAudioAnalyser=null,studioAudioDestination=null,studioAudioSources=[],studioAudioMeterRaf=0,studioDistributionPoll=0;
const STUDIO_PROFILES={
  vertical480:{label:'9:16 · SD · 480×854 · 30 FPS',width:480,height:854,fps:30},
  vertical720:{label:'9:16 · HD · 720×1280 · 30 FPS',width:720,height:1280,fps:30},
  vertical1080:{label:'9:16 · FHD · 1080×1920 · 30 FPS',width:1080,height:1920,fps:30},
  vertical1080p60:{label:'9:16 · FHD · 1080×1920 · 60 FPS',width:1080,height:1920,fps:60},
  horizontal480:{label:'16:9 · SD · 854×480 · 30 FPS',width:854,height:480,fps:30},
  horizontal720:{label:'16:9 · HD · 1280×720 · 30 FPS',width:1280,height:720,fps:30},
  horizontal1080:{label:'16:9 · FHD · 1920×1080 · 30 FPS',width:1920,height:1080,fps:30},
  horizontal1080p60:{label:'16:9 · FHD · 1920×1080 · 60 FPS',width:1920,height:1080,fps:60},
  square1080:{label:'1:1 · FHD · 1080×1080 · 30 FPS',width:1080,height:1080,fps:30},
  portrait4x5:{label:'4:5 · FHD · 1080×1350 · 30 FPS',width:1080,height:1350,fps:30},
  horizontal1440:{label:'16:9 · 2K · 2560×1440 · 30 FPS · conditional',width:2560,height:1440,fps:30,conditional:true},
  horizontal2160:{label:'16:9 · 4K · 3840×2160 · 30 FPS · conditional',width:3840,height:2160,fps:30,conditional:true}
};
...
function localizedDynamicUiMessage(msg){const text=String(msg??''),gift=text.match(/^Подарунок відправлено · Orbit (\d+)$/),walletGift=text.match(/^Надіслано ×(\d+) · баланс ◈ (.+)$/);if(gift)return`${u('giftSent')} · Orbit ${gift[1]}`;if(walletGift)return`${u('sent')} ×${walletGift[1]} · ${u('sentBalance')} ◈ ${walletGift[2]}`;if(text==='Оберіть або створіть LIVE')return u('chooseOrCreateLive');if(text==='Спочатку увімкни камеру або screen share')return u('enableCameraOrScreen');return text}
...
function nav(view){if(conferenceSessionCleanup){conferenceSessionCleanup();conferenceSessionCleanup=null}if(activeCallCleanup){activeCallCleanup();activeCallCleanup=null}if(state.view==='studio'&&view!=='studio'){studioRenderEpoch++;stopStudioTracks();disconnectStudioObs();stopStudioDistributionPolling()}if(state.view==='live'&&view!=='live'){cleanupLiveViewer();tiktokPilotCleanup?.();tiktokPilotCleanup=null}if(state.view==='ai'&&view!=='ai'){stopSyloraRealtime();stopSyloraVoice()}if(liveEventSource){liveEventSource.close();liveEventSource=null}state.view=view;syncPathForView(view);syncReferenceShell(view);render()}
...
async function renderFeed(){
  ...
  const greetingName=getLocale()==='uk'&&/^ivan$/i.test(rawName)?'Іване':rawName;
  ...
  app.innerHTML=`<div class="screen-grid home-screen">
    <section class="content-stack">
      <div class="hero-copy">
        ...
        <p>${esc(t('homeHorizonIntro'))}</p>
        <div class="hero-actions"><button class="depth-button" type="button" data-horizon-create><span>${esc(t('create'))}</span>${referenceIcon('sparkles')}</button><button class="depth-button depth-button--secondary" type="button" data-horizon-view="live"><span>${esc(t('openLive'))}</span>${referenceIcon('live')}</button></div>
      </div>

      <div class="quick-worlds">
        <button class="world-button" style="--world-accent:#e83b79" type="button" data-horizon-view="live"><span>${referenceIcon('live')}</span><b>LIVE</b><small>${rooms.length} ${esc(t('liveStreams'))}</small>${referenceIcon('arrow')}</button>
        <button class="world-button" style="--world-accent:#16899b" type="button" data-horizon-view="studio"><span>${referenceIcon('studio')}</span><b>Studio</b><small>${esc(t(state.me?'openLabel':'signinRequired'))}</small>${referenceIcon('arrow')}</button>
        <button class="world-button" style="--world-accent:#28927d" type="button" data-horizon-view="learning"><span>${referenceIcon('learning')}</span><b>${esc(t('learning'))}</b><small>${esc(t(continueItem?.kind==='learning'?'continue':'openLabel'))}</small>${referenceIcon('arrow')}</button>
        <button class="world-button" style="--world-accent:#ad7a28" type="button" data-horizon-view="business"><span>${referenceIcon('business')}</span><b>${esc(t('business'))}</b><small>${esc(t('openLabel'))}</small>${referenceIcon('arrow')}</button>
      </div>

      <div class="section-title"><div><span>${esc(t('nowInSylora'))}</span><h2>${esc(t('livingMoment'))}</h2></div><button type="button" data-horizon-view="live">${esc(t('viewAll'))}${referenceIcon('arrow')}</button></div>
      <div class="home-feature-grid">
        <button type="button" class="glass-card featured-live reference-live-button" data-horizon-view="live"${featured?.id?` data-live-id="${esc(featured.id)}"`:''}>
          ...
          <div class="live-topline"><span class="status-pill status-pill--live">${referenceIcon('live')} LIVE</span><span>${totalViewers.toLocaleString()} ${esc(t('viewersWatching'))}</span></div>
        ...
        <div class="moment-list">
          <article class="glass-card moment-card" data-eco-nav="clips"><span class="moment-icon violet">${referenceIcon('clip')}</span><div><small>CLIPS</small><b>${esc(t('realVideos'))}</b><span>${posts.length?`${posts.length} ${esc(t('feedPublications'))}`:esc(t('noFakeViews'))}</span></div>${referenceIcon('play')}</article>
          <article class="glass-card moment-card" data-eco-nav="learning"><span class="moment-icon mint">${referenceIcon('learning')}</span><div><small>${esc(t(continueItem?'continue':'learning')).toUpperCase()}</small><b>${esc(continueItem?.label||t('openLearningSpace'))}</b><span>${esc(t(continueItem?'savedProgress':'chooseCourse'))}</span></div>${referenceIcon('arrow')}</article>
          <article class="glass-card moment-card" data-eco-nav="business"><span class="moment-icon gold">${referenceIcon('business')}</span><div><small>${esc(t('business')).toUpperCase()}</small><b>${esc(t('honestAnalytics'))}</b><span>${esc(t('realMetricsAfterActivation'))}</span></div>${referenceIcon('arrow')}</article>
        </div>
      </div>

      <section class="dynamic-home-feed">
        <div class="section-title"><div><span>${esc(t('yourSpace'))}</span><h2>${esc(t('forYou'))}</h2></div></div>
    ...
    <aside class="context-stack">
      <article class="glass-card pulse-card"><div class="card-top"><span>${referenceIcon('activity')} LIVE PULSE</span><span class="status-pill status-pill--success">${esc(t('connectedLabel'))}</span></div><div class="pulse-number"><b>${totalViewers.toLocaleString()}</b><small>${esc(t('viewersLiveNow'))}</small></div><div class="split-stats"><span><b>${rooms.length}</b><small>${esc(t('activeBroadcasts'))}</small></span><span><b>${state.me?'ON':'—'}</b><small>${esc(t('personalSpace')).toLocaleLowerCase()}</small></span></div></article>
      <article class="glass-card"><div class="section-title"><div><h2>${esc(t('importantOnePlace'))}</h2></div></div><div class="focus-list">
        <button type="button" data-horizon-view="messages"><span class="focus-icon blue">${referenceIcon('mail')}</span><span><b>${unread} ${esc(t('inboxMessages')).toLocaleLowerCase()}</b><small>${conversationCount} ${esc(t('conversations'))}</small></span>${referenceIcon('arrow')}</button>
        ...
        <button type="button" data-horizon-view="gifts"><span class="focus-icon gold">${referenceIcon('wallet')}</span><span><b>${state.me?`${Number(state.wallet?.balance||0).toLocaleString()} LUMEN`:esc(t('wallet'))}</b><small>${esc(t(state.me?'currentBalance':'signinToView'))}</small></span>${referenceIcon('arrow')}</button>
      </div></article>
    </aside>
  </div>`;
...
async function logout(){if(userEventsAbort){userEventsAbort.abort();userEventsAbort=null}if(state.view==='studio'){stopStudioTracks();await studioBroadcastCleanup.catch(()=>{})}if(state.view==='live')cleanupLiveViewer();await api('/api/auth/logout',{method:'POST'}).catch(()=>{});state.token='';state.me=null;state.wallet=null;clearRtcConfigCache();localStorage.removeItem('sylora_token');account();refreshRightRail();refreshRailProgress();nav('feed')}
...
function renderClipUploader(){const box=document.querySelector('#uploader');box.innerHTML=`<form id="clipUpload" class="card auth fields"><h3>${esc(u('newClip'))}</h3><input name="title" maxlength="120" aria-label="${esc(u('title'))}" placeholder="${esc(u('title'))}" required>${mediaFilePickerMarkup()}<p class="muted">${esc(u('clipFileLimit'))}</p><button class="primary">${esc(u('uploadPublish'))}</button><p id="uploadStatus" class="muted"></p></form>`;const form=document.querySelector('#clipUpload');bindMediaFilePicker(form);form.onsubmit=async e=>{e.preventDefault();const data=new FormData(form),file=data.get('file'),status=document.querySelector('#uploadStatus');if(!(file instanceof File)||!file.size)return;status.textContent=u('uploading');try{const response=await fetch('/api/media/upload',{method:'POST',headers:{authorization:`Bearer ${state.token}`,'content-type':file.type},body:file});const uploaded=await response.json();if(!response.ok)throw new Error(uploaded.error||'UPLOAD_FAILED');status.textContent=u('publishing');await api('/api/videos',{method:'POST',body:JSON.stringify({mediaId:uploaded.media.id,title:data.get('title'),format:'clip'})});toast(u('clipPublished'));renderClips()}catch(err){status.textContent=`${u('errorLabel')}: ${humanError(err.message)}`}}}

async function renderVideos(){const {videos}=await api('/api/videos?format=video');app.innerHTML=`<div class="card hero"><span class="eyebrow">SYLORA VIDEO</span><h1>${esc(u('videoStoriesTitle'))}</h1><p>${esc(u('videoStoriesCopy'))}</p>${state.me?`<button id="videoUploadOpen" class="primary">＋ ${esc(u('uploadVideo'))}</button>`:`<button id="videoLogin" class="primary">${esc(t('signin'))}</button>`}</div><div id="videoUploader"></div><div class="video-grid">${videos.map(v=>`<article class="card video-card"><video controls playsinline preload="metadata" src="${v.media.url}" aria-label="${esc(v.title)}"></video><div class="video-card-body"><span class="pipeline">${v.stream?.status==='ready'?'HLS READY':(v.stream?.status||'ORIGINAL').toUpperCase()}</span><h3>${esc(v.title)}</h3><p class="muted">@${esc(v.author.username)} · ${esc(v.description)}</p></div></article>`).join('')||`<div class="card empty">${esc(u('noLongVideos'))}</div>`}</div>`;document.querySelector('#videoLogin')?.addEventListener('click',renderAuth);document.querySelector('#videoUploadOpen')?.addEventListener('click',renderVideoUploader);if(videos.some(v=>v.stream&&['queued','processing'].includes(v.stream.status)))setTimeout(()=>{if(state.view==='videos')renderVideos()},1500)}
function renderVideoUploader(){const box=document.querySelector('#videoUploader');box.innerHTML=`<form id="videoUpload" class="card auth fields"><h3>${esc(u('newVideo'))}</h3><input name="title" maxlength="120" aria-label="${esc(u('title'))}" placeholder="${esc(u('title'))}" required><textarea name="description" maxlength="2000" aria-label="${esc(u('description'))}" placeholder="${esc(u('description'))}"></textarea>${mediaFilePickerMarkup()}<button class="primary">${esc(u('uploadPublish'))}</button><p id="videoUploadStatus" class="muted"></p></form>`;const form=document.querySelector('#videoUpload');bindMediaFilePicker(form);form.onsubmit=async e=>{e.preventDefault();const data=new FormData(form),file=data.get('file'),status=document.querySelector('#videoUploadStatus');if(!(file instanceof File)||!file.size)return;status.textContent=u('uploading');try{const response=await fetch('/api/media/upload',{method:'POST',headers:{authorization:`Bearer ${state.token}`,'content-type':file.type},body:file}),uploaded=await response.json();if(!response.ok)throw new Error(uploaded.error||'UPLOAD_FAILED');status.textContent=u('publishingHls');await api('/api/videos',{method:'POST',body:JSON.stringify({mediaId:uploaded.media.id,title:data.get('title'),description:data.get('description'),format:'video'})});toast(u('videoPublished'));renderVideos()}catch(err){status.textContent=`${u('errorLabel')}: ${humanError(err.message)}`}}}
...
async function renderExploreReference(){
  app.innerHTML=`<section class="route-hero-row compact explore-heading"><div><span class="status-pill status-pill--violet">${referenceIcon('search')} UNIVERSAL DISCOVERY</span><h1>${esc(u('exploreNextTitle'))}</h1><p>${esc(u('exploreNextCopy'))}</p></div></section>
  <form id="search" class="explore-search"><span>${referenceIcon('search')}</span><input name="q" minlength="2" aria-label="${esc(u('exploreSearchPlaceholder'))}" placeholder="${esc(u('exploreSearchPlaceholder'))}" autocomplete="off" autofocus><button type="submit">${referenceIcon('sparkles')} ${esc(u('find'))}</button></form>
  <section id="discoveryDefault" class="discovery-grid">
    <article class="glass-card discovery-hero"><div class="living-stage-art" aria-hidden="true"><i></i><i></i><i></i></div><span class="live-stage-shade"></span><div><span class="status-pill status-pill--gold">LIVING HORIZON</span><h2>${esc(u('responsiveWorld'))}</h2><p>${esc(u('realDiscoveryCopy'))}</p></div></article>
    ${[
      ['users',u('people'),u('explorePeopleCopy'),'creator'],
      ['live',u('liveNow'),u('exploreLiveCopy'),'live'],
      ['learning',t('learning'),u('exploreLearningCopy'),'course'],
      ['business',t('business'),u('exploreBusinessCopy'),'company']
    ].map(([icon,title,description,query],index)=>`<article class="glass-card discovery-card"><span class="discovery-icon nav-icon-plate">${referenceIcon(icon)}</span><span class="status-pill ${index===1?'status-pill--rose':index===2?'status-pill--success':'status-pill--violet'}">${index===1?'REALTIME':'DISCOVER'}</span><h3>${esc(title)}</h3><p>${esc(description)}</p><button type="button" data-discovery-query="${query}">${esc(u('exploreAction'))} ${referenceIcon('arrow')}</button></article>`).join('')}
  </section>
  ...
  const runSearch=async query=>{
    ...
    results.innerHTML=`<div class="glass-card discovery-loading"><span class="runtime-core">✦</span><b>${esc(u('syloraSearching'))}</b></div>`;
    ...
    const sections=[
      [u('people'),found.users,x=>`@${esc(x.username)} · ${esc(x.displayName)}`,'users'],
      [u('posts'),found.posts,item=>`${esc(item.author?.username||'')}: ${esc(item.text)}`,'clip'],
      [t('communities'),found.communities,item=>esc(item.name),'users'],
      [u('courses'),found.courses,item=>esc(item.title),'learning'],
    ...
    let html=sections.map(([name,items,format,icon],index)=>`<article class="glass-card discovery-card search-result-card"><span class="discovery-icon nav-icon-plate">${referenceIcon(icon)}</span><span class="status-pill ${index===5?'status-pill--rose':'status-pill--violet'}">${(items||[]).length} ${esc(u('results'))}</span><h3>${esc(name)}</h3><div class="result-lines">${(items||[]).slice(0,8).map(item=>`<p>${format(item)}</p>`).join('')||`<p class="muted">${esc(u('nothingFound'))}</p>`}</div></article>`).join('');
    if(universal?.semantic?.length)html+=`<article class="glass-card discovery-card search-result-card semantic-result"><span class="discovery-icon nav-icon-plate">${referenceIcon('sparkles')}</span><span class="status-pill status-pill--success">SEMANTIC</span><h3>${esc(u('smartSearch'))}</h3><div class="result-lines">${universal.semantic.slice(0,12).map(item=>`<p><b>${esc(item.type)}</b> · ${esc(item.label||'')}</p>`).join('')}</div><small>${esc(universal.semanticHonesty?.note||'')}</small></article>`;
...
async function renderLive(){
  ...
  const roomCards=visibleRooms.map(r=>{const opponent=rooms.find(x=>x.id!==r.id&&x.host?.id!==r.host?.id);return`<article class="live-room-card"><div><span class="status-pill status-pill--live">● LIVE · ${r.viewerCount||0}</span><h3>${esc(r.title)}</h3><p>@${esc(r.host.username)}</p></div><div class="live-tools"><button class="depth-button watch-live" data-id="${esc(r.id)}">${esc(u('watch'))}</button><button class="ghost open-live" data-id="${esc(r.id)}">${esc(t('liveChat'))}</button>${state.me?`<button class="ghost ask-live" data-id="${esc(r.id)}">Sylora</button>`:''}${state.me?.id===r.host?.id&&opponent?`<button class="ghost resonance-start" data-id="${esc(r.id)}" data-opponent="${esc(opponent.id)}">✦ Battle</button>`:''}${state.me?.id===r.host?.id?`<button class="ghost live-copilot" data-id="${esc(r.id)}">Copilot</button>`:''}</div></article>`}).join('');
  app.innerHTML=`<section class="route-hero-row compact live-route-head"><div><span class="status-pill status-pill--live">${referenceIcon('live')} SYLORA LIVE</span><h1>${esc(u('liveSpaceTitle'))}</h1><p>${esc(u('liveSpaceCopy'))}</p></div><div class="live-tabs filter-row"><button type="button" data-live-tab="discover" class="${tab==='discover'?'active':''}">${esc(t('discoverLive'))}</button><button type="button" data-live-tab="following" class="${tab==='following'?'active':''}">${esc(t('following'))}</button><button type="button" data-live-tab="create" class="${tab==='create'?'active':''}">${esc(t('createLive'))}</button><button type="button" data-live-tab="battles" class="${tab==='battles'?'active':''}">${esc(t('battles'))}</button><button type="button" data-live-tab="studio">Studio</button></div></section>
  <div class="live-layout">
    <section class="live-lifecycle live-readiness"><div class="lifecycle-heading"><div><small>${esc(u('liveStepTitle')||t('tiktokStepLaunch'))}</small><b>${esc(state.me?u('liveNoAutoStart'):u('signInToConfigure'))}</b></div><span class="status-pill ${hasTikTokDestination&&routerReady?'status-pill--success':''}">${esc(hasTikTokDestination&&routerReady?u('readyToCheck'):u('liveNotFinished'))}</span></div><div class="live-setup-grid">${setupSteps.map(([number,label,status,ready,target])=>`<button type="button" class="live-setup-step ${ready?'is-ready':'is-pending'}" data-live-setup="${target}"><span>${number}</span><span><b>${esc(label)}</b><small>${esc(status)}</small></span><i>${ready?'✓':'→'}</i></button>`).join('')}</div></section>
    <section class="live-stage-card"><div class="living-stage-art" aria-hidden="true"><i></i><i></i><i></i></div><div class="live-stage-shade"></div><div class="live-stage-top"><div><span class="status-pill status-pill--live">${referenceIcon('live')} LIVE</span><span class="status-pill">${Number(stageRoom?.viewerCount||0).toLocaleString()} ${esc(u('watching'))}</span></div><button type="button" data-live-tab="studio" aria-label="${esc(u('studioOpen'))}">${referenceIcon('studio')}</button></div><div class="live-stage-copy"><small>SYLORA ORIGINAL · LIVING HORIZON</small><h1>${esc(stageRoom?.title||t('startLive'))}</h1><p>${esc(stageRoom?u('liveReactiveCopy'):u('createFirstLiveCopy'))}</p><div class="host-line"><span class="avatar">${esc((stageRoom?.host?.username||state.me?.username||'S').slice(0,1).toUpperCase())}</span><span><b>${esc(stageRoom?.host?.username||state.me?.displayName||u('syloraCreator'))}</b><small>${esc(stageRoom?u('liveHost'):u('futureStage'))}</small></span></div></div><div class="stage-controls">${stageRoom?`<button class="watch-live" data-id="${esc(stageRoom.id)}">${referenceIcon('play')} ${esc(u('watch'))}</button><button class="open-live" data-id="${esc(stageRoom.id)}">${referenceIcon('mail')} ${esc(t('liveChat'))}</button>${state.me?`<button class="ask-live" data-id="${esc(stageRoom.id)}">✦ Sylora</button>`:''}`:`<button data-live-empty-action="create">＋ ${esc(t('createLive'))}</button><button data-live-tab="studio">${referenceIcon('studio')} Studio</button>`}</div></section>
    <aside class="live-chat-panel"><div class="panel-tabs"><span>${esc(t('liveChat'))}</span></div><div class="chat-stream"><div class="chat-event">${referenceIcon('activity')}<span><b>${esc(stageRoom?u('chatAvailable'):u('chatNotConnected'))}</b><small>${esc(stageRoom?u('openLiveForRealtimeMessages'):u('messagesAfterLiveStarts'))}</small></span></div><div class="sylora-chat"><span class="ai-mark">✦</span><p><b>Sylora</b><small>${esc(u('syloraRuleBound'))}</small></p></div>${stageRoom?`<button class="depth-button open-live live-chat-open" data-id="${esc(stageRoom.id)}">${esc(u('openChat'))} →</button>`:''}</div>${state.me&&tab==='create'?'<div id="tiktokPilotMount"></div>':''}</aside>
    <div class="live-lower"><section class="glass-card live-director"><div class="card-top"><span>${esc(u('liveDirector'))}</span><b>${(ent.battleModes||[]).length} ${esc(u('modes'))}</b></div>${state.me&&tab==='create'?`<div class="card fields live-creator-launchpad"><input id="liveTitle" maxlength="120" placeholder="${esc(u('liveTitlePlaceholder'))}"><button id="goLive" class="depth-button">＋ ${esc(t('createLive'))}</button><button class="ghost" id="openStudioFromLive">Creator Studio</button><input id="eventTitle" maxlength="160" placeholder="${esc(u('eventTitle'))}"><input id="eventWhen" maxlength="80" placeholder="${esc(u('startsWhen'))}"><button id="createEventBtn" class="ghost">＋ ${esc(t('createEvent'))}</button></div>`:(state.me?`<div class="director-grid"><button data-live-tab="create"><span>＋</span><span><b>${esc(u('newLive'))}</b><small>${esc(u('cameraOrStudio'))}</small></span></button><button data-live-tab="battles"><span>✦</span><span><b>${esc(t('battles'))}</b><small>${esc(u('cohostMode'))}</small></span></button><button data-live-tab="studio"><span>▣</span><span><b>Studio</b><small>OBS · multistream</small></span></button></div>`:`<button id="liveLogin" class="depth-button">${esc(t('signin'))}</button>`)}</section><section class="glass-card live-room-browser"><div class="card-top"><span>${tab==='following'?esc(t('following')):tab==='battles'?esc(t('battles')):esc(u('liveNowLabel'))}</span><b>${visibleRooms.length}</b></div><div class="live-room-grid">${roomCards||emptyCopy}</div></section></div>
  ...
  document.querySelectorAll('[data-live-setup]').forEach(button=>button.onclick=()=>{const target=button.dataset.liveSetup;if(target==='login')return state.me?toast(u('signedIn')):renderAuth();if(target==='studio')return nav('studio');state.liveTab='create';renderLive()});
  ...
  document.querySelector('#createEventBtn')?.addEventListener('click',async()=>{const title=document.querySelector('#eventTitle')?.value||'SYLORA Event';const startsAt=document.querySelector('#eventWhen')?.value||'tba';await api('/api/platform-events',{method:'POST',body:JSON.stringify({title,startsAt,mode:'online'})});toast(u('eventCreated'));state.liveTab='discover';renderLive()});
  document.querySelectorAll('.ask-live').forEach(b=>b.onclick=async()=>{const q=prompt(u('askSylora'),u('whatDidIMiss'))||u('whatDidIMiss');try{const out=await api('/api/ai/ask',{method:'POST',body:JSON.stringify({contentType:'live',contentId:b.dataset.id,question:q,view:'live'})});toast(out.answer||u('requestCompleted'))}catch(e){toast(humanError(e.message))}});
  document.querySelectorAll('.live-copilot').forEach(b=>b.onclick=async()=>{try{const out=await api(`/api/live/${b.dataset.id}/copilot`);toast((out.highlights||[]).slice(0,2).map(h=>h.text).join(' · ')||out.policy?.note||u('copilotReady'))}catch(e){toast(humanError(e.message))}});
...
function liveConnectionLabel(connectionState){return({connecting:u('connecting'),connected:u('connectedStatus'),failed:u('failedStatus'),disconnected:u('disconnectedStatus'),closed:u('closedStatus')})[connectionState]||String(connectionState||'').toUpperCase()}
...
function configureLiveViewerPeer(peer,id){
  peer.ontrack=e=>{if(liveViewerPeer!==peer)return;const video=document.querySelector('#liveVideo');if(video){video.srcObject=e.streams[0];giftEngine.bindLiveVideo?.(video).catch(()=>{})}const s=document.querySelector('#webrtcStatus');if(s)s.textContent=t('liveVideoStatus')};
  peer.onconnectionstatechange=()=>{if(liveViewerPeer!==peer)return;const s=document.querySelector('#webrtcStatus'),stateNow=peer.connectionState||'connecting',key=stateNow==='connected'?'connected':stateNow==='connecting'||stateNow==='new'?'connecting':'connectionLost';if(s)s.textContent=t(key)};
  peer.onicecandidate=e=>{if(liveViewerPeer===peer&&e.candidate&&liveViewerHostPeerId)sendLiveSignal(id,'ice',liveViewerId,liveViewerHostPeerId,e.candidate.toJSON()).catch(()=>{})};
  return peer
}
function replaceLiveViewerPeer(id,rtc){const previous=liveViewerPeer;liveViewerPeer=configureLiveViewerPeer(new RTCPeerConnection({iceServers:rtc.iceServers||[]}),id);previous?.close();return liveViewerPeer}
...
async function watchLive(id){
  if(!state.me)return renderAuth();
  cleanupLiveViewer();const viewerEpoch=liveViewerEpoch,viewerCurrent=()=>viewerEpoch===liveViewerEpoch&&liveViewerLiveId===id;if(liveEventSource){liveEventSource.close();liveEventSource=null}
  ...
  if(viewerEpoch!==liveViewerEpoch)return;
  if(!room)return toast(t('liveEnded'));
  liveViewerLiveId=id;liveViewerId=`viewer:${crypto.randomUUID()}`;liveViewerPeer=configureLiveViewerPeer(new RTCPeerConnection({iceServers:rtc.iceServers}),id);
  ...
  try{const source=await openLiveSignalSource(id,liveViewerId,'viewer',pending=>{if(!viewerCurrent())return pending.close();liveViewerSource=pending;const terminate=revoked=>{if(!viewerCurrent())return;const status=document.querySelector('#webrtcStatus');if(status)status.textContent=revoked?t('signin'):t('liveEnded');cleanupLiveViewer(false)};pending.addEventListener('room-closed',()=>terminate(false));pending.addEventListener('session-revoked',()=>terminate(true))});if(!viewerCurrent()){source.close();return}liveViewerSource=source}catch(error){const ended=error.message==='LIVE_SIGNAL_ROOM_CLOSED',revoked=error.message==='LIVE_SIGNAL_SESSION_REVOKED',status=document.querySelector('#webrtcStatus');if(status)status.textContent=ended?t('liveEnded'):revoked?t('signin'):`${t('liveSignalingError')}: ${humanError(error.message)}`;if(viewerCurrent())cleanupLiveViewer(!(ended||revoked));return}
  liveViewerSource.addEventListener('signal',async e=>{if(!viewerCurrent())return;const s=JSON.parse(e.data);if(s.toPeerId!==liveViewerId)return;if(s.kind==='viewer-rejected'){if(liveViewerAnnounceTimer){clearInterval(liveViewerAnnounceTimer);liveViewerAnnounceTimer=null}const status=document.querySelector('#webrtcStatus');if(status)status.textContent=t(s.data?.reason==='P2P_PEER_LIMIT'?'p2pLimit':'hostUnavailable');return}if(s.kind==='offer'){const sameHost=liveViewerHostPeerId===s.fromPeerId,replacePeer=!sameHost||['failed','closed'].includes(liveViewerPeer.connectionState),exactDuplicate=!replacePeer&&liveViewerPeer.remoteDescription?.sdp===s.data?.sdp&&liveViewerPeer.localDescription;if(exactDuplicate){await sendLiveSignal(id,'answer',liveViewerId,s.fromPeerId,liveViewerPeer.localDescription);return}if(replacePeer){const nextRtc=await liveRtcConfig();if(!viewerCurrent())return;replaceLiveViewerPeer(id,nextRtc)}liveViewerHostPeerId=s.fromPeerId;const queuedIce=liveViewerPendingIce.filter(item=>item.fromPeerId===s.fromPeerId);liveViewerPendingIce=[];const peer=liveViewerPeer;await refreshRtcPeerConfiguration(peer);if(!viewerCurrent()||peer!==liveViewerPeer)return;await peer.setRemoteDescription(s.data);if(!viewerCurrent()||peer!==liveViewerPeer)return;for(const item of queuedIce)try{await peer.addIceCandidate(item.data)}catch{}const answer=await peer.createAnswer();if(!viewerCurrent()||peer!==liveViewerPeer)return;await peer.setLocalDescription(answer);if(viewerCurrent()&&peer===liveViewerPeer)await sendLiveSignal(id,'answer',liveViewerId,liveViewerHostPeerId,peer.localDescription)}else if(s.kind==='ice'){if(s.fromPeerId===liveViewerHostPeerId&&liveViewerPeer.remoteDescription)try{await liveViewerPeer.addIceCandidate(s.data)}catch{}else{liveViewerPendingIce.push({fromPeerId:s.fromPeerId,data:s.data});if(liveViewerPendingIce.length>64)liveViewerPendingIce.shift()}}});
  liveViewerPublicSource=new EventSource(`/api/live/${id}/events`);
  liveViewerPublicSource.addEventListener('like',e=>{const x=JSON.parse(e.data),el=document.querySelector('#liveLikeCount');if(el)el.textContent=x.likes});
  liveViewerPublicSource.addEventListener('gift',e=>showGift(JSON.parse(e.data)));
  liveViewerPublicSource.addEventListener('resonance',()=>watchLive(id).catch(()=>{}));
  const announce=()=>{if(liveViewerPeer&&!['connected','closed'].includes(liveViewerPeer.connectionState))sendLiveSignal(id,'viewer-ready',liveViewerId,null,{}).catch(()=>{})};announce();liveViewerAnnounceTimer=setInterval(announce,2000)
}
async function sendLiveSignal(liveId,kind,fromPeerId,toPeerId,data,{signal,streamToken}={}){return api(`/api/live/${liveId}/signal`,{method:'POST',body:JSON.stringify({kind,fromPeerId,toPeerId,data,...(streamToken?{streamToken}:{})}),signal})}
function validLiveHostStreamToken(value){return/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(value||''))}
async function openLiveSignalSource(liveId,peerId,role,onSource=null){
  let closed=false,controller=null,hostFenceConflictTimer=null,dispatchChain=Promise.resolve();const listeners=new Map(),source={role,streamToken:null,hostFenceLost:false,addEventListener(type,handler){if(!listeners.has(type))listeners.set(type,new Set());listeners.get(type).add(handler)},close(){closed=true;controller?.abort();clearTimeout(hostFenceConflictTimer);hostFenceConflictTimer=null;listeners.clear()},loseHostFence(){dispatchSourceEvent('host-fence-lost',JSON.stringify({status:'fence-lost'}))},reportHostFenceConflict(streamToken){if(role!=='host'||closed||source.streamToken!==streamToken||hostFenceConflictTimer)return;hostFenceConflictTimer=setTimeout(()=>{hostFenceConflictTimer=null;if(!closed&&source.streamToken===streamToken)source.loseHostFence()},3_500)}};
  const dispatchSourceEvent=(type,data)=>{const terminal=type==='room-closed'||type==='session-revoked'||type==='host-fence-lost';if(type==='host-fence-lost')source.hostFenceLost=true;for(const handler of listeners.get(type)||[])dispatchChain=dispatchChain.then(()=>terminal||!closed?handler({data}):undefined).catch(error=>reportClientIssue('live-signal-handler',error));if(terminal){closed=true;controller?.abort()}};
  const connect=async()=>{controller=new AbortController();const response=await fetch(`/api/live/${encodeURIComponent(liveId)}/signals`,{headers:{authorization:`Bearer ${state.token}`,'x-sylora-peer-id':peerId,'x-sylora-peer-role':role},signal:controller.signal});if(!response.ok){const ended=response.status===404||response.status===410,revoked=response.status===401||response.status===403,fenceConflict=role==='host'&&response.status===409,pendingFence=fenceConflict&&validLiveHostStreamToken(source.streamToken);if(ended)dispatchSourceEvent('room-closed',JSON.stringify({status:'ended'}));else if(revoked)dispatchSourceEvent('session-revoked',JSON.stringify({status:'revoked'}));else if(pendingFence)source.reportHostFenceConflict(source.streamToken);else if(fenceConflict)dispatchSourceEvent('host-fence-lost',JSON.stringify({status:'fence-lost'}));else controller.abort();throw new Error(ended?'LIVE_SIGNAL_ROOM_CLOSED':revoked?'LIVE_SIGNAL_SESSION_REVOKED':pendingFence?'LIVE_SIGNAL_HOST_FENCE_PENDING':fenceConflict?'LIVE_SIGNAL_HOST_FENCE_LOST':`LIVE_SIGNAL_ACCESS_FAILED_${response.status}`)}if(role==='host'){const streamToken=response.headers.get('x-sylora-host-stream-token');if(!validLiveHostStreamToken(streamToken)){dispatchSourceEvent('host-fence-lost',JSON.stringify({status:'missing-fence'}));throw new Error('LIVE_SIGNAL_HOST_FENCE_LOST')}clearTimeout(hostFenceConflictTimer);hostFenceConflictTimer=null;source.streamToken=streamToken;source.hostFenceLost=false}return response};
  const pump=async response=>{const reader=response.body.getReader(),decoder=new TextDecoder();let buffer='';try{while(!closed){const {done,value}=await reader.read();if(done)break;buffer+=decoder.decode(value,{stream:true});let boundary;while(!closed&&(boundary=buffer.indexOf('\n\n'))>=0){const raw=buffer.slice(0,boundary);buffer=buffer.slice(boundary+2);let type='message',data='';for(const line of raw.split('\n')){if(line.startsWith('event:'))type=line.slice(6).trim();else if(line.startsWith('data:'))data+=line.slice(5).trim()}if(data)dispatchSourceEvent(type,data)}}}catch(error){if(error.name!=='AbortError')reportClientIssue('live-signal-stream',error)}if(!closed)setTimeout(async()=>{if(closed)return;try{await pump(await connect())}catch(error){if(!closed){reportClientIssue('live-signal-reconnect',error);pumpReconnect()}}},1500)};
  const pumpReconnect=()=>{if(closed)return;setTimeout(async()=>{if(closed)return;try{await pump(await connect())}catch(error){if(!closed){reportClientIssue('live-signal-reconnect',error);pumpReconnect()}}},2500)};
  onSource?.(source);const response=await connect();pump(response);return source
}
function cleanupLiveViewer(notifyHost=true){liveViewerEpoch++;if(notifyHost&&liveViewerLiveId&&liveViewerId)sendLiveSignal(liveViewerLiveId,'viewer-left',liveViewerId,liveViewerHostPeerId,null).catch(()=>{});if(liveViewerAnnounceTimer){clearInterval(liveViewerAnnounceTimer);liveViewerAnnounceTimer=null}if(liveViewerSource){liveViewerSource.close();liveViewerSource=null}if(liveViewerPublicSource){if(liveEventSource===liveViewerPublicSource)liveEventSource=null;liveViewerPublicSource.close();liveViewerPublicSource=null}if(liveViewerPeer){liveViewerPeer.close();liveViewerPeer=null}liveViewerPendingIce=[];liveViewerId=null;liveViewerLiveId=null;liveViewerHostPeerId=null}
async function openLiveChat(id){
  const sharedSource=liveViewerLiveId===id?liveViewerPublicSource:null;if(liveEventSource&&liveEventSource!==sharedSource)liveEventSource.close();
  ...
  liveEventSource=sharedSource||new EventSource(`/api/live/${id}/events`);
...
function localizeStudioCore(ownRooms){
  ...
  setText('#studioProfileHelp','outputProfileHelp');setText('#cameraBtn','cameraMic');setText('#screenBtn','screenShare');setText('#studioSourceStatus','waitingSource');
...
async function renderStudio(){
  if(app.querySelector('.studio-reference-layout')&&(studioSourceStream||studioSourceRequestPending||studioMediaBusy()||studioObsClient||studioCompanionClient||studioObsCredentials||studioDistributionPoll))return;
  const renderEpoch=++studioRenderEpoch;
  ...
  if(renderEpoch!==studioRenderEpoch||state.view!=='studio'||!state.me)return;
  studioScenes=scenes;
  app.innerHTML=`<div class="card hero"><span class="eyebrow">SYLORA CREATOR STUDIO</span><h1>${esc(u('studioHeroTitle'))}</h1><p>${esc(u('studioHeroCopy'))}</p></div><div class="studio-layout"><div class="studio-stage" data-source-state="idle"><video id="studioSource" autoplay muted playsinline hidden></video><canvas id="studioCanvas" width="720" height="1280"></canvas></div><div class="studio-controls"><div class="card fields"><span class="eyebrow">${esc(u('sources'))}</span><select id="studioProfile" aria-label="${esc(u('outputProfile'))}">${Object.entries(STUDIO_PROFILES).map(([id,p])=>`<option value="${id}" ${p.conditional?'disabled':''}>${p.label}</option>`).join('')}</select><div id="studioProfileHelp" class="studio-note">${esc(u('outputProfile'))}</div><button id="cameraBtn" class="primary">${esc(u('cameraMic'))}</button><button id="screenBtn" class="ghost">${esc(u('screenShare'))}</button><label class="ghost">＋ ${esc(u('logoImage'))}<input id="overlayImage" type="file" accept="image/png,image/jpeg,image/webp" hidden></label><button id="clearImage" class="ghost">${esc(u('removeImage'))}</button><button id="stopSource" class="ghost">${esc(u('stopSource'))}</button><div id="studioSourceStatus" class="studio-note" role="status" aria-live="polite">${esc(u('waitingSource'))}</div></div><div class="card fields"><span class="eyebrow">${esc(u('audioMixer'))}</span><label class="studio-mixer-label"><span>${esc(u('micSource'))}</span><strong id="studioGainValue">100%</strong></label><input id="studioMicGain" type="range" min="0" max="150" value="100" step="1" disabled aria-label="${esc(u('micSource'))}"><div class="studio-meter"><i id="studioAudioMeter"></i></div><button id="studioMicMute" class="ghost" disabled aria-pressed="false">${esc(u('mute'))}</button><div id="studioAudioStatus" class="studio-note" role="status" aria-live="polite">${esc(u('enableAudio'))}</div></div><div class="card fields"><span class="eyebrow">${esc(u('scenes'))}</span><select id="savedScene"><option value="">${esc(u('savedScene'))}</option>${scenes.map(s=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('')}</select><input id="sceneName" placeholder="${esc(u('sceneName'))}" maxlength="60"><input id="overlayTitle" value="SYLORA LIVE" maxlength="60"><select id="overlayStyle"><option value="violet">Violet Glow</option><option value="cyan">Cyan Pulse</option><option value="clean">Clean</option></select><button id="saveScene" class="ghost">${esc(u('saveScene'))}</button><button id="deleteScene" class="ghost">${esc(u('deleteScene'))}</button></div><div class="card fields"><span class="eyebrow">${esc(u('broadcast'))}</span><select id="studioLiveRoom"><option value="">${esc(u('chooseLive'))}</option>${ownRooms.map(r=>`<option value="${esc(r.id)}">${esc(r.title)}</option>`).join('')}</select><button id="createStudioLive" class="ghost">＋ ${esc(u('createLive'))}</button><button id="broadcastBtn" class="primary">${esc(u('startWebrtcLive'))}</button><button id="stopBroadcast" class="ghost" disabled>${esc(u('stopBroadcast'))}</button><div id="broadcastStatus" class="studio-note">${esc(ownRooms.length?u('liveReady'):u('createLiveFirst'))}</div></div><div class="card fields"><span class="eyebrow">OBS WEBSOCKET 5.x</span><input id="obsUrl" value="${esc(localStorage.getItem('sylora_obs_url')||'ws://127.0.0.1:4455')}" aria-label="OBS WebSocket URL"><input id="obsPassword" type="password" placeholder="${esc(u('obsPasswordPlaceholder'))}" autocomplete="off"><button id="obsConnect" class="ghost">${esc(u('connectObs'))}</button><select id="obsScenes" disabled><option value="">${esc(u('obsScenes'))}</option></select><button id="obsVirtualStart" class="ghost" disabled>${esc(u('startVirtualCamera'))}</button><button id="obsVirtualStop" class="ghost" disabled>${esc(u('stopVirtualCamera'))}</button><button id="obsStreamStart" class="primary" disabled>${esc(u('startObsStream'))}</button><button id="obsStreamStop" class="ghost" disabled>${esc(u('stopObsStream'))}</button><div id="obsStatus" class="studio-note">${esc(u('obsLocalPassword'))}</div><div class="studio-note">${esc(u('obsStreamSafety'))}</div></div><div class="card fields"><span class="eyebrow">${esc(u('browserSource'))}</span><button id="browserSourceCreate" class="ghost">${esc(u('createBrowserUrl'))}</button><input id="browserSourceUrl" readonly placeholder="${esc(u('urlAppears'))}"><button id="browserSourceCopy" class="ghost" disabled>${esc(u('copyUrl'))}</button><div id="browserSourceStatus" class="studio-note">${esc(u('temporaryUrl'))}</div></div><div class="card"><span class="eyebrow">${esc(u('record'))}</span><button id="recordBtn" class="primary">● ${esc(u('startRecording'))}</button><button id="stopRecord" class="ghost" disabled>${esc(u('stop'))}</button><div id="recordStatus" class="studio-note" role="status" aria-live="polite">${esc(u('waitingSource'))}</div><div id="recordResult"></div></div></div></div><p class="studio-note">${esc(u('studioSecurityNote'))}</p>`;
  ...
  if(legacyLayout&&stage&&controls){
    ...
    referenceLayout.innerHTML=`<header class="studio-modebar"><div class="studio-suite"><span>${referenceIcon('studio')}</span><div><small>SYLORA CREATOR SUITE</small><b>${esc(u('studio'))} · ${scenes.length} ${esc(u('scenes').toLowerCase())} · ${ownRooms.length} LIVE</b></div></div><nav><button class="active" type="button" data-studio-panel-target="sources"><i>01</i> ${esc(u('program'))}</button><button type="button" data-studio-panel-target="scenes"><i>02</i> ${esc(u('scenes'))}</button><button type="button" data-studio-panel-target="distribution"><i>03</i> ${esc(u('multistream'))}</button><button type="button" data-studio-panel-target="obs"><i>04</i> ${esc(u('integrations'))}</button></nav><div class="connection-health"><i></i><span><b>${esc(distribution.configuration?.configured?u('routerReadyUpper'):u('localStudioReady'))}</b><small>OBS · TikTok · YouTube · TikFinity</small></span></div></header><aside class="scene-rail"><div class="scene-rail-title"><b>${esc(u('scenes'))}</b><button type="button" data-studio-panel-target="scenes">＋</button></div>${scenes.length?scenes.slice(0,6).map((scene,index)=>`<button type="button" class="scene-thumb ${index===0?'active':''}" data-studio-scene="${esc(scene.id)}"><span class="scene-preview"><span class="scene-art" aria-hidden="true">✦</span></span><span><b>${esc(scene.name)}</b><small>${esc(u('savedSceneLabel'))}</small></span>${index===0?'<i></i>':''}</button>`).join(''):`<button type="button" class="scene-thumb active" data-studio-panel-target="scenes"><span class="scene-preview">＋</span><span><b>${esc(u('currentScene'))}</b><small>${esc(u('notSaved'))}</small></span><i></i></button>`}</aside><main class="program-panel"><div class="program-header"><span><i></i> ${esc(u('program').toUpperCase())} · LIVING HORIZON</span><div><button type="button" data-studio-proxy="screen" aria-label="${esc(u('shareScreen'))}">▣</button><button type="button" data-studio-panel-target="sources" aria-label="${esc(u('sources'))}">＋</button></div></div><div data-program-slot></div><div class="destination-strip"><span class="destination-label">${referenceIcon('live')}<span><small>${esc(u('multistream').toUpperCase())}</small><b>${connectedProviders.size} ${esc(u('active'))}</b></span></span><div class="destination-items">${destinationChip('tiktok','TT','TikTok','tone-tiktok')}${destinationChip('youtube','YT','YouTube','tone-youtube')}${destinationChip('twitch','TW','Twitch','tone-twitch')}${destinationChip('facebook','FB','Facebook','tone-facebook')}</div></div><div class="transport"><button type="button" data-studio-proxy="camera" aria-label="${esc(u('cameraMic'))}">▣</button><button type="button" data-studio-proxy="screen" aria-label="${esc(u('screenShare'))}">▱</button><button type="button" data-studio-proxy="record" aria-label="${esc(u('record'))}">●</button><button type="button" data-studio-panel-target="audio" aria-label="${esc(u('audioMixer'))}">≋</button><button type="button" class="depth-button" data-studio-proxy="broadcast">${referenceIcon('live')} ${esc(u('startLive'))}</button></div></main><aside data-inspector-slot></aside>`;
    stage.classList.add('program-canvas');stage.insertAdjacentHTML('beforeend',`<span class="program-safe-zone" aria-hidden="true"></span><span class="program-badge"><span class="ai-mark">✦</span><span><small>SYLORA AI DIRECTOR</small><b>${esc(u('aiDirectorReady'))}</b></span></span><span class="multistream-badge">${referenceIcon('activity')}<span><small>OUTPUT</small><b>${distribution.configuration?.configured?'RTMPS ROUTER READY':'LOCAL / WEBRTC'}</b></span></span>`);
    controls.classList.add('inspector-panel');controls.insertAdjacentHTML('afterbegin',`<div class="panel-tabs"><button class="active" type="button">${esc(u('controls'))}</button><button type="button">${esc(u('connectors'))}</button></div><div class="integration-flow"><span>TF</span><div><b>TikFinity → SYLORA Companion</b><small>${esc(u('companionPairingFlow'))}</small></div>${referenceIcon('arrow')}</div>`);
  ...
  document.querySelector('#app>.hero')?.insertAdjacentHTML('beforeend',`<div class="scene-readout studio-readout"><span><small>${esc(u('scenes').toUpperCase())}</small><b>${scenes.length}</b></span><span><small>${esc(u('myLive'))}</small><b>${ownRooms.length}</b></span><span><small>OUTPUT</small><b>${Object.keys(STUDIO_PROFILES).length} PROFILES</b></span></div>`);
  document.querySelector('#obsConnect').insertAdjacentHTML('beforebegin',`<input id="companionUrl" value="${esc(localStorage.getItem('sylora_companion_url')||'http://127.0.0.1:43179')}" aria-label="SYLORA Companion URL"><input id="companionToken" type="password" placeholder="${esc(u('companionPairingToken'))}" autocomplete="off"><button id="companionConnect" class="primary">${esc(u('connectViaCompanion'))}</button><div class="studio-note">${esc(u('companionLocalSecretNote'))}</div>`);
  const profileSelect=document.querySelector('#studioProfile'),savedProfile=localStorage.getItem('sylora_studio_profile'),savedProfileConfig=STUDIO_PROFILES[savedProfile];profileSelect.value=savedProfileConfig&&!savedProfileConfig.conditional?savedProfile:'vertical720';restoreStudioRecovery();profileSelect.dataset.activeProfile=profileSelect.value;profileSelect.onchange=handleStudioProfileChange;document.querySelector('#cameraBtn').onclick=()=>startStudioSource('camera');document.querySelector('#screenBtn').onclick=()=>startStudioSource('screen');document.querySelector('#stopSource').onclick=requestStopStudioSource;document.querySelector('#recordBtn').onclick=startStudioRecording;document.querySelector('#stopRecord').onclick=stopStudioRecording;document.querySelector('#broadcastBtn').onclick=startStudioBroadcast;document.querySelector('#stopBroadcast').onclick=stopStudioBroadcast;document.querySelector('#overlayImage').onchange=loadStudioOverlayImage;document.querySelector('#clearImage').onclick=()=>{studioOverlayImage=null};document.querySelector('#savedScene').onchange=()=>loadStudioScene(studioScenes);document.querySelector('#saveScene').onclick=saveStudioScene;document.querySelector('#deleteScene').onclick=deleteStudioScene;document.querySelector('#studioMicGain').oninput=updateStudioAudioGain;document.querySelector('#studioMicMute').onclick=toggleStudioAudioMute;document.querySelector('#overlayTitle').oninput=persistStudioRecovery;document.querySelector('#overlayStyle').onchange=persistStudioRecovery;document.querySelector('#companionConnect').onclick=connectStudioCompanion;document.querySelector('#obsConnect').onclick=connectStudioObs;document.querySelector('#obsScenes').onchange=changeStudioObsScene;document.querySelector('#obsVirtualStart').onclick=()=>setStudioVirtualCamera(true);document.querySelector('#obsVirtualStop').onclick=()=>setStudioVirtualCamera(false);document.querySelector('#obsStreamStart').onclick=()=>setStudioObsStream(true);document.querySelector('#obsStreamStop').onclick=()=>setStudioObsStream(false);document.querySelector('#browserSourceCreate').onclick=createStudioBrowserSource;document.querySelector('#browserSourceCopy').onclick=copyStudioBrowserSource;
  document.querySelector('#createStudioLive').onclick=async()=>{const title=prompt(u('liveTitlePrompt'))||'SYLORA Studio LIVE';const {live}=await api('/api/live',{method:'POST',body:JSON.stringify({title})});const select=document.querySelector('#studioLiveRoom'),option=document.createElement('option');option.value=live.id;option.textContent=live.title;select.append(option);select.value=live.id;document.querySelector('#broadcastStatus').textContent=u('liveCreatedEnableSource');await refreshStudioDistributionStatus()};
  ...
  document.querySelectorAll('[data-studio-panel-target]').forEach(button=>button.onclick=()=>openStudioPanel(button));
  document.querySelectorAll('[data-studio-scene]').forEach(button=>button.onclick=()=>{const select=document.querySelector('#savedScene');select.value=button.dataset.studioScene;if(loadStudioScene(studioScenes))document.querySelectorAll('[data-studio-scene]').forEach(item=>item.classList.toggle('active',item===button))});
  configureStudioCanvas();drawStudioPlaceholder();syncStudioOutputLock();mountStudioIntelligence(ownRooms)
}
...
function openStudioPanel(button){const panel=button?.dataset?.studioPanelTarget;if(!panel)return;document.querySelectorAll('.studio-modebar nav button').forEach(item=>item.classList.toggle('active',item===button));if(matchMedia('(max-width: 900px)').matches){document.dispatchEvent(new CustomEvent('sylora:studio-open-panel',{detail:{panel}}));return}document.querySelector(`[data-studio-panel="${panel}"]`)?.scrollIntoView({behavior:'smooth',block:'nearest'})}
...
function mountStudioDistribution(distribution){
  ...
  panel.innerHTML=`<div class="distribution-heading"><span class="eyebrow">MULTISTREAM DISTRIBUTION</span><span class="badge ${distribution.configuration.configured?'is-ready':'is-blocked'}">${esc(distribution.configuration.configured?u('routerReady'):u('setupRequired'))}</span></div><h3>${esc(u('oneLiveAllChannels'))}</h3><p class="studio-note">${esc(u('distributionPrivacyNote'))}</p><div id="distributionDestinations" class="distribution-destinations">${distribution.destinations.map(item=>distributionDestinationMarkup(item,selected)).join('')||`<p class="studio-note">${esc(u('noDestinations'))}</p>`}</div><details class="distribution-add"><summary>＋ ${esc(u('addDestination'))}</summary><form id="distributionDestinationForm" class="fields"><label>${esc(u('platform'))}<select name="provider" required>${distribution.providers.map(item=>`<option value="${esc(item.id)}">${esc(item.label)}</option>`).join('')}</select></label><label>${esc(u('name'))}<input name="label" maxlength="60" placeholder="YouTube" required></label><label>RTMPS server<input name="serverUrl" maxlength="600" inputmode="url" placeholder="rtmps://a.rtmp.youtube.com/live2" required></label><label>Stream key<input name="streamKey" type="password" maxlength="512" autocomplete="new-password" placeholder="${esc(u('pastePlatformKey'))}" required></label><button class="primary" type="submit">${esc(u('encryptAdd'))}</button></form></details><label class="distribution-record"><input id="distributionRecord" type="checkbox" checked><span>${esc(u('keepBackupRecording'))}</span></label><div class="distribution-actions"><button id="distributionPreflight" class="ghost" type="button">${esc(u('checkReadiness'))}</button><button id="distributionStart" class="primary" type="button">${esc(u('prepareMultistream'))}</button><button id="distributionStop" class="ghost danger" type="button" disabled>${esc(u('stopMultistream'))}</button></div><div id="distributionStatus" class="studio-note" aria-live="polite">${esc(u('distributionInstructions'))}</div><div id="distributionIngest" class="distribution-ingest" hidden><strong>OBS → Settings → Stream → Custom</strong><label>Server<input id="distributionIngestServer" readonly></label><label>${esc(u('streamKeyOnce'))}<input id="distributionIngestKey" type="password" readonly></label><div class="distribution-actions"><button id="distributionCopyServer" class="ghost" type="button">${esc(u('copyServer'))}</button><button id="distributionCopyKey" class="primary" type="button">${esc(u('copyKey'))}</button><button id="distributionForgetKey" class="ghost" type="button">${esc(u('hideKey'))}</button></div></div>`;
  ...
  panel.querySelector('#distributionCopyServer').onclick=()=>copyStudioDistributionValue('#distributionIngestServer',u('copyServerDone'));
  panel.querySelector('#distributionCopyKey').onclick=()=>copyStudioDistributionValue('#distributionIngestKey',u('copyKeyDone'));
...
async function createStudioDistributionDestination(event){event.preventDefault();const form=event.currentTarget,button=form.querySelector('button[type="submit"]'),input=Object.fromEntries(new FormData(form));button.disabled=true;button.textContent=u('encrypting');try{await api('/api/studio/distribution/destinations',{method:'POST',body:JSON.stringify(input)});form.reset();toast(u('destinationAdded'));await reloadStudioDistributionPanel()}catch(error){toast(distributionErrorText(error));button.disabled=false;button.textContent=u('encryptAdd')}}
...
async function deleteStudioDistributionDestination(id){if(!confirm(u('deleteDestinationConfirm')))return;try{await api(`/api/studio/distribution/destinations/${id}`,{method:'DELETE'});await reloadStudioDistributionPanel();toast(u('destinationDeleted'))}catch(error){toast(distributionErrorText(error))}}
async function preflightStudioDistribution(){const liveId=document.querySelector('#studioLiveRoom')?.value,status=document.querySelector('#distributionStatus');if(!liveId)return toast(u('chooseOrCreateLive'));status.textContent=u('checkingDistribution');try{const out=await api(`/api/live/${liveId}/distribution/preflight`,{method:'POST',body:JSON.stringify({destinationIds:distributionSelectedIds()})});status.textContent=out.ready?`${u('distributionReady')} · ${out.destinations.length} ${u('destinationsCount')} · ${u('routerResponding')}.`:distributionErrorText({data:{details:{reasons:out.reasons}}})}catch(error){status.textContent=distributionErrorText(error)}}
function renderStudioDistributionStatus(out){const status=document.querySelector('#distributionStatus'),start=document.querySelector('#distributionStart'),stop=document.querySelector('#distributionStop'),session=out?.session;if(!status||!start||!stop)return;const active=session&&['preparing','waiting_for_source','live','degraded'].includes(session.status);start.disabled=!!active;stop.disabled=!active;if(!session){status.textContent=u('multistreamNotPrepared');stopStudioDistributionPolling();return}if(session.status==='live'){const forwarding=session.destinations.filter(x=>x.status==='forwarding').length;status.textContent=`● MULTISTREAM LIVE · ${forwarding}/${session.destinations.length} ${u('destinationsCount')} · in ${formatDistributionBytes(session.inboundBytes)} · out ${formatDistributionBytes(session.outboundBytes)}`}else if(session.status==='degraded'){const failed=session.destinations.filter(x=>x.status==='error'||x.hasError).map(x=>x.label);status.textContent=`⚠ MULTISTREAM DEGRADED · ${u('errorLabelLower')}: ${failed.join(', ')||u('externalDestination')}`}else if(session.status==='waiting_for_source')status.textContent=`${u('waitingObsSource')} · ${session.destinations.length} ${u('destinationsConfigured')} · key ${session.ingestKeyFingerprint}`;else if(session.status==='failed')status.textContent=`⚠ ${u('multistreamPathLost')}`;else if(session.status==='stopped')status.textContent=`${u('multistreamStopped')} · ${new Date(session.stoppedAt).toLocaleString()}`;else status.textContent=`Multistream ${session.status} · router ${session.routerReachable===false?u('routerOffline'):u('routerOnline')}`;if(active&&!studioDistributionPoll)studioDistributionPoll=setInterval(()=>refreshStudioDistributionStatus().catch(()=>{}),5000);if(!active)stopStudioDistributionPolling()}
...
async function startStudioDistribution(){const liveId=document.querySelector('#studioLiveRoom')?.value,status=document.querySelector('#distributionStatus'),button=document.querySelector('#distributionStart');if(!liveId)return toast(u('chooseOrCreateLive'));const destinationIds=distributionSelectedIds();button.disabled=true;status.textContent=u('creatingSecureIngest');try{const out=await api(`/api/live/${liveId}/distribution/start`,{method:'POST',body:JSON.stringify({destinationIds,record:document.querySelector('#distributionRecord')?.checked!==false})}),ingest=document.querySelector('#distributionIngest');document.querySelector('#distributionIngestServer').value=out.ingest.serverUrl;document.querySelector('#distributionIngestKey').value=out.ingest.streamKey;ingest.hidden=false;renderStudioDistributionStatus({session:out.session});toast(u('multistreamPrepared'))}catch(error){button.disabled=false;status.textContent=distributionErrorText(error)}}
async function stopStudioDistribution(){const liveId=document.querySelector('#studioLiveRoom')?.value;if(!liveId)return;if(!confirm(u('stopAllDistributionConfirm')))return;const button=document.querySelector('#distributionStop');button.disabled=true;try{const out=await api(`/api/live/${liveId}/distribution/stop`,{method:'POST',body:'{}'});forgetStudioDistributionKey();renderStudioDistributionStatus(out);toast(u('multistreamStoppedAll'))}catch(error){button.disabled=false;toast(distributionErrorText(error))}}
async function copyStudioDistributionValue(selector,message){const input=document.querySelector(selector);if(!input?.value)return;try{await navigator.clipboard.writeText(input.value);toast(message)}catch{input.select();toast(u('copyManual'))}}
...
async function openStudioObs(reconnecting){if(!studioObsCredentials||state.view!=='studio')return;clearTimeout(studioObsReconnectTimer);studioObsClient?.disconnect();const status=document.querySelector('#obsStatus');if(status)status.textContent=reconnecting?`${u('obsReconnect')} · ${u('attempt')} ${studioObsReconnectAttempt}…`:u('obsConnecting');try{const client=new ObsWebSocketClient({...studioObsCredentials,onDisconnect:scheduleStudioObsReconnect});studioObsClient=client;await client.connect();const caps=await client.capabilities();if(studioObsClient!==client)return;studioObsReconnectAttempt=0;applyStudioObsCapabilities(caps,'OBS')}catch(error){if(status)status.textContent=`OBS: ${obsErrorText(error)}`;scheduleStudioObsReconnect()}}
function scheduleStudioObsReconnect(){if(!studioObsCredentials||state.view!=='studio'||studioObsReconnectTimer)return;studioObsReconnectAttempt=Math.min(6,studioObsReconnectAttempt+1);const delay=Math.min(10000,750*2**(studioObsReconnectAttempt-1));studioObsReconnectTimer=setTimeout(()=>{studioObsReconnectTimer=null;openStudioObs(true)},delay);const status=document.querySelector('#obsStatus');if(status)status.textContent=`OBS offline · ${u('reconnectIn')} ${(delay/1000).toFixed(1)}s`}
function obsErrorText(error){const labels={OBS_LOCALHOST_ONLY:u('obsLocalOnly'),OBS_URL_PROTOCOL:u('obsProtocol'),OBS_PASSWORD_REQUIRED:u('obsPasswordRequired'),OBS_CONNECTION_FAILED:u('obsConnectionFailed'),OBS_CONNECT_TIMEOUT:u('obsTimeout'),COMPANION_LOCALHOST_ONLY:u('companionLocalOnly'),COMPANION_URL_PROTOCOL:u('companionProtocol'),PAIRING_TOKEN_REQUIRED:u('pairingTokenRequired'),PAIRING_REQUIRED:u('pairingRequired'),ORIGIN_NOT_ALLOWED:u('companionOriginBlocked')};return labels[error?.message]||humanError(error?.message||u('connectionError'))}
...
async function changeStudioObsScene(event){if(!event.target.value)return;try{await studioObsCommand('setScene',event.target.value);document.querySelector('#obsStatus').textContent=`${u('obsScene')} → ${event.target.value}`}catch(error){document.querySelector('#obsStatus').textContent=`${u('obsSceneError')}: ${humanError(error.message)}`}}
async function setStudioVirtualCamera(active){try{await studioObsCommand(active?'startVirtualCamera':'stopVirtualCamera');document.querySelector('#obsVirtualStart').disabled=active;document.querySelector('#obsVirtualStop').disabled=!active;document.querySelector('#obsStatus').textContent=`OBS Virtual Camera ${active?'ON':'OFF'}`}catch(error){document.querySelector('#obsStatus').textContent=`${u('virtualCameraError')}: ${humanError(error.message)}`}}
async function setStudioObsStream(active){try{await studioObsCommand(active?'startStream':'stopStream');document.querySelector('#obsStreamStart').disabled=active;document.querySelector('#obsStreamStop').disabled=!active;document.querySelector('#obsStatus').textContent=`OBS Stream ${active?'LIVE':'OFF'} · ${u('obsTargetManagedLocally')}`}catch(error){document.querySelector('#obsStatus').textContent=`${u('obsStreamError')}: ${humanError(error.message)}`}}
async function createStudioBrowserSource(){const liveId=document.querySelector('#studioLiveRoom')?.value,status=document.querySelector('#browserSourceStatus');if(!liveId)return toast(u('createLiveFirst'));try{const out=await api('/api/studio/browser-source',{method:'POST',body:JSON.stringify({liveId})}),url=new URL(out.path,location.origin).toString(),input=document.querySelector('#browserSourceUrl');input.value=url;document.querySelector('#browserSourceCopy').disabled=false;status.textContent=`${u('browserSourceReadyUntil')} ${new Date(out.expiresAt).toLocaleTimeString()}. ${u('addUrlToObs')}`}catch(error){status.textContent=`Browser Source: ${humanError(error.message)}`}}
async function copyStudioBrowserSource(){const input=document.querySelector('#browserSourceUrl');if(!input?.value)return;try{await navigator.clipboard.writeText(input.value);toast(u('browserSourceCopied'))}catch{input.select();toast(u('urlSelectedCopyManually'))}}
...
function restoreStudioRecovery(){try{const saved=JSON.parse(localStorage.getItem('sylora_studio_recovery')||'null');if(!saved)return;const profile=STUDIO_PROFILES[saved.profileId];if(profile&&!profile.conditional)document.querySelector('#studioProfile').value=saved.profileId;if(typeof saved.overlayTitle==='string')document.querySelector('#overlayTitle').value=saved.overlayTitle;if(['violet','cyan','clean'].includes(saved.overlayStyle))document.querySelector('#overlayStyle').value=saved.overlayStyle;const gain=Math.max(0,Math.min(150,Number(saved.micGain)));if(Number.isFinite(gain))document.querySelector('#studioMicGain').value=String(gain);const mute=document.querySelector('#studioMicMute');if(mute){mute.dataset.muted=saved.micMuted?'1':'0';mute.textContent=t(saved.micMuted?'unmute':'mute');mute.setAttribute('aria-pressed',String(Boolean(saved.micMuted)))}}catch{}}
function loadStudioScene(scenes){
  const select=document.querySelector('#savedScene'),id=select?.value,scene=scenes.find(x=>x.id===id);if(!scene)return;
  if(studioMediaBusy()||studioSourceRequestPending){if(select)select.value=studioLoadedSceneId;toast(t('outputLocked'));return false}
  const profileSelect=document.querySelector('#studioProfile'),profileOption=[...(profileSelect?.options||[])].find(option=>option.value===scene.profileId);
  if(!STUDIO_PROFILES[scene.profileId]||profileOption?.disabled){if(select)select.value=studioLoadedSceneId;toast(t('highQualityUnavailable'));return false}
  document.querySelector('#sceneName').value=scene.name;document.querySelector('#overlayTitle').value=scene.overlayTitle;document.querySelector('#overlayStyle').value=scene.overlayStyle;profileSelect.value=scene.profileId;profileSelect.dataset.activeProfile=scene.profileId;studioLoadedSceneId=scene.id;
  const gain=document.querySelector('#studioMicGain'),mute=document.querySelector('#studioMicMute');if(gain)gain.value=String(Math.max(0,Math.min(150,Number(scene.micGain??100))));if(mute){mute.dataset.muted=scene.micMuted?'1':'0';mute.textContent=t(scene.micMuted?'unmute':'mute');mute.setAttribute('aria-pressed',String(Boolean(scene.micMuted)))}localStorage.setItem('sylora_studio_profile',profileSelect.value);persistStudioRecovery();configureStudioCanvas();updateStudioAudioGain();if(studioSourceStream)composeStudioFrame();else drawStudioPlaceholder();return true
}
function refreshStudioSceneUi(selectedId=''){
  const select=document.querySelector('#savedScene');if(select){select.innerHTML=`<option value="">${esc(t('savedScene'))}</option>${studioScenes.map(scene=>`<option value="${esc(scene.id)}">${esc(scene.name)}</option>`).join('')}`;select.value=studioScenes.some(scene=>scene.id===selectedId)?selectedId:''}
  const rail=document.querySelector('.scene-rail');if(rail){rail.querySelectorAll('.scene-thumb').forEach(node=>node.remove());const markup=studioScenes.length?studioScenes.slice(0,6).map((scene,index)=>`<button type="button" class="scene-thumb ${scene.id===selectedId||!selectedId&&index===0?'active':''}" data-studio-scene="${esc(scene.id)}"><span class="scene-preview"><span class="scene-art" aria-hidden="true">✦</span></span><span><b>${esc(scene.name)}</b><small>${esc(t('savedScene').replace(/…$/,''))}</small></span>${scene.id===selectedId||!selectedId&&index===0?'<i></i>':''}</button>`).join(''):`<button type="button" class="scene-thumb active" data-studio-panel-target="scenes"><span class="scene-preview">＋</span><span><b>${esc(t('currentScene'))}</b><small>${esc(t('notSaved'))}</small></span><i></i></button>`;rail.insertAdjacentHTML('beforeend',markup);rail.querySelectorAll('[data-studio-scene]').forEach(button=>button.onclick=()=>{select.value=button.dataset.studioScene;if(loadStudioScene(studioScenes))rail.querySelectorAll('[data-studio-scene]').forEach(item=>item.classList.toggle('active',item===button))});rail.querySelectorAll('[data-studio-panel-target]').forEach(button=>button.onclick=()=>openStudioPanel(button))}
  const sceneCount=document.querySelector('.studio-readout span:first-child b');if(sceneCount)sceneCount.textContent=String(studioScenes.length);const suite=document.querySelector('.studio-suite b');if(suite)suite.textContent=`Studio · ${studioScenes.length} scenes · ${document.querySelectorAll('#studioLiveRoom option:not([value=""])').length} LIVE`;
}
async function saveStudioScene(){const id=document.querySelector('#savedScene').value,name=document.querySelector('#sceneName').value,preset=studioPresetState();try{const {scene}=await api(id?`/api/studio/scenes/${id}`:'/api/studio/scenes',{method:id?'PATCH':'POST',body:JSON.stringify({name,...preset})}),index=studioScenes.findIndex(item=>item.id===scene.id);if(index>=0)studioScenes[index]=scene;else studioScenes.unshift(scene);studioLoadedSceneId=scene.id;persistStudioRecovery();refreshStudioSceneUi(scene.id);toast(t(id?'sceneUpdated':'sceneSaved'))}catch(e){toast(humanError(e.message))}}
async function deleteStudioScene(){const id=document.querySelector('#savedScene').value;if(!id)return toast(t('chooseSavedScene'));try{await api(`/api/studio/scenes/${id}`,{method:'DELETE'});studioScenes=studioScenes.filter(scene=>scene.id!==id);studioLoadedSceneId='';document.querySelector('#sceneName').value='';refreshStudioSceneUi();toast(t('sceneDeleted'))}catch(error){toast(humanError(error.message))}}

function studioMediaBusy(){return studioRecorder?.state==='recording'||studioRecordingFinalizing||studioBroadcastPending||studioBroadcastStopping||Boolean(studioBroadcastStream)}
function setStudioSourceState(next,key,detail=''){const stage=document.querySelector('.studio-stage'),status=document.querySelector('#studioSourceStatus');if(stage)stage.dataset.sourceState=next;if(status)status.textContent=`${t(key)}${detail?` · ${detail}`:''}`}
function syncStudioOutputLock(){
  const mediaBusy=studioMediaBusy(),locked=mediaBusy||studioSourceRequestPending,profile=document.querySelector('#studioProfile'),scene=document.querySelector('#savedScene'),help=document.querySelector('#studioProfileHelp');
  if(profile)profile.disabled=locked;if(scene)scene.disabled=locked;for(const selector of ['#saveScene','#deleteScene']){const button=document.querySelector(selector);if(button)button.disabled=locked}
  for(const selector of ['#cameraBtn','#screenBtn']){const button=document.querySelector(selector);if(button)button.disabled=locked}
  const stopSource=document.querySelector('#stopSource');if(stopSource)stopSource.disabled=locked||!studioSourceStream;
  const record=document.querySelector('#recordBtn'),broadcast=document.querySelector('#broadcastBtn');if(record)record.disabled=locked||!studioSourceStream;if(broadcast)broadcast.disabled=locked||!studioSourceStream;
  if(help)help.textContent=t(mediaBusy?'outputLocked':'outputProfileHelp');
}
function handleStudioProfileChange(event){
  const select=event.currentTarget,previous=select.dataset.activeProfile||'vertical720',option=select.selectedOptions[0];
  if(studioMediaBusy()){select.value=previous;toast(t('outputLocked'));return}
  if(option?.disabled){select.value=previous;toast(t('highQualityUnavailable'));return}
  select.dataset.activeProfile=select.value;localStorage.setItem('sylora_studio_profile',select.value);persistStudioRecovery();configureStudioCanvas();if(studioSourceStream)composeStudioFrame();else drawStudioPlaceholder();
}
function updateStudioProfileAvailability(track){
  const select=document.querySelector('#studioProfile');if(!select||!track)return;
  const capabilities=track.getCapabilities?.()||{},settings=track.getSettings?.()||{},maxWidth=Number(capabilities.width?.max||settings.width||0),maxHeight=Number(capabilities.height?.max||settings.height||0),longEdge=Math.max(maxWidth,maxHeight),shortEdge=Math.min(maxWidth,maxHeight),cores=Number(navigator.hardwareConcurrency||0),memory=Number(navigator.deviceMemory||0),canvas=document.querySelector('#studioCanvas'),encoderReady=typeof MediaRecorder!=='undefined'&&['video/webm;codecs=vp9,opus','video/webm;codecs=vp8,opus','video/webm','video/mp4'].some(type=>MediaRecorder.isTypeSupported?.(type)),recorderReady=encoderReady&&typeof canvas?.captureStream==='function';
  for(const [id,profile] of Object.entries(STUDIO_PROFILES)){if(!profile.conditional)continue;const option=select.querySelector(`option[value="${id}"]`),resolutionReady=longEdge>=Math.max(profile.width,profile.height)&&shortEdge>=Math.min(profile.width,profile.height),cpuFloor=id==='horizontal2160'?8:6,memoryFloor=id==='horizontal2160'?8:4,deviceReady=(!cores||cores>=cpuFloor)&&(!memory||memory>=memoryFloor);if(option)option.disabled=!(recorderReady&&resolutionReady&&deviceReady)}
  if(select.selectedOptions[0]?.disabled){select.value='vertical720';select.dataset.activeProfile='vertical720';localStorage.setItem('sylora_studio_profile','vertical720');configureStudioCanvas();toast(t('highQualityUnavailable'))}
}

function studioBroadcastIsCurrent(epoch,hostPeerId,stream){return epoch===studioBroadcastEpoch&&hostPeerId===studioHostPeerId&&stream===studioBroadcastStream&&state.view==='studio'}
function studioHostPeerIdForLive(liveId){const key=`sylora_studio_host_peer:${liveId}`;try{const saved=sessionStorage.getItem(key);if(/^host:[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(saved||''))return saved;const id=`host:${crypto.randomUUID()}`;sessionStorage.setItem(key,id);return id}catch{return`host:${crypto.randomUUID()}`}}
const studioHostReleaseDelay=ms=>new Promise(resolve=>setTimeout(resolve,ms));
async function sendStudioHostSignal(liveId,kind,hostPeerId,toPeerId,data,session,{signal}={}){const source=session?.source;let streamToken=source?.streamToken;if(!validLiveHostStreamToken(streamToken)){source?.loseHostFence();throw new Error('LIVE_SIGNAL_HOST_FENCE_LOST')}for(let attempt=0;attempt<2;attempt++)try{return await sendLiveSignal(liveId,kind,hostPeerId,toPeerId,data,{signal,streamToken})}catch(error){const fenceError=error.message==='LIVE_HOST_FENCE_LOST'||error.message==='HOST_STREAM_TOKEN_REQUIRED',replacement=source.streamToken;if(fenceError&&replacement!==streamToken&&validLiveHostStreamToken(replacement)&&!source.hostFenceLost&&attempt===0){streamToken=replacement;continue}if(fenceError&&source.streamToken===streamToken)source.reportHostFenceConflict(streamToken);throw error}}
async function releaseStudioHost(liveId,hostPeerId,streamToken){if(!liveId||!hostPeerId||!validLiveHostStreamToken(streamToken))return true;for(let attempt=0;attempt<4;attempt++){const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),1_200);try{await sendLiveSignal(liveId,'host-left',hostPeerId,null,null,{signal:controller.signal,streamToken});try{const key=`sylora_studio_host_peer:${liveId}`;if(sessionStorage.getItem(key)===hostPeerId)sessionStorage.removeItem(key)}catch{}return true}catch(error){if(error.message==='LIVE_HOST_FENCE_LOST'||error.status===409)return true;if(attempt<3)await studioHostReleaseDelay(200*(attempt+1))}finally{clearTimeout(timeout)}}return false}
async function startStudioBroadcast(){
  const liveId=document.querySelector('#studioLiveRoom')?.value;if(!liveId)return toast(t('chooseLive'));if(!studioSourceStream)return toast(t('enableCameraScreen'));if(studioSourceRequestPending)return toast(t('sourceChangeBlocked'));if(studioMediaBusy())return;
  const canvas=document.querySelector('#studioCanvas'),profile=currentStudioProfile(),epoch=++studioBroadcastEpoch,hostPeerId=studioHostPeerIdForLive(liveId),broadcastStream=canvas.captureStream(profile.fps);for(const track of studioOutputAudioTracks())broadcastStream.addTrack(track);
  studioBroadcastPending=true;studioBroadcastHostAcquired=false;studioBroadcastLiveId=liveId;studioHostPeerId=hostPeerId;studioBroadcastStream=broadcastStream;const start=document.querySelector('#broadcastBtn'),stop=document.querySelector('#stopBroadcast'),status=document.querySelector('#broadcastStatus');if(start)start.disabled=true;if(stop)stop.disabled=false;if(status)status.textContent=t('connecting');syncStudioOutputLock();let source=null;
  try{
    const connectTask=openLiveSignalSource(liveId,hostPeerId,'host',pending=>{if(studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream)){studioLiveSource=pending;const terminate=reason=>{if(!studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream))return;const revoked=reason==='revoked',cleanup=revoked?stopStudioTracks():stopStudioBroadcast();cleanup.finally(()=>{const currentStatus=document.querySelector('#broadcastStatus');if(state.view==='studio'&&currentStatus)currentStatus.textContent=t(revoked?'signin':reason==='fence'?'connectionLost':'liveEnded')})};pending.addEventListener('room-closed',()=>terminate('room'));pending.addEventListener('session-revoked',()=>terminate('revoked'));pending.addEventListener('host-fence-lost',()=>terminate('fence'))}else pending.close()});studioBroadcastConnectTask=connectTask;source=await connectTask;if(studioBroadcastConnectTask===connectTask)studioBroadcastConnectTask=Promise.resolve();if(!studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream)){source.close();return}studioLiveSource=source;studioBroadcastHostAcquired=true;const session={epoch,hostPeerId,broadcastStream,source};
    await sendStudioHostSignal(liveId,'host-ready',hostPeerId,null,{transport:'p2p',peerLimit:STUDIO_P2P_PEER_LIMIT},session);if(!studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream))return;
    studioBroadcastPending=false;syncStudioOutputLock();
    source.addEventListener('signal',async e=>{if(!studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream))return;const s=JSON.parse(e.data);if(s.kind==='viewer-ready'){await createStudioViewerPeer(liveId,s.fromPeerId,session)}else if(s.kind==='viewer-left'){const entry=studioPeers.get(s.fromPeerId);if(entry?.epoch===epoch){closeStudioViewerPeer(s.fromPeerId,entry);updateStudioPeerStatus()}}else if(s.toPeerId===hostPeerId&&s.kind==='answer'){const entry=studioPeers.get(s.fromPeerId);if(entry?.epoch===epoch&&!entry.pc.remoteDescription){await entry.pc.setRemoteDescription(s.data);clearTimeout(entry.negotiationTimer);for(const c of entry.pending.splice(0))try{await entry.pc.addIceCandidate(c)}catch{}}}else if(s.toPeerId===hostPeerId&&s.kind==='ice'){const entry=studioPeers.get(s.fromPeerId);if(entry?.epoch===epoch){if(entry.pc.remoteDescription)try{await entry.pc.addIceCandidate(s.data)}catch{}else if(entry.pending.length<64&&Date.now()-entry.createdAt<30_000)entry.pending.push(s.data)}}});
    if(status)status.innerHTML=`<span class="recording">LIVE WEBRTC</span> · ${profile.width}×${profile.height} @ ${profile.fps} · 0/${STUDIO_P2P_PEER_LIMIT} ${esc(t('peersLabel'))}`
  }catch(error){source?.close();if(studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream)){stopStudioBroadcast();if(!['LIVE_SIGNAL_ROOM_CLOSED','LIVE_SIGNAL_SESSION_REVOKED','LIVE_SIGNAL_HOST_FENCE_LOST','LIVE_HOST_FENCE_LOST'].includes(error.message))toast(`${t('liveSignalingError')}: ${humanError(error.message)}`)}}
}
async function createStudioViewerPeer(liveId,viewerPeerId,session){
  const {epoch,hostPeerId,broadcastStream}=session;if(!studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream))return;let existing=studioPeers.get(viewerPeerId);if(existing?.epoch===epoch){if(existing.pc.localDescription&&Date.now()-(existing.lastOfferAt||0)>=4_000){existing.lastOfferAt=Date.now();await sendStudioHostSignal(liveId,'offer',hostPeerId,viewerPeerId,existing.pc.localDescription,session)}return}
  const rtc=await liveRtcConfig();if(!studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream))return;existing=studioPeers.get(viewerPeerId);if(existing?.epoch===epoch){if(existing.pc.localDescription&&Date.now()-(existing.lastOfferAt||0)>=4_000){existing.lastOfferAt=Date.now();await sendStudioHostSignal(liveId,'offer',hostPeerId,viewerPeerId,existing.pc.localDescription,session)}return}if(studioPeers.size>=STUDIO_P2P_PEER_LIMIT){await sendStudioHostSignal(liveId,'viewer-rejected',hostPeerId,viewerPeerId,{reason:'P2P_PEER_LIMIT'},session);return}
  const pc=new RTCPeerConnection({iceServers:rtc.iceServers}),entry={pc,pending:[],epoch,createdAt:Date.now(),lastOfferAt:0,negotiationTimer:null,disconnectTimer:null};studioPeers.set(viewerPeerId,entry);entry.negotiationTimer=setTimeout(()=>{if(studioPeers.get(viewerPeerId)===entry&&!pc.remoteDescription){closeStudioViewerPeer(viewerPeerId,entry);if(studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream))updateStudioPeerStatus()}},30_000);for(const track of broadcastStream.getTracks())pc.addTrack(track,broadcastStream);pc.onicecandidate=e=>{if(e.candidate&&studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream))sendStudioHostSignal(liveId,'ice',hostPeerId,viewerPeerId,e.candidate.toJSON(),session).catch(()=>{})};pc.onconnectionstatechange=()=>{if(pc.connectionState==='connected'){clearTimeout(entry.disconnectTimer);entry.disconnectTimer=null}else if(pc.connectionState==='disconnected'&&!entry.disconnectTimer)entry.disconnectTimer=setTimeout(()=>{if(pc.connectionState==='disconnected'&&studioPeers.get(viewerPeerId)===entry)closeStudioViewerPeer(viewerPeerId,entry)},8_000);else if(['failed','closed'].includes(pc.connectionState)&&studioPeers.get(viewerPeerId)===entry)closeStudioViewerPeer(viewerPeerId,entry);if(studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream))updateStudioPeerStatus()};
  try{const offer=await pc.createOffer();if(!studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream)||studioPeers.get(viewerPeerId)!==entry)throw new Error('STALE_STUDIO_BROADCAST');await pc.setLocalDescription(offer);if(!studioBroadcastIsCurrent(epoch,hostPeerId,broadcastStream)||studioPeers.get(viewerPeerId)!==entry)throw new Error('STALE_STUDIO_BROADCAST');entry.lastOfferAt=Date.now();await sendStudioHostSignal(liveId,'offer',hostPeerId,viewerPeerId,pc.localDescription,session);updateStudioPeerStatus()}catch(error){if(studioPeers.get(viewerPeerId)===entry)studioPeers.delete(viewerPeerId);pc.close();if(error.message!=='STALE_STUDIO_BROADCAST')throw error}
}
function closeStudioViewerPeer(viewerPeerId,entry=studioPeers.get(viewerPeerId)){if(!entry)return;clearTimeout(entry.negotiationTimer);clearTimeout(entry.disconnectTimer);if(studioPeers.get(viewerPeerId)===entry)studioPeers.delete(viewerPeerId);entry.pc.close()}
...
function stopStudioBroadcast(){if(studioBroadcastStopping)return studioBroadcastCleanup;const leavingLiveId=studioBroadcastLiveId,leavingHostPeerId=studioHostPeerId,leavingSource=studioLiveSource,leavingStreamToken=leavingSource?.streamToken,releaseAuthorized=studioBroadcastHostAcquired&&!leavingSource?.hostFenceLost&&validLiveHostStreamToken(leavingStreamToken),pendingConnect=studioBroadcastConnectTask,cleanupEpoch=++studioBroadcastEpoch;studioBroadcastPending=false;studioBroadcastHostAcquired=false;studioBroadcastStopping=Boolean(leavingLiveId&&leavingHostPeerId);studioLiveSource=null;if(!releaseAuthorized)leavingSource?.close();for(const [viewerPeerId,entry] of studioPeers)closeStudioViewerPeer(viewerPeerId,entry);studioBroadcastStream?.getVideoTracks().forEach(track=>track.stop());studioBroadcastStream=null;studioBroadcastLiveId=null;studioHostPeerId=null;const start=document.querySelector('#broadcastBtn'),stop=document.querySelector('#stopBroadcast'),status=document.querySelector('#broadcastStatus');if(start)start.disabled=true;if(stop)stop.disabled=true;if(status)status.textContent=t('broadcastStopped');syncStudioOutputLock();const release=Promise.resolve(pendingConnect).catch(()=>null).then(()=>releaseAuthorized?releaseStudioHost(leavingLiveId,leavingHostPeerId,leavingStreamToken):true);studioBroadcastCleanup=release.finally(()=>{leavingSource?.close();studioBroadcastConnectTask=Promise.resolve();if(cleanupEpoch!==studioBroadcastEpoch)return;studioBroadcastStopping=false;syncStudioOutputLock()});return studioBroadcastCleanup}
...
async function acquireStudioSource(kind,profile,isCurrent=()=>true){
  const audioConstraints={echoCancellation:true,noiseSuppression:true,autoGainControl:true};
  if(kind==='camera'){const stream=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:profile.width},height:{ideal:profile.height},frameRate:{ideal:profile.fps,max:profile.fps}},audio:audioConstraints});return{stream,microphoneUnavailable:false}}
  const display=await navigator.mediaDevices.getDisplayMedia({video:{frameRate:{ideal:profile.fps,max:profile.fps}},audio:true});studioPendingSourceStreams.add(display);if(!isCurrent()){display.getTracks().forEach(track=>track.stop());studioPendingSourceStreams.delete(display);throw new DOMException('Stale Studio source request','AbortError')}let microphone=null,microphoneUnavailable=false;
  try{microphone=await navigator.mediaDevices.getUserMedia({audio:audioConstraints});studioPendingSourceStreams.add(microphone)}catch{microphoneUnavailable=true}
  const stream=new MediaStream([...display.getVideoTracks(),...display.getAudioTracks(),...(microphone?.getAudioTracks()||[])]);studioPendingSourceStreams.delete(display);if(microphone)studioPendingSourceStreams.delete(microphone);return{stream,microphoneUnavailable}
}
async function startStudioSource(kind){
  if(studioMediaBusy())return toast(t('sourceChangeBlocked'));if(studioSourceRequestPending)return;
  const requestId=++studioSourceRequestId,profile=currentStudioProfile(),video=document.querySelector('#studioSource'),previous=studioSourceStream,requestCurrent=()=>requestId===studioSourceRequestId&&state.view==='studio'&&video&&document.body.contains(video);let candidate=null,committed=false;
  studioSourceRequestPending=true;syncStudioOutputLock();
  try{
    const acquired=await acquireStudioSource(kind,profile,requestCurrent);candidate=acquired.stream;studioPendingSourceStreams.add(candidate);if(!requestCurrent()||studioMediaBusy()){candidate.getTracks().forEach(track=>track.stop());studioPendingSourceStreams.delete(candidate);candidate=null;return}const videoTrack=candidate.getVideoTracks()[0];if(!videoTrack||videoTrack.readyState!=='live')throw new Error('VIDEO_TRACK_REQUIRED');
    video.srcObject=candidate;try{await video.play()}catch(error){if(requestCurrent())video.srcObject=previous;throw error}if(!requestCurrent()||studioMediaBusy()||videoTrack.readyState!=='live'){candidate.getTracks().forEach(track=>track.stop());studioPendingSourceStreams.delete(candidate);candidate=null;return}
    studioSourceStream=candidate;studioSourceKind=kind;studioPendingSourceStreams.delete(candidate);committed=true;videoTrack.addEventListener('ended',()=>{if(studioSourceStream?.getVideoTracks()[0]===videoTrack)stopStudioTracks()},{once:true});stopStudioAudio();previous?.getTracks().forEach(track=>track.stop());
    await setupStudioAudio(acquired.microphoneUnavailable?'microphoneUnavailableScreen':'',requestId,candidate);if(!requestCurrent()||studioSourceStream!==candidate||videoTrack.readyState!=='live')return;if(acquired.microphoneUnavailable)toast(t('microphoneUnavailableScreen'));updateStudioProfileAvailability(videoTrack);
    const actual=videoTrack.getSettings?.()||{},detail=`${actual.width||profile.width}×${actual.height||profile.height} · ${actual.frameRate||profile.fps} FPS`;document.querySelector('#recordStatus').textContent=`${t(kind==='screen'?'screenShare':'cameraMic')} · ${detail}`;setStudioSourceState('ready','sourceReady',detail);composeStudioFrame();
  }catch(error){studioPendingSourceStreams.delete(candidate);if(!committed)candidate?.getTracks().forEach(track=>track.stop());if(requestId!==studioSourceRequestId||state.view!=='studio'||!video||!document.body.contains(video))return;if(video&&previous&&!committed)video.srcObject=previous;setStudioSourceState(studioSourceStream?'ready':'error','sourceOpenFailed',error.name||error.message);toast(`${u('sourceOpenFailed')}: ${error.name||error.message}`);if(!studioSourceStream){const status=document.querySelector('#recordStatus');if(status)status.textContent=`${u('sourceOpenFailed')}: ${error.name||error.message}`}}
  finally{if(requestId===studioSourceRequestId){studioSourceRequestPending=false;syncStudioOutputLock()}}
}
async function setupStudioAudio(statusKey='',requestId=studioSourceRequestId,stream=studioSourceStream){
  stopStudioAudio();const tracks=stream?.getAudioTracks()||[],gainInput=document.querySelector('#studioMicGain'),mute=document.querySelector('#studioMicMute'),status=document.querySelector('#studioAudioStatus'),isCurrent=()=>requestId===studioSourceRequestId&&stream===studioSourceStream&&state.view==='studio'&&status&&document.body.contains(status);if(!tracks.length){if(isCurrent())status.textContent=t(statusKey||'sourceNoAudio');return}const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio){if(isCurrent())status.textContent=t(statusKey||'webAudioUnavailable');return}
  const context=new Audio();let sources=[];try{await context.resume();if(!isCurrent()){await context.close().catch(()=>{});return}const gain=context.createGain(),analyser=context.createAnalyser(),destination=context.createMediaStreamDestination();analyser.fftSize=512;sources=tracks.map(track=>context.createMediaStreamSource(new MediaStream([track])));for(const source of sources)source.connect(gain);gain.connect(analyser).connect(destination);if(!isCurrent()){for(const source of sources)try{source.disconnect()}catch{}await context.close().catch(()=>{});return}studioAudioContext=context;studioAudioGain=gain;studioAudioAnalyser=analyser;studioAudioDestination=destination;studioAudioSources=sources;if(gainInput)gainInput.disabled=false;if(mute)mute.disabled=false;status.textContent=t(statusKey||'audioProcessingActive');updateStudioAudioGain();startStudioAudioMeter()}catch{for(const source of sources)try{source.disconnect()}catch{}await context.close().catch(()=>{});if(isCurrent())status.textContent=t(statusKey||'webAudioUnavailable')}
}
...
function toggleStudioAudioMute(){const button=document.querySelector('#studioMicMute');if(!button)return;const muted=button.dataset.muted!=='1';button.dataset.muted=muted?'1':'0';button.setAttribute('aria-pressed',String(muted));button.textContent=t(muted?'unmute':'mute');updateStudioAudioGain()}
...
function stopStudioAudio(){cancelAnimationFrame(studioAudioMeterRaf);studioAudioMeterRaf=0;for(const source of studioAudioSources)try{source.disconnect()}catch{}studioAudioSources=[];studioAudioContext?.close?.().catch(()=>{});studioAudioContext=null;studioAudioGain=null;studioAudioAnalyser=null;studioAudioDestination=null;const input=document.querySelector('#studioMicGain'),mute=document.querySelector('#studioMicMute'),meter=document.querySelector('#studioAudioMeter');if(input)input.disabled=true;if(mute)mute.disabled=true;if(meter)meter.style.width='0%'}
...
function requestStopStudioSource(){if(studioMediaBusy())return toast(t('sourceChangeBlocked'));stopStudioTracks()}
function stopStudioTracks(){studioSourceRequestId++;studioSourceRequestPending=false;for(const stream of studioPendingSourceStreams)stream.getTracks().forEach(track=>track.stop());studioPendingSourceStreams.clear();const broadcastCleanup=stopStudioBroadcast();cancelAnimationFrame(studioRaf);studioRaf=0;if(studioRecorder?.state==='recording')stopStudioRecording();stopStudioAudio();studioSourceStream?.getTracks().forEach(track=>track.stop());studioSourceStream=null;studioSourceKind=null;const video=document.querySelector('#studioSource');if(video)video.srcObject=null;if(document.querySelector('#studioCanvas'))drawStudioPlaceholder();const status=document.querySelector('#recordStatus');if(status)status.textContent=t('sourceOff');setStudioSourceState('idle','sourceOff');syncStudioOutputLock();return broadcastCleanup}
function startStudioRecording(){
  if(!studioSourceStream)return toast(t('enableCameraScreen'));if(studioSourceRequestPending)return toast(t('sourceChangeBlocked'));if(studioMediaBusy())return;const canvas=document.querySelector('#studioCanvas'),profile=currentStudioProfile();let composed=null,recorder=null;
  try{
    if(typeof MediaRecorder==='undefined'||typeof canvas?.captureStream!=='function')throw new Error('MEDIA_RECORDER_UNAVAILABLE');composed=canvas.captureStream(profile.fps);for(const track of studioOutputAudioTracks())composed.addTrack(track);const candidates=['video/webm;codecs=vp9,opus','video/webm;codecs=vp8,opus','video/webm','video/mp4'],mime=candidates.find(type=>MediaRecorder.isTypeSupported?.(type))||'',pixels=profile.width*profile.height*profile.fps,bitrate=Math.max(4_000_000,Math.min(12_000_000,Math.round(pixels*.07))),chunks=[],options={videoBitsPerSecond:bitrate,...(mime?{mimeType:mime}:{})};recorder=new MediaRecorder(composed,options);studioRecordingStream=composed;studioRecorder=recorder;studioRecordingFinalizing=false;studioLastBlob=null;let failed=false;
    recorder.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};
    recorder.onerror=event=>{failed=true;const status=document.querySelector('#recordStatus');if(studioRecorder===recorder&&status)status.textContent=`${t('error')}: ${humanError(event.error?.message||'MEDIA_RECORDER_FAILED')}`;if(recorder.state==='recording')try{recorder.stop()}catch{}};
    recorder.onstop=()=>{composed.getVideoTracks().forEach(track=>track.stop());if(studioRecordingStream===composed)studioRecordingStream=null;if(studioRecorder!==recorder)return;studioRecorder=null;studioRecordingFinalizing=false;const start=document.querySelector('#recordBtn'),stop=document.querySelector('#stopRecord');if(start)start.disabled=false;if(stop)stop.disabled=true;if(failed){syncStudioOutputLock();return}const type=(recorder.mimeType||mime||'video/webm').split(';')[0],blob=new Blob(chunks,{type}),extension=type==='video/mp4'?'mp4':'webm';studioLastBlob=blob;const url=URL.createObjectURL(blob),result=document.querySelector('#recordResult');if(result){result.innerHTML=`<a class="primary download-recording" href="${url}" download="sylora-studio-${Date.now()}.${extension}">${esc(u('downloadRecording'))}</a><button id="publishRecording" class="ghost">${esc(u('publishAsClip'))}</button>`;result.querySelector('#publishRecording').onclick=publishStudioRecording}const status=document.querySelector('#recordStatus');if(status)status.textContent=`${u('recordingReady')} · ${(blob.size/1024/1024).toFixed(1)} MB · ${profile.width}×${profile.height} @ ${profile.fps}`;syncStudioOutputLock()};
    recorder.start(1000);const start=document.querySelector('#recordBtn'),stop=document.querySelector('#stopRecord'),status=document.querySelector('#recordStatus');if(start)start.disabled=true;if(stop)stop.disabled=false;if(status)status.innerHTML=`<span class="recording">REC</span> · ${profile.width}×${profile.height} @ ${profile.fps}`;syncStudioOutputLock()
  }catch(error){composed?.getVideoTracks().forEach(track=>track.stop());if(studioRecordingStream===composed)studioRecordingStream=null;if(studioRecorder===recorder)studioRecorder=null;studioRecordingFinalizing=false;syncStudioOutputLock();toast(`${t('error')}: ${humanError(error.message)}`)}
}
function stopStudioRecording(){if(studioRecorder?.state!=='recording')return;studioRecordingFinalizing=true;const start=document.querySelector('#recordBtn'),stop=document.querySelector('#stopRecord');if(start)start.disabled=true;if(stop)stop.disabled=true;try{studioRecorder.stop()}catch(error){studioRecordingFinalizing=false;toast(`${t('error')}: ${humanError(error.message)}`)}syncStudioOutputLock()}
async function publishStudioRecording(){if(!studioLastBlob)return;const button=document.querySelector('#publishRecording');if(button){button.disabled=true;button.textContent=t('publishing')}try{const contentType=studioLastBlob.type.split(';')[0];if(!['video/webm','video/mp4'].includes(contentType))throw new Error('UNSUPPORTED_MEDIA_TYPE');const response=await fetch('/api/media/upload',{method:'POST',headers:{authorization:`Bearer ${state.token}`,'content-type':contentType},body:studioLastBlob});const uploaded=await response.json();if(!response.ok)throw new Error(uploaded.error||'UPLOAD_FAILED');const title=document.querySelector('#overlayTitle')?.value||'SYLORA Studio';await api('/api/videos',{method:'POST',body:JSON.stringify({mediaId:uploaded.media.id,title,format:'clip'})});toast(u('recordingPublished'));nav('clips')}catch(e){toast(humanError(e.message));if(button){button.disabled=false;button.textContent=u('tryAgain')}}}
...
async function renderIdentity(){
  ...
  const fieldKeys={profile:'identityFieldProfile',professional:'identityFieldProfessional',portfolio:'identityFieldPortfolio',skills:'identityFieldSkills',interests:'identityFieldInterests',reputation:'identityFieldReputation',agent:'identityFieldAgent',assets:'identityFieldAssets'};
  const privacyKeys={public:'privacyPublic',followers:'privacyFollowers',connections:'privacyConnections',business:'privacyBusiness',private:'privacyPrivate',ai_only:'privacyAiOnly'};
  app.innerHTML=`<div class="card hero"><span class="eyebrow">SYLORA IDENTITY</span><h1>${esc(identity.displayName||state.me.displayName)}</h1><p>${u('identityHeroCopy')}</p></div>
  <form id="identityForm" class="card fields"><span class="eyebrow">${u('professionalUpper')}</span>
  <input name="title" aria-label="${esc(u('jobTitle'))}" placeholder="${esc(u('jobTitle'))}" value="${esc(identity.professional?.title||'')}">
  <input name="company" aria-label="${esc(u('company'))}" placeholder="${esc(u('company'))}" value="${esc(identity.professional?.company||'')}">
  <input name="skills" aria-label="${esc(u('skillsCommaSeparated'))}" placeholder="${esc(u('skillsCommaSeparated'))}" value="${esc((identity.professional?.skills||[]).join(', '))}">
  <input name="interests" aria-label="${esc(u('interestsCommaSeparated'))}" placeholder="${esc(u('interestsCommaSeparated'))}" value="${esc((identity.interests||[]).join(', '))}">
  <input name="headline" aria-label="${esc(u('creatorHeadline'))}" placeholder="${esc(u('creatorHeadline'))}" value="${esc(identity.creatorPersona?.headline||'')}">
  <span class="eyebrow">${u('fieldPrivacyUpper')}</span>
  ${['profile','professional','portfolio','skills','interests','reputation','agent','assets'].map(k=>`<label>${u(fieldKeys[k])}<select name="privacy_${k}">${['public','followers','connections','business','private','ai_only'].map(v=>`<option value="${v}" ${p[k]===v?'selected':''}>${u(privacyKeys[v])}</option>`).join('')}</select></label>`).join('')}
  <button class="primary">${u('saveIdentity')}</button></form>
  <section class="card"><span class="eyebrow">KNOWLEDGE GRAPH</span><h3>${u('knowledgePermissionTitle')}</h3>
  <form id="kgForm" class="fields"><input name="label" maxlength="120" aria-label="${esc(u('favoriteLanguageExample'))}" placeholder="${esc(u('favoriteLanguageExample'))}" required><input name="value" maxlength="1000" aria-label="${esc(u('value'))}" placeholder="${esc(u('value'))}" required>
  <select name="privacy" aria-label="${u('privacy')}"><option value="private">${u('privacyPrivate')}</option><option value="ai_only">${u('privacyAiOnly')}</option><option value="connections">${u('privacyConnections')}</option><option value="public">${u('privacyPublic')}</option></select>
  <button class="ghost">${u('addNode')}</button></form>
  <div class="stack">${nodes.slice(-20).reverse().map(n=>`<div class="item row"><div><b>${esc(n.label||n.type||u('node'))}</b><p class="muted">${esc(n.data?.value||n.data?.summary||n.type||'')} · ${u(privacyKeys[n.privacy]||'privacyPrivate')}</p></div><button class="ghost kg-del" data-id="${esc(n.id)}">${u('delete')}</button></div>`).join('')||`<p class="muted">${u('knowledgeGraphEmpty')}</p>`}</div></section>`;
  document.querySelector('#identityForm').onsubmit=async e=>{
    ...
    toast(u('identityUpdated'));renderIdentity();
  };
  document.querySelector('#kgForm')?.addEventListener('submit',async e=>{
    ...
    toast(u('nodeAdded'));renderIdentity();
  });
  document.querySelectorAll('.kg-del').forEach(b=>b.onclick=async()=>{await api(`/api/kg/nodes/${b.dataset.id}`,{method:'DELETE'});toast(u('nodeDeleted'));renderIdentity()});
}
async function renderAgents(){
  ...
  <div class="stack">${agents.map(a=>`<div class="card item"><span class="eyebrow">${esc(a.category)} · ${esc(a.pricing?.model==='free'?u('free'):a.pricing?.model||u('free'))}</span><h3>${esc(a.name)}</h3><p data-user-content>${esc(a.summary)}</p><small>${u('permissions')}: ${(a.permissions||[]).map(esc).join(', ')||'—'}</small><div class="row">${installed.has(a.id)?`<button class="ghost uninstall-agent" data-id="${esc(a.id)}">${u('uninstall')}</button><button class="ghost start-negotiate" data-id="${esc(a.id)}">${u('aiProposal')}</button>`:`<button class="primary install-agent" data-id="${esc(a.id)}">${u('install')}</button>`}</div></div>`).join('')||`<div class="card empty">${u('noAgents')}</div>`}</div>`;
...
function conferenceHubHtml(kind,rooms){const science=kind==='science';return`<section class="conference-hub"><div class="conference-head"><div><span class="eyebrow">${esc(t(science?'scienceCirclesBrand':'businessRoomsBrand'))}</span><h2>${esc(t(science?'closedScienceConferences':'privateBusinessConferences'))}</h2><p>${esc(t(science?'scienceConferenceIntro':'businessConferenceIntro'))}</p></div><span class="conference-seal">${science?'⌬':'◈'}</span></div><form class="conference-create fields" data-conference-create="${kind}"><div class="inline-fields"><input name="title" required minlength="2" maxlength="120" placeholder="${esc(t('conferenceTitle'))}"><input name="description" maxlength="800" placeholder="${esc(t('shortGoal'))}"></div><button class="primary">＋ ${esc(t('createClosedRoom'))}</button></form><div class="conference-grid">${rooms.map(room=>`<article class="conference-room"><div class="conference-room-top"><span class="conference-room-icon">${science?'⌬':'◈'}</span><span class="badge">🔒 PRIVATE · ${room.memberCount}</span></div><h3>${esc(room.title)}</h3><p>${esc(room.description||t('privateConference'))}</p>${room.inviteId&&!room.role?`<button class="primary conference-accept" data-invite="${esc(room.inviteId)}">${esc(t('acceptInvitation'))}</button>`:`<div class="conference-actions"><button class="ghost conference-open" data-room="${esc(room.id)}" data-kind="${kind}">${esc(t('enter'))}</button>${room.role==='owner'?`<button class="ghost conference-sylora" data-room="${esc(room.id)}" data-enabled="${room.syloraEnabled?'1':'0'}">✦ Sylora ${esc(t(room.syloraEnabled?'syloraEnabled':'syloraDisabledLabel'))}</button>`:''}</div>${room.role==='owner'?`<form class="conference-invite" data-room="${esc(room.id)}"><input name="username" placeholder="@username" required><button class="ghost">${esc(t('invite'))}</button></form>`:''}`}</article>`).join('')||`<div class="conference-empty">${esc(t('privateRoomsEmpty'))}</div>`}</div></section>`}
function bindConferenceHub(kind,refresh){document.querySelector(`[data-conference-create="${kind}"]`)?.addEventListener('submit',async e=>{e.preventDefault();const input=Object.fromEntries(new FormData(e.currentTarget));try{await api('/api/conferences',{method:'POST',body:JSON.stringify({...input,kind})});toast(t('conferenceCreated'));refresh()}catch(error){toast(humanError(error.message))}});document.querySelectorAll('.conference-invite').forEach(form=>form.addEventListener('submit',async e=>{e.preventDefault();try{await api(`/api/conferences/${form.dataset.room}/invite`,{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(form)))});form.reset();toast(t('invitationSent'))}catch(error){toast(humanError(error.message))}}));document.querySelectorAll('.conference-accept').forEach(button=>button.onclick=async()=>{try{await api(`/api/conference-invites/${button.dataset.invite}/accept`,{method:'POST',body:'{}'});toast(t('joinedClosedGroup'));refresh()}catch(error){toast(humanError(error.message))}});document.querySelectorAll('.conference-sylora').forEach(button=>button.onclick=async()=>{try{const enabled=button.dataset.enabled!=='1';await api(`/api/conferences/${button.dataset.room}/sylora`,{method:'PATCH',body:JSON.stringify({enabled})});toast(t(enabled?'syloraJoinedRoom':'syloraDisabled'));refresh()}catch(error){toast(humanError(error.message))}});document.querySelectorAll('.conference-open').forEach(button=>button.onclick=()=>openConferenceRoomRtc(button.dataset.room,kind,refresh))}
...
function localizeConferenceRoom({room,participants=[],rtc,kind}={}){
  const back=document.querySelector('#conferenceBack');if(back)back.textContent=`← ${t('back')}`;const count=document.querySelector('.conference-live-state b');if(count)count.textContent=`${participants.length} ${t('participants')}`;const network=document.querySelector('#conferenceNetwork');if(network)network.textContent=t('connecting').toUpperCase();document.querySelectorAll('.conference-peer-state').forEach((status,index)=>{status.textContent=participants[index]?.id===state.me.id?t('you').toUpperCase():t('waitingLabel')});
  const panel=document.querySelector('#conferenceSyloraPanel');if(panel){const detail=panel.querySelector(':scope>div:first-child small'),intro=panel.querySelector('#conferenceSyloraMessages>p'),input=panel.querySelector('input');if(detail)detail.textContent=`${kind==='science'?'Science copilot':'Business copilot'} · ${t('onlyOnRequest')}`;if(intro)intro.textContent=t('joinWhenAsked');if(input)input.placeholder=t('askSyloraInput')}
  const camera=document.querySelector('#conferenceCamera span'),microphone=document.querySelector('#conferenceMic span'),leave=document.querySelector('#conferenceLeave span');if(camera)camera.textContent=t('camera');if(microphone)microphone.textContent=t('microphone');if(leave)leave.textContent=t('leave');const note=document.querySelector('.conference-note');if(note)note.textContent=`${t('encryptedWebrtc')} ${t(rtc?.turnConfigured?'turnFallbackReady':'turnNotConfigured')} ${t(room?.syloraEnabled?'syloraAllowedOwner':'syloraDisabledOwner')}`;
}
...
async function openConferenceRoomRtc(roomId,kind,back){
  ...
  localizeConferenceRoom({room,participants,rtc,kind});
...
async function renderLearningReference(){
  ...
  const researchers=(users.users||[]).slice(0,6),courseCards=courses.map((course,index)=>`<article class="glass-card course-card"><span class="nav-icon-plate">${referenceIcon(index%2?'sparkles':'learning')}</span><span class="status-pill ${course.price?'status-pill--gold':'status-pill--success'}">${course.price?`◈ ${course.price}`:u('freeUpper')}</span><h3>${esc(course.title)}</h3><p>${esc(course.description||u('syloraCourse'))} · ${course.lessonCount} ${u('lessons')}</p><div class="progress-track"><i style="width:${course.lessonCount?'100':'4'}%"></i></div><button class="open-course" type="button" data-id="${course.id}">${u('openCourse')} ${referenceIcon('arrow')}</button></article>`).join('');
  app.innerHTML=`<div class="learning-layout">
    <aside class="learning-hero"><span class="learning-orbit" aria-hidden="true">${referenceIcon('learning')}</span><div><span class="status-pill status-pill--success">${referenceIcon('learning')} SYLORA LEARNING</span><h1>${u('growAtYourPace')}</h1><p>${u('learningHeroCopy')}</p><div class="course-progress"><span><b>${courses.length} ${u('courses')}</b><small>${u('availableNow')}</small></span><div><i style="width:${courses.length?'100':'5'}%"></i></div><b>${conferenceData.rooms.length}</b></div></div></aside>
    <main class="learning-main content-stack">
      <section class="route-hero-row compact learning-heading"><div><span class="status-pill status-pill--violet">SCIENCE · RESEARCH</span><h1>${u('learningComesAlive')}</h1><p>${u('learningAiTransparency')}</p></div>${state.me?`<button type="button" class="depth-button" id="startTutor">✦ ${u('startTutor')}</button>`:`<button type="button" class="depth-button" id="learningLogin">${u('signIn')}</button>`}</section>
      <section><div class="section-title"><div><small>${u('catalogUpper')}</small><h2>${u('continueLearning')}</h2></div></div><div class="course-grid">${courseCards||`<div class="glass-card empty">${u('noPublishedCourses')}</div>`}</div></section>
      ${state.me?`<section class="glass-card study-live"><div><span class="status-pill status-pill--success">FOCUS MODE</span><h3>${u('studyWorkspace')}</h3><p>${esc((learnHub.sections||[]).slice(0,8).join(' · ')||'Tutor · Flashcards · Exam plan · Focus')}</p></div><span class="focus-clock">25:00</span><button type="button" id="focusStudy">${u('startFocus')}</button></section>
      <section class="learning-tools-grid"><article class="glass-card learning-tool-card"><span class="nav-icon-plate">${referenceIcon('sparkles')}</span><div><small>AI SUPPORT</small><h3>${u('preparation')}</h3><p>${u('flashcardsExamPlan')}</p></div><button type="button" id="makeDeck">Flashcards</button><button type="button" id="examPlan">${u('examPlan')}</button></article><article class="glass-card learning-tool-card"><span class="nav-icon-plate">${referenceIcon('chart')}</span><div><small>SCIENCE WORKSPACE</small><h3>${u('research')}</h3><p>${esc((sciHub.sections||[]).slice(0,5).join(' · ')||'Library · Dataset · Experiment')}</p></div><button type="button" id="addPaper">${u('paper')}</button><button type="button" id="newDataset">Dataset</button><button type="button" id="newExperiment">${u('experiment')}</button><button type="button" id="runCalc">${u('kineticEnergyCalc')}</button><button type="button" id="runStats">${u('statistics')}</button><button type="button" id="newFormula">${u('formula')}</button><button type="button" id="newBoard">${u('whiteboard')}</button><button type="button" id="newCircle">${u('scienceCircle')}</button></article></section>`:''}
      <section class="glass-card researchers-card"><div class="card-top"><div><small>${u('researchersUpper')}</small><h3>${u('peopleNearby')}</h3></div></div><div class="researcher-row">${researchers.map((user,index)=>`<span><i class="avatar convo-${index%5+1}">${esc((user.displayName||user.username).slice(0,1).toUpperCase())}</i><b>${esc(user.displayName||user.username)}</b><small>@${esc(user.username)}</small></span>`).join('')||`<p class="muted">${u('noResearchers')}</p>`}</div></section>
      ${state.me?conferenceHubHtml('science',conferenceData.rooms):''}
      ${state.me?`<form id="courseForm" class="glass-card course-create fields"><div class="card-top"><div><small>CREATOR LEARNING</small><h3>${u('newCourse')}</h3></div></div><input name="title" placeholder="${u('courseTitle')}" required><textarea name="description" placeholder="${u('description')}"></textarea><input name="price" type="number" min="0" value="0" aria-label="${u('coursePrice')}"><button class="depth-button">${esc(t('createCourse'))}</button></form>`:''}
    </main>
  </div>`;
  ...
  document.querySelector('#startTutor')?.addEventListener('click',async()=>{const out=await api('/api/learning/tutor',{method:'POST',body:JSON.stringify({subject:'General',mode:'teach_me'})});toast(`${u('tutor')} · ${out.session.mode} · ${u('noHiddenAnswers')}`)});
  document.querySelector('#makeDeck')?.addEventListener('click',async()=>{await api('/api/learning/flashcards',{method:'POST',body:JSON.stringify({title:'Quick deck',cards:[{front:'Concept',back:'Definition'}],aiAssisted:false})});toast(u('deckCreated'))});
  document.querySelector('#examPlan')?.addEventListener('click',async()=>{const date=new Date(Date.now()+7*864e5).toISOString().slice(0,10);await api('/api/learning/exam-plan',{method:'POST',body:JSON.stringify({subject:'Exam',examDate:date,availableMinutesPerDay:45})});toast(u('planReady'))});
  document.querySelector('#focusStudy')?.addEventListener('click',async()=>{await api('/api/focus',{method:'POST',body:JSON.stringify({preset:'25_5'})});toast(u('focusStarted'))});
  document.querySelector('#addPaper')?.addEventListener('click',async()=>{await api('/api/science/library',{method:'POST',body:JSON.stringify({type:'paper',title:'Untitled paper'})});toast(u('paperAdded'))});
  document.querySelector('#newDataset')?.addEventListener('click',async()=>{await api('/api/science/datasets',{method:'POST',body:JSON.stringify({name:'Dataset',columns:[{name:'x',type:'number'}],previewRows:[]})});toast(u('datasetCreated'))});
  document.querySelector('#newBoard')?.addEventListener('click',async()=>{await api('/api/whiteboard',{method:'POST',body:JSON.stringify({space:'learning',title:'Study board'})});toast(u('whiteboardCreated'))});
  ...
  document.querySelector('#runStats')?.addEventListener('click',async()=>{const {analysis}=await api('/api/science/statistics',{method:'POST',body:JSON.stringify({data:[2,4,4,4,5,5,7,9]})});toast(analysis.explanation.slice(0,140))});
  document.querySelector('#newCircle')?.addEventListener('click',async()=>{await api('/api/science/circles',{method:'POST',body:JSON.stringify({title:'Paper discussion'})});toast(u('circleCreated'))});
  document.querySelector('#newFormula')?.addEventListener('click',async()=>{await api('/api/science/formulas',{method:'POST',body:JSON.stringify({title:'Energy',latex:'E=mc^2',units:['J']})});toast(u('formulaCreated'))});
...
async function openCourse(id){
  try{
    ...
    app.innerHTML=`<button id="backLearning" class="ghost">← ${u('courses')}</button><div class="card hero"><span class="eyebrow">SYLORA LEARNING</span><h1>${esc(course.title)}</h1><p>${esc(course.description)}</p>${enrollment?`<p class="muted">${u('progress')}: ${progress}%</p><div class="progress"><span style="width:${progress}%"></span></div>`:''}${locked&&state.me&&course.price===0?`<button id="enrollCourse" class="primary">${u('enrollFree')}</button>`:''}${locked&&!state.me?`<p class="muted">${u('signInForLessons')}</p>`:''}${locked&&course.price>0?`<p class="muted">${u('paidCoursesLater')}</p>`:''}</div><div class="stack">${lessons.map(l=>`<article class="card item"><span class="badge">${u('lesson')} ${l.position}${l.completed?` · ✓ ${u('completed')}`:''}</span><h3>${esc(l.title)}</h3>${locked?`<p class="muted">🔒 ${u('enrollToUnlock')}</p>`:`<p>${esc(l.content||'')}</p>${!l.completed?`<button class="ghost complete-lesson" data-id="${l.id}">${u('markCompleted')}</button>`:''}<button class="ghost lesson-quiz" data-id="${l.id}">${u('quiz')}</button>`}</article>`).join('')}</div><div id="lessonQuiz"></div>`;
    document.querySelector('#backLearning').onclick=renderLearningReference;
    document.querySelector('#enrollCourse')?.addEventListener('click',async()=>{await api(`/api/courses/${id}/enroll`,{method:'POST',body:'{}'});toast(u('enrolledCourse'));await openCourse(id)});
    ...
    document.querySelectorAll('.lesson-quiz').forEach(b=>b.onclick=async()=>{
      ...
      quizBox.querySelectorAll('.quiz-opt').forEach(opt=>opt.onclick=async()=>{
        ...
        toast(grade.correct?u('correct'):u('tryAnotherExplanation'));
        quizBox.insertAdjacentHTML('beforeend',`<p class="muted">${esc(grade.explanation)} · ${u('next')}: ${esc(grade.adaptive?.nextStep||'')}</p>`);
      });
    });
  }catch(e){toast(humanError(e));renderLearningReference()}
}
...
function localizeMeetingAssistant(panel){if(!panel)return;const title=panel.querySelector('#meetTitle'),notes=panel.querySelector('#meetNotes'),brief=panel.querySelector('#meetBrief'),summary=panel.querySelector('#meetSummary');if(title)title.placeholder=t('meetingTitle');if(notes)notes.placeholder=t('meetingNotes');if(brief)brief.textContent=t('meetingBrief');if(summary)summary.textContent=t('summaryDecisions')}
...
async function renderBusinessReference(){
  ...
  const businessSectionKeys={dashboard:'businessSectionDashboard',companies:'businessSectionCompanies',clients:'businessSectionClients',projects:'businessSectionProjects',tasks:'businessSectionTasks',calendar:'businessSectionCalendar',meetings:'businessSectionMeetings',documents:'businessSectionDocuments',finance:'businessSectionFinance',invoices:'businessSectionInvoices'};
  hub.sections=(hub.sections||[]).map(section=>u(businessSectionKeys[section]||section));
  ...
  app.innerHTML=`<div class="business-layout">
    <main class="business-overview">
      <section class="route-hero-row compact business-heading"><div><span class="status-pill status-pill--gold">${referenceIcon('business')} SYLORA BUSINESS</span><h1>${u('businessRhythm')}</h1><p>${u('businessSafetyCopy')}</p></div>${state.me?`<button type="button" class="depth-button" id="bizInvoice">＋ ${u('draftInvoice')}</button>`:`<button type="button" class="depth-button" id="businessLogin">${u('signIn')}</button>`}</section>
      <section class="metric-grid">
        <article class="glass-card metric-card"><span>${referenceIcon('business')}</span><small>${u('organizationsUpper')}</small><b>${orgs.length}</b><em>${u('workspaces')}</em></article>
        <article class="glass-card metric-card"><span>${referenceIcon('chart')}</span><small>${u('paidUpper')}</small><b>${paidGross.toLocaleString()}</b><em>${esc(currency)} · ${u('byInvoices')}</em></article>
        <article class="glass-card metric-card"><span>${referenceIcon('activity')}</span><small>${u('unpaidUpper')}</small><b>${unpaid.length}</b><em>${u('needAttention')}</em></article>
        <article class="glass-card metric-card"><span>${referenceIcon('users')}</span><small>${u('companiesUpper')}</small><b>${businesses.length}</b><em>${u('inCatalog')}</em></article>
      </section>
      <section class="glass-card revenue-card"><div class="card-top"><div><small>${u('invoicesRealData')}</small><h3>${u('latestAmounts')}</h3></div><button type="button" id="bizFinanceAsk">${referenceIcon('sparkles')} ${u('askSylora')}</button></div>${chartBars?`<div class="revenue-chart"><div class="chart-y"><span>${maxValue.toLocaleString()}</span><span>${Math.round(maxValue/2).toLocaleString()}</span><span>0</span></div><div class="chart-bars">${chartBars}</div></div><div class="chart-footer"><span><i></i> ${u('grossAmount')}</span><b>${invoiceList.length} ${u('invoices')}</b></div>`:`<div class="business-empty-chart"><span class="runtime-core">◈</span><b>${u('noInvoices')}</b><p>${u('createDraftChart')}</p></div>`}</section>
      ${state.me?`<section class="glass-card business-actions-card"><div class="card-top"><div><small>BUSINESS HUB · ${esc(country.profile?.countryCode||'DEFAULT')}</small><h3>${u('quickActions')}</h3></div><span class="status-pill status-pill--success">${u('notABank')}</span></div><p class="muted">${esc((hub.sections||[]).slice(0,10).join(' · '))}</p><div class="business-action-grid"><button type="button" id="bizCrm">${referenceIcon('users')} ${u('addClient')}</button><button type="button" id="bizQuote">${referenceIcon('wallet')} ${u('quote')}</button><button type="button" id="bizTime">${referenceIcon('activity')} ${u('workTimer')}</button></div><form id="countryForm" class="country-form"><label>${u('countryProfile')}<select name="countryCode"><option value="PL">PL</option><option value="UA">UA</option><option value="DE">DE</option><option value="US">US</option><option value="DEFAULT">DEFAULT</option></select></label><button class="plain-action">${u('save')} ${referenceIcon('arrow')}</button></form></section>
      <section class="glass-card org-reference-card"><div class="card-top"><div><small>BUSINESS OS</small><h3>${u('organizations')}</h3></div></div><form id="orgForm" class="org-create"><input name="name" required placeholder="${u('organizationName')}"><button class="depth-button">${esc(t('createHub'))}</button></form><div class="org-reference-list">${orgs.map(org=>`<article><span class="business-emblem">◈</span><div><b>${esc(org.name)}</b><small>${u('teamsDocumentsTasks')}</small></div><div><button type="button" class="open-org-workspace" data-id="${org.id}">${esc(t('workspace'))}</button><button type="button" class="add-org-team" data-id="${org.id}">+ ${esc(t('teams'))}</button><button type="button" class="add-org-doc" data-id="${org.id}">+ ${esc(t('documents'))}</button><button type="button" class="add-org-task" data-id="${org.id}">+ ${esc(t('tasks'))}</button></div></article>`).join('')||`<p class="muted">${u('createFirstOrganization')}</p>`}</div><div id="orgWorkspacePanel" class="org-workspace-reference"></div></section>
      ${conferenceHubHtml('business',conferenceData.rooms)}
      <form id="businessForm" class="glass-card business-company-form fields"><div class="card-top"><div><small>${u('companyProfileUpper')}</small><h3>${u('addCompany')}</h3></div></div><input name="name" placeholder="${u('company')}" required><textarea name="description" placeholder="${u('description')}"></textarea><input name="website" placeholder="https://"><button class="depth-button">${u('saveProfile')}</button></form>`:''}
      <section class="business-company-grid">${businesses.map(company=>`<article class="glass-card business-company"><span class="business-emblem">◈</span><div><small>${u('companyUpper')}</small><h3>${esc(company.name)}</h3><p>${esc(company.description)}</p>${company.website?`<span class="status-pill">${esc(company.website)}</span>`:''}</div></article>`).join('')||`<div class="glass-card empty">${u('noCompanies')}</div>`}</section>
    </main>
    <aside class="context-stack">
      <section class="glass-card"><div class="card-top"><div><small>${u('systemStatusUpper')}</small><h3>${u('businessControl')}</h3></div></div><div class="task-list"><label><input type="checkbox" checked disabled><span><b>${u('financialConfirmation')}</b><small>${u('requiredBeforeSending')}</small></span>${referenceIcon('shield')}</label><label><input type="checkbox" checked disabled><span><b>${u('countryProfile')}</b><small>${u('manualNotIp')}</small></span>${referenceIcon('check')}</label><label><input type="checkbox" ${state.me?'checked':''} disabled><span><b>${u('privateRooms')}</b><small>${conferenceData.rooms.length} ${u('available')}</small></span>${referenceIcon('camera')}</label></div></section>
      <section class="glass-card sylora-business">${referenceIcon('sparkles')}<div><small>SYLORA BUSINESS</small><b>${u('helpNoHiddenActions')}</b><p>${u('businessAiCopy')}</p><button type="button" data-business-ai>${u('openSylora')}</button></div></section>
    </aside>
  </div>`;
  localizeBusinessPage({orgs,businesses,paidGross,unpaid,invoiceList,currency,conferenceRooms:conferenceData.rooms});
  const meetingPanel=document.querySelector('#orgWorkspacePanel');if(meetingPanel){const observer=new MutationObserver(()=>{if(meetingPanel.querySelector('#meetTitle')){localizeMeetingAssistant(meetingPanel);observer.disconnect()}});observer.observe(meetingPanel,{childList:true,subtree:true})}
  ...
  document.querySelector('#bizInvoice')?.addEventListener('click',async()=>{await api('/api/business/invoices',{method:'POST',body:JSON.stringify({items:[{description:'Service',quantity:1,unitNetPrice:100,taxRate:23}],seller:{name:'Me'},buyer:{name:'Client'}})});toast(u('invoiceDraftCreated'));await renderBusinessReference()});
  document.querySelector('#bizCrm')?.addEventListener('click',async()=>{const name=prompt(u('clientName'));if(!name)return;await api('/api/business/crm',{method:'POST',body:JSON.stringify({type:'client',name})});toast(u('crmRecordCreated'))});
  document.querySelector('#bizQuote')?.addEventListener('click',async()=>{await api('/api/business/quotes',{method:'POST',body:JSON.stringify({items:[{description:'Estimate',quantity:1,unitNetPrice:50,taxRate:0}]})});toast(u('quoteDraftCreated'))});
  document.querySelector('#bizTime')?.addEventListener('click',async()=>{await api('/api/business/time',{method:'POST',body:JSON.stringify({action:'start',note:'Work'})});toast(u('workTimerStarted'))});
  document.querySelector('#bizFinanceAsk')?.addEventListener('click',async()=>{if(!state.me)return renderAuth();const out=await api('/api/business/finance/ask',{method:'POST',body:JSON.stringify({query:'неоплачені фактури'})});toast(`${u('unpaid')}: ${out.answer?.unpaidCount??0} · ${u('confirmationRequired')}`)});
  ...
  document.querySelector('#countryForm')?.addEventListener('submit',async event=>{event.preventDefault();await api('/api/business/country',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(event.currentTarget)))});toast(u('countryProfileSaved'));await renderBusinessReference()});
  document.querySelector('#orgForm')?.addEventListener('submit',async event=>{event.preventDefault();const name=new FormData(event.currentTarget).get('name');await api('/api/orgs',{method:'POST',body:JSON.stringify({name})});toast(u('organizationCreated'));await renderBusinessReference()});
  const showWorkspace=async id=>{const out=await api(`/api/orgs/${id}/workspace`),panel=document.querySelector('#orgWorkspacePanel');if(!panel)return;panel.innerHTML=`<div class="org-workspace-grid"><section><small>${esc(t('teams'))}</small>${(out.teams||[]).map(item=>`<p><b>${esc(item.name)}</b></p>`).join('')||'<p class="muted">—</p>'}</section><section><small>${esc(t('documents'))}</small>${(out.documents||[]).map(item=>`<p><b>${esc(item.title)}</b></p>`).join('')||'<p class="muted">—</p>'}</section><section><small>${esc(t('tasks'))}</small>${(out.tasks||[]).map(item=>`<p><b>${esc(item.title)}</b></p>`).join('')||'<p class="muted">—</p>'}</section></div><div class="meeting-assistant fields"><input id="meetTitle" placeholder="${u('meetingTitle')}"><textarea id="meetNotes" placeholder="${u('notesTranscript')}"></textarea><div><button type="button" id="meetBrief">${u('meetingBrief')}</button><button type="button" id="meetSummary">${u('summaryDecisions')}</button></div><pre id="meetOut" hidden></pre></div>`;document.querySelector('#meetBrief').onclick=async()=>{const result=await api(`/api/orgs/${id}/meeting-brief`,{method:'POST',body:JSON.stringify({title:document.querySelector('#meetTitle').value||u('brief'),agenda:document.querySelector('#meetNotes').value||''})}),pre=document.querySelector('#meetOut');pre.hidden=false;pre.textContent=JSON.stringify(result.brief,null,2)};document.querySelector('#meetSummary').onclick=async()=>{const result=await api(`/api/orgs/${id}/meeting-summary`,{method:'POST',body:JSON.stringify({title:document.querySelector('#meetTitle').value||u('summary'),notes:document.querySelector('#meetNotes').value||''})}),pre=document.querySelector('#meetOut');pre.hidden=false;pre.textContent=JSON.stringify({summary:result.summary,proposedTasks:result.proposedTasks},null,2)}};
  document.querySelectorAll('.open-org-workspace').forEach(button=>button.onclick=()=>showWorkspace(button.dataset.id));
  document.querySelectorAll('.add-org-team').forEach(button=>button.onclick=async()=>{const name=prompt(t('teams'));if(!name)return;await api(`/api/orgs/${button.dataset.id}/teams`,{method:'POST',body:JSON.stringify({name})});toast(u('teamAdded'));showWorkspace(button.dataset.id)});
  document.querySelectorAll('.add-org-doc').forEach(button=>button.onclick=async()=>{const title=prompt(t('documents'));if(!title)return;await api(`/api/orgs/${button.dataset.id}/documents`,{method:'POST',body:JSON.stringify({title})});toast(u('documentAdded'));showWorkspace(button.dataset.id)});
  document.querySelectorAll('.add-org-task').forEach(button=>button.onclick=async()=>{const title=prompt(t('tasks'));if(!title)return;await api(`/api/orgs/${button.dataset.id}/tasks`,{method:'POST',body:JSON.stringify({title})});toast(u('taskAdded'));showWorkspace(button.dataset.id)});
