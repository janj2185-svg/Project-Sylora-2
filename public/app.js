import { t, setLocale, getLocale, humanError } from './i18n.js?v=20260829-live5';
...
function handleUserEvent(event,data){
  ...
  if(event==='message'){toast(`@${data.actor?.username||'user'} · new message`);if(state.view==='messages')renderMessagesReference()}
}
...
async function renderFeed(){
  ...
  const sparkline=heights=>`<div class="sparkline" aria-label="Графік динаміки">${heights.map((height,index)=>`<i style="height:${height}%;animation-delay:${index*45}ms"></i>`).join('')}</div>`;
...
async function renderClips(){
  ...
  const cards=videos.map((v,index)=>{
    ...
    const duration=v.media.duration?`${v.media.duration.toFixed(1)}s`:'тривалість уточнюється';
    return `<article class="clip-card ${layout}" data-clip-card="${index}">
      ...
      <div class="clip-copy"><span><small>@${esc(v.author.username)} · ${pipeline}</small><b>${esc(v.title)}</b><small>${v.media.width||'?'}×${v.media.height||'?'} · ${duration}${v.stream?.progress?` · ${v.stream.progress}%`:''}</small></span><button type="button" data-clip-focus="${index}">${referenceIcon('play')} Дивитися</button></div>
    </article>`;
  }).join('');
  app.innerHTML=`<section class="route-hero-row compact clips-route-head"><div><span class="status-pill status-pill--violet">${referenceIcon('clip')} SYLORA CLIPS</span><h1>Момент у русі.</h1><p>Миттєвий вертикальний плеєр із підготовкою наступного відео — у власній візуальній мові SYLORA.</p></div><div class="hero-actions">${state.me?'<button id="openUpload" class="depth-button" type="button">＋ Завантажити Clip</button>':'<button id="clipLogin" class="depth-button" type="button">Увійти для публікації</button>'}</div></section>
  <section class="media-runtime" aria-label="Стан медіаплеєра"><span class="runtime-core">${referenceIcon('play')}</span><span><small>INSTANT MEDIA CORE</small><b>${videos.length} clips · ${ready} готові</b></span><span class="preload-meter"><span><b>Наступне відео</b><small>${videos.length>1?'передзавантажується':'очікує'}</small></span><i><em style="width:${videos.length>1?'86':'18'}%"></em></i></span><span class="status-pill status-pill--success">${ready===videos.length&&videos.length?'READY':'ADAPTIVE'}</span></section>
  <div id="uploader"></div><section class="clips-grid">${cards||'<div class="glass-card clips-empty"><span class="runtime-core">＋</span><div><small>CLIPS</small><b>Тут з’явиться перший живий момент.</b><p>Завантаж вертикальне MP4 або WebM — SYLORA підготує його до швидкого старту.</p></div></div>'}</section>`;
...
async function renderExploreReference(){
  ...
  <form id="search" class="explore-search"><span>${referenceIcon('search')}</span><input name="q" minlength="2" placeholder="${esc(u('exploreSearchPlaceholder'))}" autocomplete="off" autofocus><button type="submit">${referenceIcon('sparkles')} ${esc(u('find'))}</button></form>
...
async function renderLive(){
  ...
  const roomCards=visibleRooms.map(r=>{const opponent=rooms.find(x=>x.id!==r.id&&x.host?.id!==r.host?.id);return`<article class="live-room-card"><div><span class="status-pill status-pill--live">● LIVE · ${r.viewerCount||0}</span><h3>${esc(r.title)}</h3><p>@${esc(r.host.username)}</p></div><div class="live-tools"><button class="depth-button watch-live" data-id="${r.id}">${esc(u('watch'))}</button><button class="ghost open-live" data-id="${r.id}">${esc(u('chat'))}</button>${state.me?`<button class="ghost ask-live" data-id="${r.id}">Sylora</button>`:''}${state.me?.id===r.host?.id&&opponent?`<button class="ghost resonance-start" data-id="${r.id}" data-opponent="${opponent.id}">✦ Battle</button>`:''}${state.me?.id===r.host?.id?`<button class="ghost live-copilot" data-id="${r.id}">Copilot</button>`:''}</div></article>`}).join('');
  app.innerHTML=`<section class="route-hero-row compact live-route-head"><div><span class="status-pill status-pill--live">${referenceIcon('live')} SYLORA LIVE</span><h1>${esc(u('liveSpaceTitle'))}</h1><p>${esc(u('liveSpaceCopy'))}</p></div><div class="live-tabs filter-row"><button type="button" data-live-tab="discover" class="${tab==='discover'?'active':''}">${esc(t('discoverLive'))}</button><button type="button" data-live-tab="following" class="${tab==='following'?'active':''}">${esc(t('following'))}</button><button type="button" data-live-tab="create" class="${tab==='create'?'active':''}">${esc(t('createLive'))}</button><button type="button" data-live-tab="battles" class="${tab==='battles'?'active':''}">${esc(t('battles'))}</button><button type="button" data-live-tab="studio">${esc(u('studio'))}</button></div></section>
  ...
  document.querySelector('#goLive')?.addEventListener('click',async()=>{const title=document.querySelector('#liveTitle')?.value||u('syloraCreator');await api('/api/live',{method:'POST',body:JSON.stringify({title})});state.liveTab='discover';renderLive()});
  document.querySelector('#createEventBtn')?.addEventListener('click',async()=>{const title=document.querySelector('#eventTitle')?.value||u('syloraEvent');const startsAt=document.querySelector('#eventWhen')?.value||u('toBeAnnounced');await api('/api/platform-events',{method:'POST',body:JSON.stringify({title,startsAt,mode:'online'})});toast(u('eventCreated'));state.liveTab='discover';renderLive()});
...
async function openLiveChat(id){if(liveEventSource)liveEventSource.close();const r=await api(`/api/live/${id}/chat`);const box=document.querySelector('#liveChat');box.innerHTML=`<div class="card auth"><div class="row"><h3>${esc(u('liveChat'))}</h3><span class="badge" id="viewerCount">● ${esc(u('realtime'))}</span></div><div id="liveMessages">${r.messages.map(liveMessageHtml).join('')||`<p class="muted empty-chat">${esc(u('quiet'))}</p>`}</div>${state.me?`<form id="liveChatForm" class="fields"><input name="text" maxlength="500" placeholder="${esc(u('writeLive'))}"><button class="primary">${esc(u('send'))}</button></form>`:''}</div>`;liveEventSource=new EventSource(`/api/live/${id}/events`);liveEventSource.addEventListener('chat',e=>{const msg=JSON.parse(e.data),messages=document.querySelector('#liveMessages');messages?.querySelector('.empty-chat')?.remove();messages?.insertAdjacentHTML('beforeend',liveMessageHtml(msg))});liveEventSource.addEventListener('viewers',e=>{const v=JSON.parse(e.data);const el=document.querySelector('#viewerCount');if(el)el.textContent=`● ${v.count} ${u('connected')}`});document.querySelector('#liveChatForm')?.addEventListener('submit',async e=>{e.preventDefault();const field=e.currentTarget.elements.text,text=field.value.trim();if(!text)return;field.value='';await api(`/api/live/${id}/chat`,{method:'POST',body:JSON.stringify({text})})})}
...
async function renderStudio(){
  ...
  document.querySelector('#app>.hero')?.insertAdjacentHTML('beforeend',`<div class="scene-readout studio-readout"><span><small>${esc(u('scenes').toUpperCase())}</small><b>${scenes.length}</b></span><span><small>${esc(u('myLive'))}</small><b>${ownRooms.length}</b></span><span><small>OUTPUT</small><b>4 ${esc(u('profiles'))}</b></span></div>`);
...
function distributionDestinationMarkup(item,selected){return`<div class="distribution-destination" data-distribution-row="${item.id}"><label><input type="checkbox" data-distribution-destination value="${item.id}" ${item.enabled&&selected.has(item.id)?'checked':''} ${item.enabled?'':'disabled'}><span><b>${esc(item.label)}</b><small>${esc(item.providerLabel)} · ${esc(item.ingestHost)} · key ${esc(item.keyFingerprint)}</small></span></label><button type="button" class="ghost" data-distribution-toggle="${item.id}" data-enabled="${item.enabled?'1':'0'}">${esc(item.enabled?u('pause'):u('enable'))}</button><button type="button" class="ghost danger" data-distribution-delete="${item.id}" aria-label="${esc(u('delete'))} ${esc(item.label)}">×</button></div>`}
...
function obsErrorText(error){const labels={OBS_LOCALHOST_ONLY:u('obsLocalOnly'),OBS_URL_PROTOCOL:u('obsProtocol'),OBS_PASSWORD_REQUIRED:u('obsPasswordRequired'),OBS_CONNECTION_FAILED:u('obsConnectionFailed'),OBS_CONNECT_TIMEOUT:u('obsTimeout'),COMPANION_LOCALHOST_ONLY:u('companionLocalOnly'),COMPANION_URL_PROTOCOL:u('companionProtocol'),PAIRING_TOKEN_REQUIRED:u('pairingTokenRequired'),PAIRING_REQUIRED:u('pairingRequired'),ORIGIN_NOT_ALLOWED:u('companionOriginBlocked')};return labels[error?.message]||error?.message||u('connectionError')}
...
function restoreStudioRecovery(){try{const saved=JSON.parse(localStorage.getItem('sylora_studio_recovery')||'null');if(!saved)return;if(STUDIO_PROFILES[saved.profileId])document.querySelector('#studioProfile').value=saved.profileId;if(typeof saved.overlayTitle==='string')document.querySelector('#overlayTitle').value=saved.overlayTitle;if(['violet','cyan','clean'].includes(saved.overlayStyle))document.querySelector('#overlayStyle').value=saved.overlayStyle;const gain=Math.max(0,Math.min(150,Number(saved.micGain)));if(Number.isFinite(gain))document.querySelector('#studioMicGain').value=String(gain);const mute=document.querySelector('#studioMicMute');if(mute){mute.dataset.muted=saved.micMuted?'1':'0';mute.textContent=saved.micMuted?u('unmute'):u('mute')}}catch{}}
function loadStudioScene(scenes){const id=document.querySelector('#savedScene').value,scene=scenes.find(x=>x.id===id);if(!scene)return;document.querySelector('#sceneName').value=scene.name;document.querySelector('#overlayTitle').value=scene.overlayTitle;document.querySelector('#overlayStyle').value=scene.overlayStyle;if(STUDIO_PROFILES[scene.profileId])document.querySelector('#studioProfile').value=scene.profileId;const gain=document.querySelector('#studioMicGain'),mute=document.querySelector('#studioMicMute');if(gain)gain.value=String(Math.max(0,Math.min(150,Number(scene.micGain??100))));if(mute){mute.dataset.muted=scene.micMuted?'1':'0';mute.textContent=scene.micMuted?u('unmute'):u('mute')}localStorage.setItem('sylora_studio_profile',document.querySelector('#studioProfile').value);persistStudioRecovery();configureStudioCanvas();updateStudioAudioGain();if(studioSourceStream)composeStudioFrame();else drawStudioPlaceholder()}
async function saveStudioScene(){const id=document.querySelector('#savedScene').value,name=document.querySelector('#sceneName').value,preset=studioPresetState();try{await api(id?`/api/studio/scenes/${id}`:'/api/studio/scenes',{method:id?'PATCH':'POST',body:JSON.stringify({name,...preset})});persistStudioRecovery();toast(id?u('sceneUpdated'):u('sceneSaved'));renderStudio()}catch(e){toast(e.message)}}
async function deleteStudioScene(){const id=document.querySelector('#savedScene').value;if(!id)return toast(u('chooseSavedScene'));await api(`/api/studio/scenes/${id}`,{method:'DELETE'});toast(u('sceneDeleted'));renderStudio()}
...
function drawStudioPlaceholder(){const canvas=document.querySelector('#studioCanvas');if(!canvas)return;const ctx=canvas.getContext('2d'),dw=canvas.width,dh=canvas.height,g=ctx.createLinearGradient(0,0,dw,dh),unit=Math.min(dw/720,dh/1280);g.addColorStop(0,'#171a33');g.addColorStop(.5,'#312766');g.addColorStop(1,'#113f54');ctx.fillStyle=g;ctx.fillRect(0,0,dw,dh);ctx.fillStyle='#ffffff';ctx.font=`700 ${Math.max(26,42*unit)}px system-ui`;ctx.textAlign='center';ctx.fillText('SYLORA STUDIO',dw/2,dh*.48);ctx.font=`${Math.max(16,24*unit)}px system-ui`;ctx.fillStyle='#cfd3ff';ctx.fillText(u('chooseCameraOrScreen'),dw/2,dh*.52)}
async function startStudioSource(kind){stopStudioTracks();try{const profile=currentStudioProfile(),videoConstraints={width:{ideal:profile.width},height:{ideal:profile.height},frameRate:{ideal:profile.fps,max:profile.fps}};studioSourceStream=kind==='screen'?await navigator.mediaDevices.getDisplayMedia({video:{frameRate:{ideal:profile.fps,max:profile.fps}},audio:true}):await navigator.mediaDevices.getUserMedia({video:videoConstraints,audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});const video=document.querySelector('#studioSource');video.srcObject=studioSourceStream;await video.play();await setupStudioAudio();studioSourceStream.getVideoTracks()[0]?.addEventListener('ended',stopStudioTracks,{once:true});const actual=studioSourceStream.getVideoTracks()[0]?.getSettings?.()||{};document.querySelector('#recordStatus').textContent=`${kind==='screen'?u('screenShare'):u('cameraMic')} · ${actual.width||profile.width}×${actual.height||profile.height} · ${actual.frameRate||profile.fps} FPS`;composeStudioFrame()}catch(err){document.querySelector('#recordStatus').textContent=`${u('sourceOpenFailed')}: ${err.name||err.message}`}}
async function setupStudioAudio(){stopStudioAudio();const tracks=studioSourceStream?.getAudioTracks()||[],gainInput=document.querySelector('#studioMicGain'),mute=document.querySelector('#studioMicMute'),status=document.querySelector('#studioAudioStatus');if(!tracks.length){if(status)status.textContent=u('sourceHasNoAudio');return}const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio){if(status)status.textContent=u('webAudioUnavailable');return}studioAudioContext=new Audio();await studioAudioContext.resume();const source=studioAudioContext.createMediaStreamSource(new MediaStream(tracks));studioAudioGain=studioAudioContext.createGain();studioAudioAnalyser=studioAudioContext.createAnalyser();studioAudioDestination=studioAudioContext.createMediaStreamDestination();studioAudioAnalyser.fftSize=512;source.connect(studioAudioGain).connect(studioAudioAnalyser).connect(studioAudioDestination);if(gainInput)gainInput.disabled=false;if(mute)mute.disabled=false;if(status)status.textContent=u('audioProcessingActive');updateStudioAudioGain();startStudioAudioMeter()}
...
function toggleStudioAudioMute(){const button=document.querySelector('#studioMicMute');if(!button)return;const muted=button.dataset.muted!=='1';button.dataset.muted=muted?'1':'0';button.textContent=muted?u('unmute'):u('mute');updateStudioAudioGain()}
...
async function renderIdentity(){
  ...
  <input name="title" placeholder="${u('jobTitle')}" value="${esc(identity.professional?.title||'')}">
  <input name="company" placeholder="${u('company')}" value="${esc(identity.professional?.company||'')}">
  <input name="skills" placeholder="${u('skillsCommaSeparated')}" value="${esc((identity.professional?.skills||[]).join(', '))}">
  <input name="interests" placeholder="${u('interestsCommaSeparated')}" value="${esc((identity.interests||[]).join(', '))}">
  <input name="headline" placeholder="${u('creatorHeadline')}" value="${esc(identity.creatorPersona?.headline||'')}">
  ...
  <form id="kgForm" class="fields"><input name="label" maxlength="120" placeholder="${u('favoriteLanguageExample')}" required><input name="value" maxlength="1000" placeholder="${u('value')}" required>
...
async function renderDeveloper(){
  ...
  app.innerHTML=`<div class="card hero"><span class="eyebrow">DEVELOPER PLATFORM</span><h1>Будуй на SYLORA.</h1><p>Sandbox додатки, hashed API-ключі, scopes. OAuth/OIDC підготовлено архітектурно.</p></div>
  <form id="devAppForm" class="card fields"><span class="eyebrow">НОВИЙ APP</span><input name="name" required maxlength="80" placeholder="Назва додатку"><input name="scopes" value="identity.read" placeholder="scopes через кому"><button class="primary">Створити</button></form>
  <div class="card"><span class="eyebrow">OAUTH</span><pre style="white-space:pre-wrap;font-size:12px">${esc(JSON.stringify(oauth,null,2))}</pre></div>
  <div class="stack" id="devApps">${apps.map(a=>`<div class="card item"><b>${esc(a.name)}</b><p class="muted">${esc((a.scopes||[]).join(', '))}</p><button class="ghost create-key" data-id="${a.id}">Створити API key</button></div>`).join('')||'<p class="muted">Ще немає додатків.</p>'}</div>
  <pre id="devKeyOut" class="card" hidden></pre>`;
  document.querySelector('#devAppForm').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);await api('/api/developer/apps',{method:'POST',body:JSON.stringify({name:fd.get('name'),scopes:String(fd.get('scopes')||'').split(',').map(x=>x.trim()).filter(Boolean)})});toast('App створено');renderDeveloper()};
...
async function renderAI(speakText=''){
  ...
  if(voicePicker){const languagePicker=document.createElement('label');languagePicker.className='voice-hint voice-language-picker';languagePicker.textContent='Мова голосу ';const select=document.createElement('select');select.id='aiVoiceLocale';select.innerHTML=`<option value="">Як інтерфейс</option>${Object.entries({uk:'Українська',en:'English',pl:'Polski',de:'Deutsch',ru:'Русский',es:'Español',fr:'Français',it:'Italiano',pt:'Português'}).map(([code,label])=>`<option value="${code}" ${code===syloraVoiceLocale?'selected':''}>${label}</option>`).join('')}`;languagePicker.append(select);voicePicker.after(languagePicker)}
  ...
  const hero=document.querySelector('.sylora-ai-hero');hero?.classList.toggle('sylora-visual-hidden',localStorage.getItem('sylora_visual_hidden')==='1');document.querySelector('#aiVisualToggle')?.addEventListener('click',event=>{const hidden=!hero.classList.contains('sylora-visual-hidden');hero.classList.toggle('sylora-visual-hidden',hidden);localStorage.setItem('sylora_visual_hidden',hidden?'1':'0');event.currentTarget.textContent=hidden?'Показати Sylora':'Сховати Sylora'});
  document.querySelector('#aiPermLive')?.addEventListener('click',async()=>{await api('/api/ai/permissions',{method:'PATCH',body:JSON.stringify({permissions:{live_assist:!(dashboard.access||{}).live_assist}})});toast('Дозволи оновлено');renderAI()});
...
api('/api/ai/intelligence').then(intel=>{const sel=document.querySelector('#aiProactive');if(!sel)return;sel.value=intel.proactive||intel.agent?.proactiveLevel||'IMPORTANT_ONLY';sel.onchange=async()=>{await api('/api/ai/proactive',{method:'PATCH',body:JSON.stringify({level:sel.value})});toast('OK')}}).catch(()=>{});
  document.querySelector('#aiVoiceSelect')?.addEventListener('change',event=>{syloraVoiceId=event.target.value;localStorage.setItem('sylora_voice_id',syloraVoiceId);toast(syloraVoiceId?'Голос Sylora змінено':'Автоматичний голос увімкнено')});
  document.querySelector('#aiVoiceLocale')?.addEventListener('change',event=>{syloraVoiceLocale=event.target.value;localStorage.setItem('sylora_voice_locale',syloraVoiceLocale);syloraVoiceId='';localStorage.removeItem('sylora_voice_id');toast(`Мова голосу: ${event.target.selectedOptions[0]?.textContent||'як інтерфейс'}`);renderAI()});
...
async function renderCommunitiesReference(){
  ...
  document.querySelector('#funCoffee')?.addEventListener('click',async()=>{if(!state.me)return renderAuth();await api('/api/social/fun-rooms',{method:'POST',body:JSON.stringify({kind:'coffee',title:'Coffee Room'})});toast('Coffee Room створено')});
  ...
  document.querySelector('#communityWorkshop')?.addEventListener('click',async()=>{if(!state.me)return renderAuth();await api('/api/social/community-events',{method:'POST',body:JSON.stringify({kind:'workshop',title:'Community Workshop'})});toast('Workshop створено')});
  document.querySelector('#discoveryOptIn')?.addEventListener('click',async()=>{await api('/api/social/discovery',{method:'POST',body:JSON.stringify({optIn:true,languages:['uk','en'],interests:['science','music'],topics:['learning']})});const matches=await api('/api/social/discovery/matches');toast(`Discovery увімкнено · збігів ${matches.matches?.length||0}`)});
  document.querySelectorAll('.join-community').forEach(button=>button.onclick=async()=>{try{await api(`/api/communities/${button.dataset.id}/join`,{method:'POST'});toast('Ви у спільноті');await openCommunity(button.dataset.id)}catch(error){toast(error.message)}});
...
async function openCommunity(id){
  ...
  }catch(e){toast(e.message);renderCommunitiesReference()}
}
