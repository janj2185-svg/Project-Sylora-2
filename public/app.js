...
async function renderLearningReference(){
  ...
  document.querySelector('#startTutor')?.addEventListener('click',async()=>{const out=await api('/api/learning/tutor',{method:'POST',body:JSON.stringify({subject:'General',mode:'teach_me'})});toast(`Tutor · ${out.session.mode} · без прихованих відповідей`)});
  document.querySelector('#makeDeck')?.addEventListener('click',async()=>{await api('/api/learning/flashcards',{method:'POST',body:JSON.stringify({title:'Quick deck',cards:[{front:'Concept',back:'Definition'}],aiAssisted:false})});toast('Колоду створено')});
  document.querySelector('#examPlan')?.addEventListener('click',async()=>{const date=new Date(Date.now()+7*864e5).toISOString().slice(0,10);await api('/api/learning/exam-plan',{method:'POST',body:JSON.stringify({subject:'Exam',examDate:date,availableMinutesPerDay:45})});toast('План готовий')});
  document.querySelector('#focusStudy')?.addEventListener('click',async()=>{await api('/api/focus',{method:'POST',body:JSON.stringify({preset:'25_5'})});toast('Focus 25/5 запущено')});
  document.querySelector('#addPaper')?.addEventListener('click',async()=>{await api('/api/science/library',{method:'POST',body:JSON.stringify({type:'paper',title:'Untitled paper'})});toast('Paper додано')});
  document.querySelector('#newDataset')?.addEventListener('click',async()=>{await api('/api/science/datasets',{method:'POST',body:JSON.stringify({name:'Dataset',columns:[{name:'x',type:'number'}],previewRows:[]})});toast('Dataset workspace створено')});
  document.querySelector('#newBoard')?.addEventListener('click',async()=>{await api('/api/whiteboard',{method:'POST',body:JSON.stringify({space:'learning',title:'Study board'})});toast('Whiteboard створено')});
  ...
  document.querySelector('#runStats')?.addEventListener('click',async()=>{const {analysis}=await api('/api/science/statistics',{method:'POST',body:JSON.stringify({data:[2,4,4,4,5,5,7,9]})});toast(analysis.explanation.slice(0,140))});
  document.querySelector('#newCircle')?.addEventListener('click',async()=>{await api('/api/science/circles',{method:'POST',body:JSON.stringify({title:'Paper discussion'})});toast('Science Circle створено')});
  document.querySelector('#newFormula')?.addEventListener('click',async()=>{await api('/api/science/formulas',{method:'POST',body:JSON.stringify({title:'Energy',latex:'E=mc^2',units:['J']})});toast('Formula workspace створено')});
