import { t, setLocale, getLocale, humanError } from './i18n.js?v=20260830-studio1';
...
async function watchLive(id){
  ...
  liveViewerLiveId=id;liveViewerId=crypto.randomUUID();liveViewerPeer=configureLiveViewerPeer(new RTCPeerConnection({iceServers:rtc.iceServers}),id);
...
async function renderStudio(){
  ...
  document.querySelector('#app>.hero')?.insertAdjacentHTML('beforeend',`<div class="scene-readout studio-readout"><span><small>СЦЕНИ</small><b>${scenes.length}</b></span><span><small>МОЇ LIVE</small><b>${ownRooms.length}</b></span><span><small>OUTPUT</small><b>${Object.keys(STUDIO_PROFILES).length} PROFILES</b></span></div>`);
...
function studioHostPeerIdForLive(liveId){const key=`sylora_studio_host_peer:${liveId}`;try{const saved=sessionStorage.getItem(key);if(/^[A-Za-z0-9._:-]{8,80}$/.test(saved||''))return saved;const id=crypto.randomUUID();sessionStorage.setItem(key,id);return id}catch{return crypto.randomUUID()}}
...
async function openCourse(id){
  try{
    ...
    app.innerHTML=`<button id="backLearning" class="ghost">← ${esc(t('backToCourses'))}</button><div class="card hero"><span class="eyebrow">SYLORA LEARNING</span><h1>${esc(course.title)}</h1><p>${esc(course.description)}</p>${enrollment?`<p class="muted">${esc(t('progressLabel'))}: ${progress}%</p><div class="progress"><span style="width:${progress}%"></span></div>`:''}${locked&&state.me&&course.price===0?`<button id="enrollCourse" class="primary">${esc(t('enrollFree'))}</button>`:''}${locked&&!state.me?`<p class="muted">${esc(t('signinToEnroll'))}</p>`:''}${locked&&course.price>0?`<p class="muted">${esc(t('paidCoursesBlocked'))}</p>`:''}</div><div class="stack">${lessons.map(l=>`<article class="card item"><span class="badge">${esc(t('lessonLabel'))} ${l.position}${l.completed?` · ✓ ${esc(t('completedLabel'))}`:''}</span><h3>${esc(l.title)}</h3>${locked?`<p class="muted">🔒 ${esc(t('enrollToUnlock'))}</p>`:`<p>${esc(l.content||'')}</p>${!l.completed?`<button class="ghost complete-lesson" data-id="${l.id}">${esc(t('markCompleted'))}</button>`:''}<button class="ghost lesson-quiz" data-id="${l.id}">Quiz</button>`}</article>`).join('')}</div><div id="lessonQuiz"></div>`;
    document.querySelector('#backLearning').onclick=renderLearningReference;
    document.querySelector('#enrollCourse')?.addEventListener('click',async()=>{await api(`/api/courses/${id}/enroll`,{method:'POST',body:'{}'});toast(t('courseEnrolled'));await openCourse(id)});
    ...
    document.querySelectorAll('.lesson-quiz').forEach(b=>b.onclick=async()=>{
      ...
      quizBox.querySelectorAll('.quiz-opt').forEach(opt=>opt.onclick=async()=>{
        ...
        toast(grade.correct?t('correctAnswer'):t('tryAnotherExplanation'));
        quizBox.insertAdjacentHTML('beforeend',`<p class="muted">${esc(grade.explanation)} · ${esc(t('nextStep'))}: ${esc(grade.adaptive?.nextStep||'')}</p>`);
      });
    });
