...
function localizedDynamicUiMessage(msg){const text=String(msg??''),gift=text.match(/^Подарунок відправлено · Orbit (\d+)$/);if(gift)return`${u('giftSent')} · Orbit ${gift[1]}`;if(text==='Оберіть або створіть LIVE')return u('chooseOrCreateLive');if(text==='Спочатку увімкни камеру або screen share')return u('enableCameraOrScreen');return text}
