...
export class PostgresAuthSocialRepository {
  ...
  async listFollowingIds(userId){const result=await this.pool.query('SELECT following_id FROM follows WHERE follower_id=$1',[userId]);return result.rows.map(r=>r.following_id)}
}
