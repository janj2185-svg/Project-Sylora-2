...
import { hasTurnServer, publicRtcClientConfig, resolveIceConfig } from './rtc-config.mjs';
import { AI_STATUS, assertProductionBoot, loadConfig, safeErrorCode } from './config.mjs';
import { buildHealthPayload, buildReadyPayload } from './readiness.mjs';
...
if (process.env.NODE_ENV !== 'test') assertProductionBoot(config);
...
const openaiModel=config.ai.model;
const openaiRealtimeModel=config.ai.realtimeModel;
const openaiRealtimeVoice=config.ai.realtimeVoice;
const liveIceConfig=resolveIceConfig(process.env);
const liveIceServers=liveIceConfig.iceServers;
const openai=config.ai.configured?new OpenAI({apiKey:process.env.OPENAI_API_KEY,...(config.ai.baseURL?{baseURL:config.ai.baseURL}:{})}):null;
const aiBuckets=new Map();
const postgres=new PostgresService(config.database.url);
...
async function dependencyHealth(){const [pg,cache,outbox]=await Promise.all([postgres.ping(),redis.ping(),outboxRepo.health()]);return {postgres:pg,redis:cache,outbox}}
function aiUnavailableBody(){return {error:'AI_PROVIDER_NOT_CONFIGURED',aiStatus:config.ai.status,reason:config.ai.reason}}
...
async function api(req, res, url) {
  ...
  if(req.method==='GET'&&p==='/api/health'){const dependencies=await dependencyHealth();return json(res,200,buildHealthPayload({config,dependencies}))}
  ...
  if(req.method==='GET'&&p==='/api/ready'){const dependencies=await dependencyHealth();const payload=buildReadyPayload({config,dependencies});return json(res,payload.ready?200:503,payload)}
  ...
  if(req.method==='POST'&&p==='/api/ai/realtime'){
    const user=await requireUser(req,res);if(!user)return;if(!openai)return json(res,503,aiUnavailableBody());if(!await allowAi(user.id))return json(res,429,{error:'AI_RATE_LIMITED'});
  ...
  m=route('/api/conferences/:id/invite',p);if(req.method==='POST'&&m){const user=await requireUser(req,res);if(!user)return;const input=await body(req),username=safeText(input.username,80).replace(/^@/,'').trim();if(!username)return json(res,400,{error:'USERNAME_REQUIRED'});const invite=await inviteConference({id:store.id(),roomId:m.id,ownerId:user.id,username,createdAt:store.now()});if(invite===undefined)return json(res,403,{error:'CONFERENCE_OWNER_REQUIRED'});if(invite===null)return json(res,404,{error:'USER_NOT_FOUND'});return json(res,201,{invite})}
  ...
  m=route('/api/conferences/:id/participants',p);if(req.method==='GET'&&m){const user=await requireUser(req,res);if(!user)return;const participants=await conferenceParticipants(m.id,user.id);if(!participants)return json(res,404,{error:'CONFERENCE_NOT_FOUND'});return json(res,200,{participants})}
  ...
  if(req.method==='GET'&&p==='/api/live/rtc-config'){const user=await requireUser(req,res);if(!user)return;return json(res,200,publicRtcClientConfig(liveIceConfig,{nodeEnv:config.nodeEnv}))}
  ...
  m=route('/api/live/:id/end',p);if(req.method==='POST'&&m){const user=await requireUser(req,res);if(!user)return;const live=await endLiveRoom(m.id,user.id);if(!live)return json(res,404,{error:'LIVE_NOT_FOUND'});livePeerRegistry.clearLocalRoom(live.id);return json(res,200,{live});}
  ...
  if(req.method==='GET'&&p==='/api/ai/history'){const user=await requireUser(req,res);if(!user)return;const [messages,memories,pendingActions]=await Promise.all([aiListMessages(user.id,50),aiListMemories(user.id,50),aiListPendingActions(user.id,20)]);return json(res,200,{messages,memories,pendingActions,model:openaiModel,configured:!!openai,aiStatus:config.ai.status,reason:config.ai.reason,fallback:config.ai.fallback})}
  ...
  if(req.method==='POST'&&p==='/api/ai/chat'){
    ...
    }catch(error){console.error('OpenAI request failed',error?.status||error?.name||'error');return json(res,502,{error:'AI_PROVIDER_ERROR',aiStatus:AI_STATUS.DEGRADED,reason:'OPENAI_PROVIDER_ERROR'});}
  }
...
export const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && (url.pathname === '/__client_error' || url.pathname === '/__client_rejection')) {
      const phase = String(url.searchParams.get('phase') || url.pathname.slice(3)).slice(0, 80);
      if (!config.isProduction) {
        const message = String(url.searchParams.get('m') || '').slice(0, 1800);
        console.error('[SYLORA_CLIENT]', { phase, message });
      }
      res.writeHead(204);
      res.end();
      return;
    }
    const mediaMatch = route('/media/:id', url.pathname);
    if (mediaMatch && req.method === 'GET') return serveMedia(req, res, mediaMatch.id);
    const hlsMatch = route('/hls/:mediaId/:file', url.pathname);
    if (hlsMatch && req.method === 'GET') return serveHls(req, res, hlsMatch.mediaId, hlsMatch.file);
    if (url.pathname.startsWith('/api/')) {
      if (!await allowRequest(req)) return json(res, 429, { error: 'RATE_LIMITED' });
      await api(req, res, url);
    } else staticFile(req, res, url);
  } catch (e) {
    console.error(e?.name || 'request_error');
    const allowed = new Set(['BODY_TOO_LARGE', 'INVALID_JSON', 'UNSUPPORTED_MEDIA_TYPE', 'MEDIA_TOO_LARGE', 'INVALID_VIDEO_FILE']);
    const code = safeErrorCode(e, allowed);
    if (!res.headersSent) json(res, e.message === 'BODY_TOO_LARGE' ? 413 : 400, { error: code });
    else res.end();
  }
});
