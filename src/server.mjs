...
import { assertProductionBoot, evaluateHealth, evaluateReadiness, loadConfig } from './config.mjs';
...
const liveIceServers=config.webrtc.iceServers;
const openai=config.ai.configured?new OpenAI({apiKey:config.ai.apiKey,...(config.ai.baseUrl?{baseURL:config.ai.baseUrl}:{})}):null;
...
async function dependencyHealth(){const [pg,cache,outbox]=await Promise.all([postgres.ping(),redis.ping(),outboxRepo.health()]);return evaluateReadiness(config,{postgres:pg,redis:cache,outbox})}
function publicAiStatus(){return {status:config.ai.status,reason:config.ai.reason,configured:config.ai.configured,fallback:config.ai.fallback,provider:config.ai.provider,model:config.ai.configured?config.ai.model:null}}
...
async function api(req, res, url) {
  ...
  if(req.method==='GET'&&p==='/api/health'){const readiness=await dependencyHealth();return json(res,200,{...evaluateHealth(config),persistence:readiness.persistence,ecosystem:'personal-ai-identity-kg-agents-developers',ecosystemPersistence:ecosystemRepo.enabled?'postgres+json-cache':'json',ai:publicAiStatus(),dependencies:readiness.dependencies})}
  if(req.method==='GET'&&p==='/api/ai/status'){return json(res,200,publicAiStatus())}
  ...
  if(req.method==='GET'&&p==='/api/ready'){const readiness=await dependencyHealth();return json(res,readiness.ready?200:503,readiness)}
  ...
  if(req.method==='POST'&&p==='/api/ai/realtime'){
    const user=await requireUser(req,res);if(!user)return;if(!config.ai.configured)return json(res,503,{error:'AI_PROVIDER_NOT_CONFIGURED',...publicAiStatus()});if(!await allowAi(user.id))return json(res,429,{error:'AI_RATE_LIMITED'});
    ...
    try{const upstream=await fetch('https://api.openai.com/v1/realtime/calls',{method:'POST',headers:{authorization:`Bearer ${config.ai.apiKey}`,'OpenAI-Safety-Identifier':tokenHash(user.id)},body:form});const answer=await upstream.text();if(!upstream.ok)return json(res,502,{error:'REALTIME_SESSION_FAILED',aiStatus:'AI_DEGRADED'});res.writeHead(200,{'content-type':'application/sdp','cache-control':'no-store'});return res.end(answer)}catch(error){console.error('OpenAI Realtime session failed',error?.name||'error');return json(res,502,{error:'REALTIME_PROVIDER_ERROR',aiStatus:'AI_DEGRADED'})}
  }
  ...
  if(req.method==='GET'&&p==='/api/live/rtc-config'){const user=await requireUser(req,res);if(!user)return;return json(res,200,{iceServers:liveIceServers,turnConfigured:hasTurnServer(liveIceServers),stunConfigured:config.webrtc.stunConfigured,credentialPolicy:config.webrtc.credentialPolicy})}
  ...
  if(req.method==='POST'&&p==='/api/ai/chat'){
    const user=await requireUser(req,res);if(!user)return;if(!openai)return json(res,503,{error:'AI_PROVIDER_NOT_CONFIGURED',...publicAiStatus()});if(!await allowAi(user.id))return json(res,429,{error:'AI_RATE_LIMITED'});
...
export const server = http.createServer(async (req, res) => {
  try { securityHeaders(res); const url = new URL(req.url, `http://${req.headers.host || 'localhost'}`); if(req.method==='GET'&&(url.pathname==='/__client_error'||url.pathname==='/__client_rejection')){if(config.isProduction){res.writeHead(404);return res.end('Not found')}const phase=String(url.searchParams.get('phase')||url.pathname.slice(3)).slice(0,80),message=String(url.searchParams.get('m')||'').slice(0,1800);console.error('[SYLORA_CLIENT]',{phase,message});res.writeHead(204);res.end();return;} const mediaMatch=route('/media/:id',url.pathname); if(mediaMatch&&req.method==='GET')return serveMedia(req,res,mediaMatch.id);const hlsMatch=route('/hls/:mediaId/:file',url.pathname);if(hlsMatch&&req.method==='GET')return serveHls(req,res,hlsMatch.mediaId,hlsMatch.file); if (url.pathname.startsWith('/api/')) { if (!await allowRequest(req)) return json(res,429,{error:'RATE_LIMITED'}); await api(req,res,url); } else staticFile(req,res,url); }
  catch (e) { const known=e.message==='BODY_TOO_LARGE'||e.message==='INVALID_JSON'; console.error(e?.name||'request_error'); if (!res.headersSent) json(res, e.message === 'BODY_TOO_LARGE' ? 413 : 400, { error: (!config.isProduction || known) ? (e.message || 'BAD_REQUEST') : 'BAD_REQUEST' }); else res.end(); }
});
