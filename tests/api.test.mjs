...
test('auth → post → gift → ledger works end to end', async () => {
  ...
  try {
    ...
    const hostSignal=await openSignalStream(alice.token,'host-test-1','host');
    ...
    const duplicateHostPeer=await openSignalStream(alice.token,'host-test-1','host');assert.equal(duplicateHostPeer.response.status,409);duplicateHostPeer.controller.abort();
    const secondHost=await openSignalStream(alice.token,'host-test-2','host');assert.equal(secondHost.response.status,409);secondHost.controller.abort();
    const viewerOnHostPeer=await openSignalStream(bob.token,'host-test-1','viewer');assert.equal(viewerOnHostPeer.response.status,409);viewerOnHostPeer.controller.abort();
    const bobSignal=await openSignalStream(bob.token,'viewer-test-1','viewer');assert.equal(bobSignal.response.status,200);assert.match(await nextSseEvent(bobSignal),/event: presence/);
    const carolSignal=await openSignalStream(carol.token,'viewer-test-2','viewer');assert.equal(carolSignal.response.status,200);assert.match(await nextSseEvent(carolSignal),/event: presence/);
    const unfencedHostReady=await fetch(`${base}/api/live/${live.live.id}/signal`,{method:'POST',headers:{...auth,'content-type':'application/json'},body:JSON.stringify({kind:'host-ready',fromPeerId:'host-test-1',data:{transport:'p2p',peerLimit:6}})});assert.equal(unfencedHostReady.status,400);assert.equal((await unfencedHostReady.json()).error,'HOST_STREAM_TOKEN_REQUIRED');
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:auth,body:JSON.stringify({kind:'host-ready',fromPeerId:'host-test-1',streamToken:hostStreamToken,data:{transport:'p2p',peerLimit:6}})});
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:bobAuth,body:JSON.stringify({kind:'viewer-ready',fromPeerId:'viewer-test-1',data:{}})});
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:{authorization:`Bearer ${carol.token}`},body:JSON.stringify({kind:'viewer-ready',fromPeerId:'viewer-test-2',data:{}})});
    const readySignals=[signalJson(await nextSseEvent(hostSignal)),signalJson(await nextSseEvent(hostSignal))];
    assert.deepEqual(new Set(readySignals.map(item=>item.fromPeerId)),new Set(['viewer-test-1','viewer-test-2']));assert.equal(readySignals.every(item=>item.kind==='viewer-ready'&&item.toPeerId==='host-test-1'),true);assert.doesNotMatch(JSON.stringify(readySignals),/targetUserId/);
    const offerData={type:'offer',sdp:'v=0\r\no=sylora 1 1 IN IP4 127.0.0.1\r\n'};
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:auth,body:JSON.stringify({kind:'offer',fromPeerId:'host-test-1',toPeerId:'viewer-test-1',streamToken:hostStreamToken,data:offerData})});
    const bobOffer=signalJson(await nextSseEvent(bobSignal));assert.equal(bobOffer.kind,'offer');assert.equal(bobOffer.toPeerId,'viewer-test-1');
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:auth,body:JSON.stringify({kind:'offer',fromPeerId:'host-test-1',toPeerId:'viewer-test-2',streamToken:hostStreamToken,data:offerData})});
    const carolOffer=signalJson(await nextSseEvent(carolSignal));assert.equal(carolOffer.kind,'offer');assert.equal(carolOffer.toPeerId,'viewer-test-2');assert.notEqual(carolOffer.toPeerId,bobOffer.toPeerId);
    ...
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:auth,body:JSON.stringify({kind:'ice',fromPeerId:'host-test-1',toPeerId:'viewer-test-1',streamToken:hostStreamToken,data:iceData})});
    const bobIce=signalJson(await nextSseEvent(bobSignal));assert.equal(bobIce.kind,'ice');assert.equal(bobIce.toPeerId,'viewer-test-1');
    const answerData={type:'answer',sdp:'v=0\r\no=viewer 1 1 IN IP4 127.0.0.1\r\n'};
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:bobAuth,body:JSON.stringify({kind:'answer',fromPeerId:'viewer-test-1',toPeerId:'host-test-1',data:answerData})});
    const hostAnswer=signalJson(await nextSseEvent(hostSignal));assert.equal(hostAnswer.kind,'answer');assert.equal(hostAnswer.fromPeerId,'viewer-test-1');
    const viewerToViewer=await fetch(`${base}/api/live/${live.live.id}/signal`,{method:'POST',headers:{...bobAuth,'content-type':'application/json'},body:JSON.stringify({kind:'answer',fromPeerId:'viewer-test-1',toPeerId:'viewer-test-2',data:answerData})});assert.equal(viewerToViewer.status,409);
    const hostToHostIce=await fetch(`${base}/api/live/${live.live.id}/signal`,{method:'POST',headers:{...auth,'content-type':'application/json'},body:JSON.stringify({kind:'ice',fromPeerId:'host-test-1',toPeerId:'host-test-1',streamToken:hostStreamToken,data:iceData})});assert.equal(hostToHostIce.status,409);
    const spoofedViewerPeer=await fetch(`${base}/api/live/${live.live.id}/signal`,{method:'POST',headers:{authorization:`Bearer ${carol.token}`,'content-type':'application/json'},body:JSON.stringify({kind:'ice',fromPeerId:'viewer-test-1',toPeerId:'host-test-1',data:iceData})});assert.equal(spoofedViewerPeer.status,403);
    const oversizedSignal=await fetch(`${base}/api/live/${live.live.id}/signal`,{method:'POST',headers:{...auth,'content-type':'application/json'},body:JSON.stringify({kind:'offer',fromPeerId:'host-test-1',toPeerId:'viewer-test-1',streamToken:hostStreamToken,data:{type:'offer',sdp:'x'.repeat(71*1024)}})});assert.equal(oversizedSignal.status,413);
    ...
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:auth,body:JSON.stringify({kind:'host-left',fromPeerId:'host-test-1',streamToken:hostStreamToken,data:null})});hostSignal.controller.abort();
    const hostOnViewerPeer=await openSignalStream(alice.token,'viewer-test-2','host');assert.equal(hostOnViewerPeer.response.status,409);hostOnViewerPeer.controller.abort();
    const successorHost=await openSignalStream(alice.token,'host-test-1','host'),successorToken=successorHost.response.headers.get('x-sylora-host-stream-token');assert.equal(successorHost.response.status,200);assert.match(successorToken,/^[0-9a-f-]{36}$/i);assert.notEqual(successorToken,hostStreamToken);assert.match(await nextSseEvent(successorHost),/event: presence/);
    const staleOfferData={type:'offer',sdp:'v=0\r\no=stale-host 1 1 IN IP4 127.0.0.1\r\n'},staleOffer=await fetch(`${base}/api/live/${live.live.id}/signal`,{method:'POST',headers:{...auth,'content-type':'application/json'},body:JSON.stringify({kind:'offer',fromPeerId:'host-test-1',toPeerId:'viewer-test-2',streamToken:hostStreamToken,data:staleOfferData})});assert.equal(staleOffer.status,409);assert.equal((await staleOffer.json()).error,'LIVE_HOST_FENCE_LOST');
    const staleIce=await fetch(`${base}/api/live/${live.live.id}/signal`,{method:'POST',headers:{...auth,'content-type':'application/json'},body:JSON.stringify({kind:'ice',fromPeerId:'host-test-1',toPeerId:'viewer-test-2',streamToken:hostStreamToken,data:iceData})});assert.equal(staleIce.status,409);assert.equal((await staleIce.json()).error,'LIVE_HOST_FENCE_LOST');
    const staleHostLeft=await fetch(`${base}/api/live/${live.live.id}/signal`,{method:'POST',headers:{...auth,'content-type':'application/json'},body:JSON.stringify({kind:'host-left',fromPeerId:'host-test-1',streamToken:hostStreamToken,data:null})});assert.equal(staleHostLeft.status,409);assert.equal((await staleHostLeft.json()).error,'LIVE_HOST_FENCE_LOST');
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:auth,body:JSON.stringify({kind:'host-ready',fromPeerId:'host-test-1',streamToken:successorToken,data:{transport:'p2p',peerLimit:6}})});const successorOfferData={type:'offer',sdp:'v=0\r\no=successor-host 1 1 IN IP4 127.0.0.1\r\n'};await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:auth,body:JSON.stringify({kind:'offer',fromPeerId:'host-test-1',toPeerId:'viewer-test-2',streamToken:successorToken,data:successorOfferData})});const successorOffer=signalJson(await nextSseEvent(carolSignal));assert.equal(successorOffer.kind,'offer');assert.equal(successorOffer.data.sdp,successorOfferData.sdp);
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:bobAuth,body:JSON.stringify({kind:'viewer-left',fromPeerId:'viewer-test-1',toPeerId:'host-test-1',data:null})});
    const staleViewerPeer=await fetch(`${base}/api/live/${live.live.id}/signal`,{method:'POST',headers:{...bobAuth,'content-type':'application/json'},body:JSON.stringify({kind:'answer',fromPeerId:'viewer-test-1',toPeerId:'host-test-1',data:answerData})});assert.equal(staleViewerPeer.status,403);
    await call(`/api/live/${live.live.id}/signal`,{method:'POST',headers:auth,body:JSON.stringify({kind:'host-left',fromPeerId:'host-test-1',streamToken:successorToken,data:null})});
