...
test('docker entrypoint and Dockerfile drop privileges without world-writable mode', () => {
  ...
  assert.match(deploy, /--env-file "\$COMPOSE_ENV_FILE"/);
  ...
  assert.match(deploy, /prepare_compose_env/);
  assert.match(deploy, /\.compose-interp\.env/);
  assert.match(deploy, /capture_deploy_diagnostics/);
  assert.match(deploy, /EMPTY_STACK/);
  assert.doesNotMatch(stripComments(deploy), /down\s+-v/);
  assert.doesNotMatch(stripComments(deploy), /\brm\s+-rf\s+.*\/app\/data/);
  assert.doesNotMatch(stripComments(deploy), /volume\s+prune|volume\s+rm|system\s+prune/);
});
